...
```bash
./build_uitoolkit_app.sh
```

This will:

1. Run `build.py` to generate `data.js` and **`catalog.json`**
2. Install dependencies in `imucl_uitoolkit/` (React, @splunk/react-ui, Vite)
3. Build the React app with Vite → `imucl_uitoolkit/dist/`
4. Copy `catalog.json` and `dist/assets/*` into `splunk_app/appserver/static/`
5. Package the Splunk app as **`imucl-uitoolkit-0.0.1.spl`** (tar.gz)

Then install in Splunk: **Apps → Install app from file** → choose `imucl-uitoolkit-0.0.1.spl`.
