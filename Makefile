...
audit-full: audit audit-placeholders audit-mitre audit-spl-duplicates audit-spl-grammar audit-spl-anti-patterns-check audit-ids audit-monitoring-type audit-codeowners-coverage audit-exclusions-coverage audit-dashboards audit-sarif audit-observability-drift audit-cost-coverage audit-vendor-changelog ## Run ALL audit checks
...
audit-ids: ## Validate UC IDs (duplicates, category match, permanent-identity ledger)
	$(SPLUNK_UC) audit-uc-ids

audit-id-ledger: ## Regenerate data/id-ledger.json from the live catalogue
	$(SPLUNK_UC) generate-id-ledger

audit-id-ledger-check: ## CI drift gate for data/id-ledger.json
	$(SPLUNK_UC) generate-id-ledger --check

audit-uc-id-migration-blast: ## Fail when bulk identifier changes lack a migration manifest
	$(SPLUNK_UC) audit-uc-id-migration-blast --check
...
generate-observability-metrics: ## Emit dist/observability freshness/quality/coverage + catalogue.prom
	$(SPLUNK_UC) generate-observability-metrics

audit-observability-drift: generate-observability-metrics ## Validate dist/observability artefacts
	$(SPLUNK_UC) audit-observability-drift --check
...
sync-generated: _require-py311 ## Run every cascade-style generator (write-mode) in dependency-safe order
	...
	@$(SPLUNK_UC) generate-mapping-ledger
	@echo "==> [12/14] derived artefact: splunk-uc-recommender app"
	@$(SPLUNK_UC) generate-recommender-app
...
sync-generated-check: _require-py311 ## CI drift gate — run every cascade generator with --check (exits non-zero on drift)
	...
	for step in \
	  ...
	  "generate-recommender-app --check"; do \
	  echo "==> $$step"; \
