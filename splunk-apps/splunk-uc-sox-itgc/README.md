...
This app packages **78 use cases** from the upstream catalogue that cite SOX — PCAOB AS 2201 ITGCs (`sox-itgc`), together with the macros, eventtypes, tags, and lookup needed to operate them.  Every saved search is shipped **disabled by default** so an operator can review the SPL and tune indexes before enabling.

* Regulation tier: **1**
* Jurisdictions: US
* Versions covered: PCAOB AS 2201
* UCs by criticality: critical = 27, high = 29, low = 9, medium = 13
...
| `ITGC.ChangeMgmt.Approval` | 6 |
| `ITGC.Logging.Continuity` | 4 |
| `ITGC.Access.Review` | 3 |
...
|----|-------|-------------|---------|
| UC-5.13.32 | Compliance Drift Detection (Was Compliant, Now Not) | high | ITGC.Change.Mgmt |
| UC-5.13.45 | Audit Log Activity Overview | high | ITGC.Logging.Review |
| UC-5.13.46 | Configuration Change Audit Trail | high | ITGC.ChangeMgmt.Approval |
| UC-5.13.48 | Failed API/Login Attempt Detection | critical | ITGC.Access.Review |
| UC-5.13.49 | After-Hours Administrative Activity | high | ITGC.Access.Review |
