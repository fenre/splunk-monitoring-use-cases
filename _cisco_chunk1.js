...
var SITE = Object.assign({
  ...
  filterAll: "Categories", siteAuthor: "", siteRepoUrl: "https://github.com/fenre/splunk-monitoring-use-cases"
}, window.SITE_CUSTOM || {});
...
var SIDEBAR_GROUP_LABELS = { infra:'Infrastructure', security:'Security', cloud:'Cloud & Containers', app:'Applications', industry:'Industry', compliance:'Compliance' };
...
var _eqById = {};
(EQUIPMENT || []).forEach(function(eq) { _eqById[eq.id] = eq; });
...
function getFilteredUCs() {
  ...
  if (currentSearch) {
    ...
    result = result.filter(function(e) {
      var uc = e.uc;
      var blob = [uc.n, uc.i, uc.v, uc.q, uc.t, uc.d, e.cat.n, e.sc.n].join(' ');
...
function githubIssueUrlForEntry(entry) {
  ...
  var issuesNew = mm ? 'https://github.com/' + mm[1] + '/' + mm[2] + '/issues/new' : 'https://github.com/fenre/splunk-monitoring-use-cases/issues/new';
  ...
  body += cat.src ? 'Open [`use-cases/' + cat.src + '`](' + mdLink + ').\n\n' : 'Browse [`use-cases/`](' + mdLink + ').\n\n';
  body += '### This page\n' + pageLink + '\n\n### What is wrong?\n\n### Suggested fix (optional)\n';
  return issuesNew + '?title=' + encodeURIComponent(issueTitle) + '&body=' + encodeURIComponent(body);
}
