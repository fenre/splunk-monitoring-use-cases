...
This reads `use-cases/*.md` and `content/INDEX.md`, then outputs `data.js` and `catalog.json`. Refresh the dashboard to see your changes.
...
```
.
├── use-cases/              Source of truth: 23 category markdown files (+ preamble); `INDEX.md` here is a symlink to `content/INDEX.md`
...
├── content/                Per-UC sidecars + `INDEX.md` (category metadata); each UC may carry a grandmaExplanation (ge) field (generated by scripts/generate_grandma_explanations.py)
