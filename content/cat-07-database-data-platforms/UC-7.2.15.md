<!-- AUTO-GENERATED from UC-7.2.15.json — DO NOT EDIT -->

---
id: "7.2.15"
title: "Redis Memory Fragmentation (Cache Tier)"
criticality: "medium"
...
*We follow memory, persistence, and replication on Redis so we can catch eviction and lag before a cache or queue tier becomes the bottleneck for applications.*

---

## Description

High `mem_fragmentation_ratio` on self-managed Redis (not only ElastiCache) wastes RAM and increases latency. Tracks non-cloud Redis clusters in the NoSQL section.
...
Line chart (fragmentation ratio), Table (hosts over threshold), Gauge (current ratio).

## Known False Positives

AOF rewrites, RDB saves, and replica full syncs can spike latency or CPU briefly on healthy hosts; check Redis persistence settings and the backup schedule.
