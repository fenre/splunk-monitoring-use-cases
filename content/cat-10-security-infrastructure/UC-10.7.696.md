<!-- AUTO-GENERATED from UC-10.7.696.json — DO NOT EDIT -->
...
# UC-10.7.696 · DNS Query Length Outliers - MLTK

> **Criticality:** High &middot; **Difficulty:** Intermediate &middot; **Pillar:** Security &middot; **Type:** Security

*We keep an eye on dNS requests with unusually large query lengths for the record type being requested so unusual activity is visible early — before it spreads or hides in normal noise.*
...
```spl
| tstats `security_content_summariesonly` count min(_time) as start_time max(_time) as end_time values(DNS.src) as src values(DNS.dest) as dest FROM datamodel=Network_Resolution
