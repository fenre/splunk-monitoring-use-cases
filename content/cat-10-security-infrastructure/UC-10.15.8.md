...
*We use behavior baselines to catch odd program launches and parent chains that single-event rules would miss in the noise.*
...
Score spikes from new role assignments, training completions, or VPN egress IP changes can look like malice when the person simply changed job — we pair with identity.
