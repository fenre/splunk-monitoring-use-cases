<!-- AUTO-GENERATED from UC-10.13.17.json — DO NOT EDIT -->
...
*We use the same kind of check described for "Recurring Infection (Same Host)" once our data is shaped in a common, repeatable way across products. We help the team see trouble early, compare different tools fairly, and avoid building a one-off report for every vendor.*
...
- `stats` rolls up events into metrics; results are split **by dest, signature** so each row reflects one combination of those dimensions.
...
Malware and quarantine events from re-scans after cleanups, slow endpoints finishing remediation, and test infections in sandboxes; confirm host state with EDR.
