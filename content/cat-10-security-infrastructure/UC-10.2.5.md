...
*We look for known attack patterns on internal network paths so we can tell when someone is moving around inside the environment after a breach, not just knocking on the front door.*
...
- `stats` rolls up events into metrics; results are split **by src, dest, signature** so each row reflects one combination of those dimensions.
...
Lateral movement style signatures are often tripped by legitimate admin tools (remote desktop, PowerShell, WMI, psexec-style utilities), jump hosts, backup agents, and scheduled configuration sync. Map maintenance windows and break-glass paths before you escalate.
