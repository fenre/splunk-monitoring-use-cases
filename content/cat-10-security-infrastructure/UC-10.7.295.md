<!-- AUTO-GENERATED from UC-10.7.295.json — DO NOT EDIT -->
...
# UC-10.7.295 · Detect Remote Access Software Usage Registry

> **Criticality:** High &middot; **Difficulty:** Intermediate &middot; **Pillar:** Security &middot; **Type:** Security

*We look for the endpoint file, process, or registry signals described in 'Detect Remote Access Software Usage Registry' so we catch abuse of common tools and persistence paths in time.*
...
```spl
| from datamodel:Endpoint.Registry| search dest=$dest$ registry_path=$registry_path$
```

## CIM SPL

```spl
| tstats summariesonly=t count from datamodel=Endpoint.Registry by Registry.dest, Registry.user, Registry.registry_path
```
