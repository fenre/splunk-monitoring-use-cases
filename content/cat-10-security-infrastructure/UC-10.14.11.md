<!-- AUTO-GENERATED from UC-10.14.11.json — DO NOT EDIT -->
...
*We watch for safety or interlock systems being sidestepped in ways that only happen when someone tries to work around the guardrails.*
...
- `stats` rolls up events into metrics; results are split **by loop_id, asset, user** so each row reflects one combination of those dimensions.
...
Test and drill cycles for safety systems, and written overrides during documented hazards, can trip bypass alerts even when the plant follows procedure — we verify procedure IDs.
