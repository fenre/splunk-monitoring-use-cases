<!-- AUTO-GENERATED from UC-10.7.418.json — DO NOT EDIT -->
...
# UC-10.7.418 · Processes Tapping Keyboard Events

> **Criticality:** High &middot; **Difficulty:** Intermediate &middot; **Pillar:** Security &middot; **Type:** Security

*We keep an eye on processes on macOS systems that are tapping keyboard events, potentially monitoring all keystrokes made by a user so unusual activity is visible early — before it spreads or hides in normal noise.*
...
Known false positives for this detection: Accessibility helpers, screen readers, remote support tools, and some security products hook keyboard input for legitimate reasons. Confirm intent on the asset and with the user before treating as malicious keylogging.
...
```spl
| tstats `summariesonly` count from datamodel=Alerts.Alerts by Alerts.dest Alerts.signature span=1h | sort -count
```
...
Accessibility helpers, screen readers, remote support tools, and some security products hook keyboard input for legitimate reasons. Confirm intent on the asset and with the user before treating as malicious keylogging.
