<!-- AUTO-GENERATED from UC-10.10.13.json — DO NOT EDIT -->
...
*We study the false side of YARA so we can tune or retire noisy rules and stop burning analyst hours.*
...
- `stats` rolls up events into metrics; results are split **by rule_name, verdict** so each row reflects one combination of those dimensions.
...
### Step 3 — Validate
Compare exported false-positive labels to the **closed disposition** in Incident Review for the same cases where both exist.
...
YARA false positive analysis includes analyst disagreements, legacy samples, and noisy rules being tuned — the word “false” in export tools does not always match final investigation outcomes.
