<!-- AUTO-GENERATED from UC-10.4.29.json — DO NOT EDIT -->
...
# UC-10.4.29 · Gsuite Drive Share In External Email
...
ESCU Risk-Based Alerting detection. Enable in ES Content Management as a Correlation Search. Generates risk events attributed to user account entities — Notable Events fire when an entity’s cumulative risk exceeds the configured threshold. Requires G Suite Drive ingested and CIM-normalized.
...
| tstats `summariesonly` count
  ...
  by All_Email.src_user All_Email.recipient All_Email.url span=1h
| sort -count
```
...
Planned share links that reference external storage for collaboration (design reviews, M&A datarooms) where governance allows external domains. Validate against sharing policy and the drive audit trail.
