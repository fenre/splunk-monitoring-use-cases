<!-- AUTO-GENERATED from UC-10.4.2.json — DO NOT EDIT -->

---
id: "10.4.2"
title: "Malicious Attachment Tracking"
...
*We watch for malware and high-risk attachments in the mail path, so dangerous files are blocked or flagged before they reach inboxes and spread inside the org.*

---

## Description

Attachment-based threats bypass URL filtering. Tracking by file type reveals attack vectors and informs policy decisions.
...
- `stats` rolls up events into metrics; results are split **by SenderAddress, Subject, FilteringResult** so each row reflects one combination of those dimensions.
...
| tstats `summariesonly` count
  ...
  where (All_Email.action="*malware*" OR All_Email.signature="*malware*")
  by All_Email.src_user All_Email.recipient All_Email.file_name
| sort -count
```
...
Bar chart (detections by file type), Table (malicious attachments), Line chart (detection trend), Pie chart (verdict distribution).

## Known False Positives

Authorized security test attachments (EICAR), user awareness attachments in training modules, and vendor compliance test files that intentionally resemble malware for validation.
