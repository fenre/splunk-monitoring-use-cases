<!-- AUTO-GENERATED from UC-10.7.111.json — DO NOT EDIT -->
...
# UC-10.7.111 · AWS Exfiltration via Anomalous GetObject API Activity

> **Criticality:** High &middot; **Difficulty:** Intermediate &middot; **Pillar:** Security &middot; **Type:** Security

*We watch this AWS and CloudTrail pattern so we can see risky API or login behavior while it can still be tied to a user or role in the console.*
...
ESCU Risk-Based Alerting detection. Enable in ES Content Management as a Correlation Search. Generates risk events attributed to IP address entities — Notable Events fire when an entity’s cumulative risk exceeds the configured threshold. Requires AWS CloudTrail GetObject ingested and CIM-normalized.
...
Suspicious AWS API call patterns from Terraform, Ansible, Pulumi, Steampipe, Prowler, CICD, or cloud management agents; map calls to the owning pipeline when possible.
...
- [https://help.splunk.com/en/splunk-enterprise/search/spl-search-reference/9.4/search-commands/anomalydetection](https://help.splunk.com/en/splunk-enterprise/search/spl-search-reference/9.4/search-commands/anomalydetection)
