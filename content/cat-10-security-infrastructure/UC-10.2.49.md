...
*We catch the execution of the PowerShell cmdlet using PowerShell Script Block Logging (EventCode=4104) — so we know when protection is off, out of date, or bypassed.*
...
Recon-style signals come from asset inventory scanners, new device onboarding, NAC health probes, and authorized discovery tools run by network operations.
