<!-- AUTO-GENERATED from UC-10.4.30.json — DO NOT EDIT -->
...
# UC-10.4.30 · GSuite Email Suspicious Attachment
...
ESCU Risk-Based Alerting detection. Enable in ES Content Management as a Correlation Search. Generates risk events attributed to email address entities — Notable Events fire when an entity’s cumulative risk exceeds the configured threshold. Requires G Suite Gmail ingested and CIM-normalized.
...
| tstats `summariesonly` count
  ...
  where (All_Email.signature="*suspicious*" OR All_Email.action="blocked")
  by All_Email.src_user All_Email.file_name
| sort -count
```
...
Unusual-looking but benign file types in specialized teams (data science, dev packages) and quarantine backlogs after a policy change. Cross-check the Google Workspace quarantine and admin report.
