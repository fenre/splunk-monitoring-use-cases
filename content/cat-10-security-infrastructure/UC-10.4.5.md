<!-- AUTO-GENERATED from UC-10.4.5.json — DO NOT EDIT -->
...
*We watch for mail that fails or weakens sender authentication, so spoofing and lookalike senders are caught and we can keep improving DMARC, SPF, and DKIM alignment.*
...
- `stats` rolls up events into metrics; results are split **by src, header_from, dkim_result, spf_result, disposition** so each row reflects one combination of those dimensions.
...
| tstats `summariesonly` count
  ...
  where (All_Email.message_info="*spf*" OR All_Email.message_info="*dkim*" OR All_Email.message_info="*dmarc*" OR All_Email.signature="*spoof*")
...
SPF, DKIM, and DMARC failures from legitimate forwarders, mailing lists, email aliases, poorly configured but valid senders, and domains during M&A or DNS transitions.
