<!-- AUTO-GENERATED from UC-10.7.687.json — DO NOT EDIT -->
...
# UC-10.7.687 · Detect DNS Query to Decommissioned S3 Bucket

> **Criticality:** High &middot; **Difficulty:** Intermediate &middot; **Pillar:** Security &middot; **Type:** Security

*We keep an eye on dNS queries to domains that match previously decommissioned S3 buckets so unusual activity is visible early — before it spreads or hides in normal noise.*
...
ESCU Risk-Based Alerting detection. Enable in ES Content Management as a Correlation Search. Generates risk events attributed to domain entities — Notable Events fire when an entity’s cumulative risk exceeds the configured threshold. Requires Sysmon EventID 22 ingested and CIM-normalized.
...
```spl
| from datamodel:Network_Resolution | search src="$src$
```
