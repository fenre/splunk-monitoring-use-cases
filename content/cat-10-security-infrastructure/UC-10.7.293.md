<!-- AUTO-GENERATED from UC-10.7.293.json — DO NOT EDIT -->
...
# UC-10.7.293 · Detect Remote Access Software Usage File

> **Criticality:** High &middot; **Difficulty:** Intermediate &middot; **Pillar:** Security &middot; **Type:** Security

*We look for the endpoint file, process, or registry signals described in 'Detect Remote Access Software Usage File' so we catch abuse of common tools and persistence paths in time.*

---

## Description

This detection identifies Detect Remote Access Software Usage File. It supports security monitoring and incident response. Part of Splunk Security Essentials (ESCU) where applicable.
...
ESCU detection using the Endpoint CIM. Enable the correlation in Content Management, enable DMA for the dataset (Filesystem, Processes, or Registry as applicable), and tune lookups for approved tools.

## Detailed Implementation

Enterprise Security Detection Rule

"Detect Remote Access Software Usage File" is a TTP (Tactics, Techniques, and Procedures) detection that identifies known adversary behaviors with high fidelity, sourced from the Splunk Enterprise Security Content Update (ESCU). The SPL shown is the ESCU investigation drilldown against the Endpoint Filesystem CIM. Use it during triage; the main detection and correlation are configured in Content Management.
...
```spl
| from datamodel:Endpoint.Filesystem | search dest=$dest$ file_name=$file_name$
```
...
Approved remote support tools, help desk software, and packaged installers can match; use ESCU allowlists and the asset record before closing.
