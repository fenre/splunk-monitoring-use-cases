<!-- AUTO-GENERATED from UC-10.2.3.json — DO NOT EDIT -->
...
*We compare our network map to the alerts we are actually seeing so we are not blind on segments where a sensor is down, mis-aimed, or missing.*
...
### Step 3 — Validate
Compare the lookup’s expected coverage with your real network design and with each sensor’s management view (Snort, Suricata, or Firepower). Confirm whether a silent segment is truly idle, newly built, or missing a feed—then verify Splunk inputs, time zones, and index permissions.
...
Legitimately quiet segments (cold disaster recovery, unused labs) may have no traffic and therefore no alerts even when sensors are fine. Stale network inventories after redesigns can falsely show a gap; refresh segment and sensor mappings before you treat silence as a failure.
