<!-- AUTO-GENERATED from UC-10.6.7.json — DO NOT EDIT -->
...
*We get pushed when a serious new problem shows up on a machine so we can jump on it before the risk sits there unnoticed.*
...
- Filters to critical and high in the `where` clause; pair with your index `first_seen` window for the operational "new" definition.

Enable Data Model Acceleration (and metric indexes for `mstats`) for the models or datasets referenced above; otherwise `tstats`/`mstats` may return no results from summaries.


### Step 3 — Validate
Compare "new" or critical findings in Splunk with the same time window in Tenable.io and Qualys VMDR (or cloud agent). For selected CVEs, compare with NIST NVD and internal ticketing to confirm a true new exposure versus feed noise.
...
Spikes from new asset discovery, scanner template or plugin updates, or after major patch releases (Patch Tuesday). A "new" row can reappear if scan metadata resets or deduping keys change.
