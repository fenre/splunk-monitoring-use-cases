...
*We check that most endpoints and servers still show healthy agent or log presence so a silent gap is not hiding the next incident.*
...
Endpoint coverage can dip during imaging delays, decommission lag in inventory, or air-gapped sites that are legitimately off the grid in policy.
