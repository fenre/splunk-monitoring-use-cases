<!-- AUTO-GENERATED from UC-10.14.4.json — DO NOT EDIT -->
...
*We catch logic downloads to controllers outside agreed maintenance windows so nobody reprograms a line in secret.*
...
- `stats` rolls up events into metrics; results are split **by plc_id, src_host, user** so each row reflects one combination of those dimensions.


### Step 3 — Validate
Compare each hit to the **CMMS / maintenance** lookup: confirm whether a `plc_id` was inside an approved window and whether the `user` matches the ticketed tech.
...
PLC config changes during planned maintenance windows, recipe changes, or controlled engineering changes can match this alert — we confirm against CMMS and change records.
