<!-- AUTO-GENERATED from UC-10.4.3.json — DO NOT EDIT -->
...
*We watch when people click links that were scanned or rewritten in email, so we can respond if someone reaches a harmful page even when the message first looked acceptable.*
...
| tstats `summariesonly` count
  ...
  where All_Email.action="clicked"
  by All_Email.recipient All_Email.url span=1h
| sort -count
```
...
```spl
index=email (sourcetype="o365:management:activity" Workload=Dlp) OR sourcetype="proofpoint:click"
| search verdict="malicious" AND action="allowed"
| table _time, userPrincipalName, url, verdict, action
```
...
Clicks from URL scanning tools, sandbox detonation, security analyst research, and end users following simulated phishing or newsletter redirectors.
