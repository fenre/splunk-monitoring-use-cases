<!-- AUTO-GENERATED from UC-10.2.167.json — DO NOT EDIT -->
...
# UC-10.2.167 · SMB Traffic Spike - MLTK

> **Criticality:** High &middot; **Difficulty:** Intermediate &middot; **Pillar:** Security &middot; **Type:** Security

*We spot spikes in the number of Server Message Block (SMB) connections using the Machine Learning Toolkit (MLTK) — so we know when protection is off, out of date, or bypassed.*
...
```spl
| tstats `security_content_summariesonly` count values(All_Traffic.dest) as dest values(All_Traffic.dest_port) as port FROM datamodel=Network_Traffic
