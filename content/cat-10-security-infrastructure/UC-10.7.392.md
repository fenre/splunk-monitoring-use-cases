<!-- AUTO-GENERATED from UC-10.7.392.json — DO NOT EDIT -->
...
*We keep an eye on Local LLM Framework Query so unusual activity is visible early — before it spreads or hides in normal noise.*
...
Known false positives for this detection: Security research, package managers, new SaaS rollouts, and browser/CDN look-alikes can hit suspicious name lists. Allowlist vetted resolvers, dev sandboxes, and known collaboration apps after review.
...
```spl
| tstats `summariesonly` count from datamodel=Network_Resolution.DNS by DNS.src DNS.query span=1h | sort -count
```
...
Security research, package managers, new SaaS rollouts, and browser/CDN look-alikes can hit suspicious name lists. Allowlist vetted resolvers, dev sandboxes, and known collaboration apps after review.
