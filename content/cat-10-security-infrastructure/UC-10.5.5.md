...
*We review blocked and risky DNS security events so malware, phishing, and C2 do not get past the resolver unlogged.*
...
```spl
index=dns_security sourcetype="cisco:umbrella:dns" action="Blocked"
| stats count by internalIp, domain, categories
| where match(categories,"Malware|Command and Control|Phishing")
| sort -count
```
...
Sinkhole hits from stale IOC lists, security research, malware sandboxes, or resolvers in lab VLANs; correlate with EDR and asset context before escalating.
