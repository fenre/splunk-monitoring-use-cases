...
*We re-score risk with supervised models so the numbers line up with how analysts *actually* closed past cases, not just raw sums.*
...
Recalibration shifts scores when notables and risk history are re-labeled, or when a new model is trained on a different season than production — re-check lift on a holdout week.
