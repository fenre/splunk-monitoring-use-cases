<!-- AUTO-GENERATED from UC-10.14.5.json — DO NOT EDIT -->
...
*We watch the historian for odd bulk reads or changes so someone is not siphoning process secrets or history they should not take.*
...
- `stats` rolls up events into metrics; results are split **by _time, historian_host** so each row reflects one combination of those dimensions.
- `eventstats` rolls up events into metrics; results are split **by historian_host** so each row reflects one combination of those dimensions.
...
Historian spikes from batch reporting, backfills, and historian maintenance can look like data theft when the process is just busy — correlate with production schedules.
