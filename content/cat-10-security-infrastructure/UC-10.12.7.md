<!-- AUTO-GENERATED from UC-10.12.7.json — DO NOT EDIT -->
...
*In Financial Services, we pay close attention to "PCI DSS Log Review Compliance" in our day-to-day work so that fraud, waste, and compliance issues do not slip by quietly. We help teams focus on the few situations that need a person, not every routine blip in the data.*
...
- `stats` rolls up events into metrics; results are split **by sourcetype, host, severity** so each row reflects one combination of those dimensions.
...
Centralized log review noise when scanners or indexers are counted as in-scope, or when scope tags lag behind architecture changes; refresh inventory first.
