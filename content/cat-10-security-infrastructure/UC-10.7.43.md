...
*We use this 'Okta MFA Exhaustion Hunt' search to catch the exact behavior in the data we already collect, so the team can confirm or clear it in the right management console or identity store.*
...
Legitimate Okta sign-in, MFA, and app policy events during onboarding, new devices, and app updates; use Okta’s own session and device views for context.
