...
*We flag plain HTTP where policy expects HTTPS so sensitive browsing is not left exposed on the network.*
...
```spl
index=proxy sourcetype="cisco:umbrella:proxy" protocol="HTTP"
| search NOT url="http://ocsp." NOT url="http://crl."
| stats count by src_user, domain
| sort -count
```
