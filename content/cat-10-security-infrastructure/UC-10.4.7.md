...
*We watch quarantine and user release activity on mail, so both analyst actions and end-user pull-through are visible and abuse of release rights is easier to catch.*
...
- `stats` rolls up events into metrics; results are split **by FilteringResult, SenderAddress** so each row reflects one combination of those dimensions.
...
User-initiated releases of newsletters and vendor digests that were over-quarantined, and bulk mail re-scanned in waves after allowlist or policy changes.
