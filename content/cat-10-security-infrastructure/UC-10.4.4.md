...
*We watch when outbound or inbound mail hits data-loss policy blocks, so sensitive content leaving by email is visible for security and compliance follow-up.*
...
```spl
index=email sourcetype="o365:management:activity" Workload=Dlp
| stats count by PolicyName, UserPrincipalName, SensitiveInformationType
| sort -count
```
...
DLP hits on legitimate finance or HR content (reports with PII, contracts) sent to approved vendors, or on heavily redacted but still–policy-tagged material. Bulk scans after policy changes can spike events.
