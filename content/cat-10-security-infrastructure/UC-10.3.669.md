<!-- AUTO-GENERATED from UC-10.3.669.json — DO NOT EDIT -->
...
# UC-10.3.669 · Detect Remote Access Software Usage DNS

> **Criticality:** High &middot; **Difficulty:** Intermediate &middot; **Pillar:** Security &middot; **Type:** Security

*We use endpoint security telemetry to stay ahead of the behavior this rule calls “Detect Remote Access Software Usage DNS”, so we catch real tradecraft and not just noise from everyday work.*
...
ESCU Risk-Based Alerting detection. Enable in ES Content Management as a Correlation Search. Generates risk events attributed to detection signature entities — Notable Events fire when an entity’s cumulative risk exceeds the configured threshold. Requires Sysmon EventID 22 ingested and CIM-normalized.
...
```spl
| from datamodel:Network_Resolution.DNS | search src=$src$ query=$query$
```
