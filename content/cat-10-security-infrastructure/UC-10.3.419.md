<!-- AUTO-GENERATED from UC-10.3.419.json — DO NOT EDIT -->
...
# UC-10.3.419 · Windows Compatibility Telemetry Tampering Through Registry

> **Criticality:** High &middot; **Difficulty:** Intermediate &middot; **Pillar:** Security &middot; **Type:** Security

*We use endpoint security telemetry to stay ahead of the behavior this rule calls “Windows Compatibility Telemetry Tampering Through Registry”, so we catch real tradecraft and not just noise from everyday work.*
...
ESCU Risk-Based Alerting detection. Enable in ES Content Management as a Correlation Search. Generates risk events attributed to process entities — Notable Events fire when an entity’s cumulative risk exceeds the configured threshold. Requires Sysmon EventID 13 ingested and CIM-normalized.
...
```spl
| from datamodel Endpoint.Registry | search registry_path = "*\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\AppCompatFlags\\TelemetryController*" AND dest = "$dest$
```
...
Registry activity from PnP, USB class drivers, GPO refresh, and endpoint agents under the same keys; use process and parent context.

## References

- [https://attack.mitre.org/techniques/T1546/](https://attack.mitre.org/techniques/T1546/)
- [https://scythe.io/threat-thursday/windows-telemetry-persistence](https://scythe.io/threat-thursday/windows-telemetry-persistence)
