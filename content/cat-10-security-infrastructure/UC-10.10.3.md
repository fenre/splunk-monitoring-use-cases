<!-- AUTO-GENERATED from UC-10.10.3.json — DO NOT EDIT -->
...
*We find rules that have gone quiet so we are not holding broken glass while attackers walk past.*
...
### Step 3 — Validate
In **ES Content Management**, confirm the rule is enabled, scheduled, and scoped to indexes that have had traffic recently. Compare to a test-notable or manual run so “never fires” is not a permissions issue.
...
A rule that never fires can mean a quiet month or a dead rule (wrong data, over-filtering, or scope drift). Compare to coverage expectations and test data before retiring content.
