...
*We use the same kind of check described for "Impossible Travel Detection" once our data is shaped in a common, repeatable way across products. We help the team see trouble early, compare different tools fairly, and avoid building a one-off report for every vendor.*
...
index=security tag=authentication action=success user=*
| iplocation src
| streamstats window=2 global=f current(Country) as cur_c last(Country) as prev_c current(_time) as ts last(_time) as pts by user
| where isnotnull(prev_c) AND cur_c!=prev_c AND (ts-pts)<3600
| table _time, user, src, cur_c, prev_c, ts, pts
```
...
Failed and odd login patterns from rotated service accounts, monitoring tools with stale credentials, M&A user migrations, and scheduled batch jobs with expired secrets.
