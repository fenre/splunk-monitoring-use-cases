<!-- AUTO-GENERATED from UC-10.7.637.json — DO NOT EDIT -->
...
# UC-10.7.637 · Windows SqlWriter SQLDumper DLL Sideload

> **Criticality:** High &middot; **Difficulty:** Intermediate &middot; **Pillar:** Security &middot; **Type:** Security

*We add these signals to the risk story for "Windows SqlWriter SQLDumper DLL Sideload" so the team can spot abuse early without getting lost in one-off alerts that lack context.*
...
The analytic is designed to be run against Sysmon event logs collected from endpoints. The analytic requires the Sysmon event logs to be ingested into Splunk. The analytic searches for EventCode 7 where the Image is either SQLDumper.exe or SQLWriter.exe and the ImageLoaded is vcruntime140.dll. The search also filters out the legitimate loading of vcruntime140.dll from the System32 directory to red…
