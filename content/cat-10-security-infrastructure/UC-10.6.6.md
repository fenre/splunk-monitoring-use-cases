<!-- AUTO-GENERATED from UC-10.6.6.json — DO NOT EDIT -->
...
*We check which open issues have sat past the fix times we promised so audits and service owners see gaps before they become incidents.*
...
- `stats` rolls up events into metrics; results are split **by severity, sla_status** so each row reflects one combination of those dimensions.

### Step 3 — Validate
Compare age and overdue counts in Splunk with Tenable.io or Qualys VMDR dashboards and with your published SLA and exception register. A finding can be "overdue" in math but still in policy if an approved exception exists.
...
Vulnerabilities with age above SLA on assets with documented exception, vendor patches not yet released, or planned remediation in the next window. Exception expirations from approved business decisions, vendor delays, or migrations not yet completed.
