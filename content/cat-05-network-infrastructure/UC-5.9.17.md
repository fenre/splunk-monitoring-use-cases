...
*We follow the chain from root to your name, so a missing or stale delegation is obvious before email and web all fail together.*
...
- `stats` rolls up events into metrics; results are split **by dns.question.name, thousandeyes.source.agent.name** so each row reflects one combination of those dimensions.
...
Delegation chain warnings can follow registrar or parent-zone glue updates that are still propagating.
