...
*We look at how much noise each severity is creating, so we are not always chasing “critical” that is really informational.*
...
- `stats` rolls up events into metrics; results are split **by severity, alert.rule.name, alert.test.name, alert.type** so each row reflects one combination of those dimensions.
...
Severity counts jump when you widen alert rules, onboard new tests, or during noisy maintenance windows.
