...
*We feed the same test results into service health, so the network and app teams share one view of what “green” means.*
...
- `stats` rolls up events into metrics; results are split **by service_name, kpiid, itsi_kpi_id** so each row reflects one combination of those dimensions.
...
ITSI health moves with KPI baselines, entity import noise, and service overlap; align entities before judging scores.
