...
*We look for broken DNS security chains, so a key rollover or a bad signature does not look like a mystery outage.*
...
- `stats` rolls up events into metrics; results are split **by dns.question.name** so each row reflects one combination of those dimensions.
...
DNSSEC failures may appear when keys roll, parental DS records lag, or a validator along the path is misconfigured.
