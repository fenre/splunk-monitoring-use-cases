...
*We line up resolvers side by side, so we can tell if one public DNS path is off while another still works fine.*
...
Resolver comparisons jump when a provider throttles, moves anycast, or when tests hit different public resolvers.
