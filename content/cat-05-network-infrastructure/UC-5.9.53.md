...
*We line up what the network test saw with what the app traces show, so we are not stuck arguing which team owns the fire.*
...
APM join mismatches misfire on hostname vs service name; bad joins can label “app” when only correlation failed.
