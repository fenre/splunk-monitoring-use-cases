...
*We time API responses from the same perspective users get, so backend delay versus network delay is easier to untangle.*
...
API response time lags from gateway queues, token refresh storms, and dependency timeouts masked as network delay.
