...
*We test whether our important names still resolve, so a DNS blip in front of an app is not silent until everything falls over.*

---

## Description

DNS failures cascade into application outages — if users cannot resolve names, nothing works. ThousandEyes DNS tests monitor availability from multiple global vantage points.
...
- `stats` rolls up events into metrics; results are split **by dns.question.name, server.address** so each row reflects one combination of those dimensions.
...
Line chart (availability % over time), Single value (current availability), Table (question, server, availability).

## Known False Positives

DNS can look “down” from one vantage when resolvers or auth servers are in rolling maintenance or slow to respond.
