...
*We check that simple web tests still get a response, so the first line of the user journey is not failing quietly.*
...
HTTP availability can be green while TLS or auth fails later in the user journey; use companion transaction tests.
