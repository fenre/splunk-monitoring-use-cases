...
*We check how our test agents talk to each other, so we see whether office-to-office or data-center links are really healthy.*
...
- `stats` rolls up events into metrics; results are split **by thousandeyes.source.agent.name, thousandeyes.target.agent.name, network.io.direction** so each row reflects one combination of those dimensions.
...
Agent-to-agent results swing when one side’s network, host, or agent software is in maintenance; compare both legs.
