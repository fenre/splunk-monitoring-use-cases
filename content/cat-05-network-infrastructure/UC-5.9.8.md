...
*We see whether our critical prefixes are reachable from the outside world, so we catch routing blips that could strand users.*
...
- `stats` rolls up events into metrics; results are split **by thousandeyes.monitor.name, network.prefix** so each row reflects one combination of those dimensions.
...
BGP reachability dips during legitimate prefix withdrawals, planned maintenance, or monitor placement limits.
