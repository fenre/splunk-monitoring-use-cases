...
*We list which bad domains your Infoblox DNS policy blocked and from which clients, so security can see what was stopped and tune what is noisy.*
...
DNS query spikes from authorized monitoring tools, IoT devices with chatty firmware, or backup resolvers can dominate top-blocked lists. Review policy scope before re-tuning.
