...
*We look at the AS path our prefixes ride, so we notice when traffic takes a longer or odder detour than usual.*
...
- `stats` rolls up events into metrics; results are split **by network.prefix, thousandeyes.monitor.name** so each row reflects one combination of those dimensions.
...
AS path length and ordering change during upstream redesigns, prepends, and regional traffic engineering.
