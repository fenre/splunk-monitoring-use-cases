...
*We see how home and café networks hold up, so we know if people are suffering last-mile pain instead of the data center.*
...
- `stats` rolls up events into metrics; results are split **by thousandeyes.source.agent.name, target.type** so each row reflects one combination of those dimensions.
...
Remote worker health swings with last-mile ISP and Wi‑Fi; coffee-shop tests are not a fair HQ baseline.
