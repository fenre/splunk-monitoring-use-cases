...
*We see whether API checks finish, so a silent partial failure in the chain does not live only in developer logs.*
...
API completion drops when rate limits, auth token skew, or partner maintenance hit only some regions.
