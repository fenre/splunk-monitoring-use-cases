...
- Ensure the following data sources are available: `sourcetype=meraki (type=flows OR type=firewall) action="deny"`.
...
Documented **Data sources**: `sourcetype=meraki (type=flows OR type=firewall) action="deny"`. **App/TA** (typical add-on context): `Cisco Meraki Add-on for Splunk` (Splunkbase 5580). The SPL below should target the same indexes and sourcetypes you configured for that feed—rename `index=` / `sourcetype=` if your deployment differs.
