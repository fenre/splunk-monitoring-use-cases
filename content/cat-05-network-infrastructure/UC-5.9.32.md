...
*We watch the CDN edge, so when the cache or an edge pop misbehaves we see it before every region is on fire.*
...
- `stats` rolls up events into metrics; results are split **by thousandeyes.test.name, thousandeyes.source.agent.name, server.address** so each row reflects one combination of those dimensions.
...
CDN performance shifts with cache fill, A/B page rollouts, and which edge a test node hits in a big region.
