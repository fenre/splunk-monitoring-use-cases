...
*We watch availability in more than one region, so a partial SaaS or DNS answer does not look green everywhere by accident.*
...
- `stats` rolls up events into metrics; results are split **by thousandeyes.test.name, thousandeyes.source.agent.geo.country.iso_code, thousandeyes.source.agent.location** so each row reflects one combination of those dimensions.
...
Region availability can track tests against one DNS answer while users fail over to another healthy region.
