...
---
id: "5.9.54"
title: "MTTR Reduction via Network vs Application Isolation"
criticality: "high"
...
> **Criticality:** High &middot; **Difficulty:** Advanced &middot; **Pillar:** Observability &middot; **Type:** Anomaly

*We see whether having this picture in one place really shortens time to find the root, so the investment in monitoring pays off.*

---

## Description

Quantifies the business value of ThousandEyes + Splunk integration by measuring how quickly teams can isolate whether a performance issue is network-caused or application-caused. Tracks Mean Time to Resolution and Mean Time to Isolate metrics for incidents where ThousandEyes data was available.
...
- `stats` rolls up events into metrics; results are split **by alert.rule.name, alert.test.name** so each row reflects one combination of those dimensions.
...
- `stats` rolls up events into metrics; results are split **by isolation_method** so each row reflects one combination of those dimensions.
...
Single value (avg MTTR with ThousandEyes), Comparison chart (MTTR with vs. without TE data), Table (incidents and isolation times), Trend line (MTTR improvement over time).

## Known False Positives

MTTR and isolation metrics depend on ITSM data quality; partial tickets can exaggerate how fast teams isolated cause.
