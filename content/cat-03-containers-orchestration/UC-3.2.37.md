...
```spl
| tstats summariesonly=true latest(Application_State.state) AS app_state count AS app_events FROM datamodel=Application_State WHERE nodename=Application_State earliest=-3h@h latest=@h BY Application_State.dest Application_State.app
| rename Application_State.dest AS correl_host
| join type=left max=0 correl_host [
| tstats summariesonly=true avg(Performance.cpu_load_percent) AS avg_cpu_load avg(Performance.mem_used_percent) AS avg_mem_used FROM datamodel=Performance WHERE nodename=Performance.CPU OR nodename=Performance.Memory earliest=-3h@h latest=@h BY Performance.host
