...
• `stats` rolls up events into metrics; results are split **by _time, namespace** so each row reflects one combination of those dimensions.
• `eventstats` rolls up events into metrics; results are split **by namespace** so each row reflects one combination of those dimensions.
