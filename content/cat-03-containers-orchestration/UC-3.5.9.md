...
istiod and validating webhook outages block config pushes and sidecar updates; early detection prevents a widening blast radius across namespaces.
...
• `stats` rolls up events into metrics; results are split **by pod, namespace** so each row reflects one combination of those dimensions.
...
Single value (istiod ready replicas), Timeline (pod restarts), Table (error snippets by pod).
