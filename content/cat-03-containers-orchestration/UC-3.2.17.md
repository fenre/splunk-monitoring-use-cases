...
• `stats` rolls up events into metrics; results are split **by metric_name, namespace, horizontalpodautoscaler** so each row reflects one combination of those dimensions.
...
• `stats` rolls up events into metrics; results are split **by namespace, horizontalpodautoscaler** so each row reflects one combination of those dimensions.
