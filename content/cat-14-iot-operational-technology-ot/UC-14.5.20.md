<!-- AUTO-GENERATED from UC-14.5.20.json — DO NOT EDIT -->
...
*We log extension and plugin errors on the broker so third-party add-ons do not break messaging quietly.*
...
- Extracts fields with `rex` (regular expression).
- `stats` rolls up events into metrics; results are split **by host, extension_id, ex_type** so each row reflects one combination of those dimensions.
...
Extension errors during plugin upgrades, vendor hotfixes, or permission changes on the broker host.
