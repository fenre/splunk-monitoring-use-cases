<!-- AUTO-GENERATED from UC-14.6.3.json — DO NOT EDIT -->
...
> **Criticality:** High &middot; **Difficulty:** Advanced &middot; **Pillar:** Security &middot; **Type:** Fault, Security, Availability

*We use deep packet inspection to see run-state and mode changes on our controllers so we can separate routine stops from something odd.*
...
- `stats` rolls up events into metrics; results are split **by source_h destination_h rosctr_name subfunction_name function_name** so each row reflects one combination of those dimensions.
...
CPU and run-state changes from routine mode switches, restarts, or process upsets with documented causes.
