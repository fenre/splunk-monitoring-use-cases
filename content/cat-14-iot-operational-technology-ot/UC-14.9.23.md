<!-- AUTO-GENERATED from UC-14.9.23.json — DO NOT EDIT -->
...
*We track how many critical and high events we get over time so our OT security posture and tuning show up in one trend.*
...
- `stats` rolls up events into metrics; results are split **by severity_label, type_id, _time** so each row reflects one combination of those dimensions.
...
Severity mix shifts when alert policies are re-tuned, new rule packs deploy, or sites onboard gradually.
