<!-- AUTO-GENERATED from UC-14.5.7.json — DO NOT EDIT -->
...
*We watch alarm and event queues on the industrial server so a backlog does not mean we miss a safety or process alert.*
...
- `stats` rolls up events into metrics; results are split **by host, endpoint** so each row reflects one combination of those dimensions.
...
Alarm and event queue depth spikes during bursty process alarms, batch downloads, or subscription tuning.
