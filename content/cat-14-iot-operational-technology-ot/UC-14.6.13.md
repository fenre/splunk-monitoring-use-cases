<!-- AUTO-GENERATED from UC-14.6.13.json — DO NOT EDIT -->
...
> **Criticality:** High &middot; **Difficulty:** Advanced &middot; **Pillar:** Security &middot; **Type:** Performance, Fault, Security

*We use deep packet inspection to watch implicit I/O messaging on EtherNet/IP so changes to cyclic traffic break from baseline.*
...
- `stats` rolls up events into metrics; results are split **by _time connection_id source_h destination_h** so each row reflects one combination of those dimensions.
- `eventstats` rolls up events into metrics; results are split **by connection_id** so each row reflects one combination of those dimensions.
...
Implicit I/O anomalies during controller program edits, I/O reseat, or slot replacements.
