<!-- AUTO-GENERATED from UC-14.5.16.json — DO NOT EDIT -->
...
*We watch cluster health on the managed broker so split brain or a sick node does not cost us availability.*
...
- Extracts fields with `rex` (regular expression).
- `stats` rolls up events into metrics; results are split **by host, node_id, cluster_event** so each row reflects one combination of those dimensions.
...
Cluster and quorum warnings during rolling upgrades, network partitions, or capacity adds.
