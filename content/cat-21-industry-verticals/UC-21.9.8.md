<!-- AUTO-GENERATED from UC-21.9.8.json — DO NOT EDIT -->
...
> **Criticality:** High &middot; **Difficulty:** Advanced &middot; **Pillar:** Observability &middot; **Type:** Fault, Performance

*We watch lift-station pumps and sensors. We help you catch motor or bearing issues before a wet-weather release.*
...
Service and risk leaders use signals from Lift Station Failure Prediction to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `stats` rolls up events into metrics; results are split **by station_id, pump_id, _time** so each row reflects one combination of those dimensions.
- `eventstats` rolls up events into metrics; results are split **by pump_id** so each row reflects one combination of those dimensions.
...
Lift-station feature noise in wet-weather inflow, VFD or impeller work, or probe maintenance that the pretreatment team annotated on the work order.
