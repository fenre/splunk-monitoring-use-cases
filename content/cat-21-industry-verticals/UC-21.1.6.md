<!-- AUTO-GENERATED from UC-21.1.6.json — DO NOT EDIT -->
...
*We watch load balance across distribution feeders. We help you catch phase or loading issues before overloads or complaints spread.*
...
Service and risk leaders use signals from Distribution Feeder Load Imbalance to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `stats` rolls up events into metrics; results are split **by feeder_id, _time** so each row reflects one combination of those dimensions.
...
Imbalance blips while switching feeds, reclosing after faults, rebalancing phases, or running planned feeder maintenance the DMS operator signed.
