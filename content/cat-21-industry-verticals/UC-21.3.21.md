<!-- AUTO-GENERATED from UC-21.3.21.json — DO NOT EDIT -->
...
*We watch DIPS database performance. We help you catch contention or load before chart access times out across wards.*
...
Service and risk leaders use signals from DIPS Arena Database Performance to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `stats` rolls up events into metrics; results are split **by db_instance, _time** so each row reflects one combination of those dimensions.
...
Database noise during index maintenance, stats refresh, storage migration, or batch report windows that the DBA and Arena admin logged as expected load.
