<!-- AUTO-GENERATED from UC-21.2.12.json — DO NOT EDIT -->
...
*We watch conveyor speed and jam signals. We help you catch mechanical trouble before the line backs up.*
...
Service and risk leaders use signals from Conveyor Belt Speed and Jam Detection to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `stats` rolls up events into metrics; results are split **by line_id, belt_id, _time** so each row reflects one combination of those dimensions.
...
Jam or speed variance on wet belts, new SKUs, or after mechanical adjustment during the maintenance window the line documented.
