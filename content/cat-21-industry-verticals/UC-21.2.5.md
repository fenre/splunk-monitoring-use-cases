<!-- AUTO-GENERATED from UC-21.2.5.json — DO NOT EDIT -->
...
> **Criticality:** Medium &middot; **Difficulty:** Advanced &middot; **Pillar:** Observability &middot; **Type:** Fault, Performance

*We watch vibration against healthy baselines. We help you catch bearing or alignment issues before the machine fails.*
...
Service and risk leaders use signals from Predictive Maintenance Vibration Baseline Drift to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `stats` rolls up events into metrics; results are split **by asset_id, _time** so each row reflects one combination of those dimensions.
- Orders rows with `sort` — combine with `head`/`tail` for top-N patterns.
- `streamstats` rolls up events into metrics; results are split **by asset_id** so each row reflects one combination of those dimensions.
...
Vibration drift after bearing lubrication, belt tension service, or speed setpoint changes during a run that the maintenance planner tied to a work order.
