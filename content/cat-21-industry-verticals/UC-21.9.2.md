<!-- AUTO-GENERATED from UC-21.9.2.json — DO NOT EDIT -->
...
*We watch pump run hours and power per flow. We help you catch efficiency loss before electric bills and failures rise.*
...
Service and risk leaders use signals from Pump Station Run Time and Efficiency Trending to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `eventstats` rolls up events into metrics; results are split **by station_id, pump_id** so each row reflects one combination of those dimensions.
...
Efficiency or runtime noise on wet-weather inflows, VFD re-tunes, or pump swaps where the run-time ratio still meets the weekly ops review.
