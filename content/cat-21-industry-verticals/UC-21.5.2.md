<!-- AUTO-GENERATED from UC-21.5.2.json — DO NOT EDIT -->
...
*We watch wellhead telemetry for gaps. We help you catch dead sensors before production reports go wrong.*
...
Service and risk leaders use signals from Wellhead Telemetry Data Gap Monitoring to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `stats` rolls up events into metrics; results are split **by well_id** so each row reflects one combination of those dimensions.
...
Gap clusters after solar-power swaps on radios, plunger-lift transients, or comms work on the lease road that the pumper and SCADA support annotated.
