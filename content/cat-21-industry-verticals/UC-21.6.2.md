<!-- AUTO-GENERATED from UC-21.6.2.json — DO NOT EDIT -->
...
*We watch self-checkout lanes and error rates. We help you catch broken lanes or high interventions before the front end chokes.*
...
Service and risk leaders use signals from Self-Checkout Lane Availability and Error Rate to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `stats` rolls up events into metrics; results are split **by store_id, lane_id, _time** so each row reflects one combination of those dimensions.
...
SCOs closed for firmware, coin-tower service, endcap remodels, or high attendant interventions during restricted sales the front-end lead expected.
