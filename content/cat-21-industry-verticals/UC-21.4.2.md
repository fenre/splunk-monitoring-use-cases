<!-- AUTO-GENERATED from UC-21.4.2.json — DO NOT EDIT -->
...
*We watch driving scores and harsh events. We help you coach drivers before risk scores and insurance costs jump.*
...
Service and risk leaders use signals from Driver Behavior Scoring to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `stats` rolls up events into metrics; results are split **by driver_id** so each row reflects one combination of those dimensions.
...
Score swings after training rides, new hire onboarding, or route changes that the safety coach uses as coaching moments rather than policy breaches.
