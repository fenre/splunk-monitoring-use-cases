<!-- AUTO-GENERATED from UC-21.2.11.json — DO NOT EDIT -->
...
*We watch robot cycle times against normal. We help you catch wear or program drift before throughput drops.*
...
Service and risk leaders use signals from Robotic Cell Cycle Time Deviation to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `stats` rolls up events into metrics; results are split **by cell_id, program_id, _time** so each row reflects one combination of those dimensions.
...
Cycle noise during teach-in, light-curtain breaks, or dual-tool changeovers that the robot log shows as normal protective stops, not a fault.
