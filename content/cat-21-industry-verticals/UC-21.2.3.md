<!-- AUTO-GENERATED from UC-21.2.3.json — DO NOT EDIT -->
...
> **Criticality:** High &middot; **Difficulty:** Intermediate &middot; **Pillar:** Observability &middot; **Type:** Performance, Compliance

*We watch batch yields against targets. We help you catch drift before scrap and rework pile up.*
...
Service and risk leaders use signals from Production Batch Yield Tracking to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `stats` rolls up events into metrics; results are split **by sku, batch_id, _time** so each row reflects one combination of those dimensions.
- `eventstats` rolls up events into metrics; results are split **by sku** so each row reflects one combination of those dimensions.
...
Yield step-changes on trial lots, new ingredients, or startup scrap after a clean-out that the quality release still accepted under the deviated batch record.
