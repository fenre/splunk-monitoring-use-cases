<!-- AUTO-GENERATED from UC-21.5.1.json — DO NOT EDIT -->
...
*We watch pipeline pressure and flow. We help you catch leaks, blocks, and odd trends before a release or environmental hit.*
...
Service and risk leaders use signals from Pipeline Pressure and Flow Rate Anomaly Detection to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `eventstats` rolls up events into metrics; results are split **by segment_id** so each row reflects one combination of those dimensions.
...
- `stats` rolls up events into metrics; results are split **by segment_id** so each row reflects one combination of those dimensions.
...
Pressure/flow noise during pigging, batch pig tracking, or SCADA time sync drift that the pipeline control center compared with the field ticket.
