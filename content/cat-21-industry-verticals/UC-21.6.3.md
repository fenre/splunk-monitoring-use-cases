<!-- AUTO-GENERATED from UC-21.6.3.json — DO NOT EDIT -->
...
> **Criticality:** High &middot; **Difficulty:** Beginner &middot; **Pillar:** Observability &middot; **Type:** Availability, Performance

*We watch in-store Wi-Fi and branch network health. We help you catch pain before staff devices and hand scanners fail.*
...
Service and risk leaders use signals from In-Store Wi-Fi and Network Infrastructure Health to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `stats` rolls up events into metrics; results are split **by store_id, ap_name, _time** so each row reflects one combination of those dimensions.
...
Wi-Fi noise during controller upgrades, high guest load, or regional VLAN work that the store network change window names on the NOC calendar.
