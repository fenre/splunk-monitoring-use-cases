<!-- AUTO-GENERATED from UC-21.2.15.json — DO NOT EDIT -->
...
> **Criticality:** Medium &middot; **Difficulty:** Intermediate &middot; **Pillar:** Observability &middot; **Type:** Performance, Change

*We watch what each shift hands to the next. We help you catch holes before safety or quality handoffs fail.*
...
Service and risk leaders use signals from Production Shift Handover Report Generation to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `stats` rolls up events into metrics; results are split **by line_id, shift, _time** so each row reflects one combination of those dimensions.
...
Handover metric gaps on holiday calendars, data-entry lag from the floor, or template changes the operations manager rolled out for the new KPI review.
