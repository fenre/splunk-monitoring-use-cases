<!-- AUTO-GENERATED from UC-21.8.8.json — DO NOT EDIT -->
...
> **Criticality:** High &middot; **Difficulty:** Advanced &middot; **Pillar:** Observability &middot; **Type:** Performance, Fault

*We watch 5G cell throughput and drop metrics. We help you catch a bad site before the neighborhood notices.*
...
Service and risk leaders use signals from 5G NR gNodeB Performance Monitoring to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
**5G NR gNodeB native OS monitoring toolsing** — gNodeB throughput, latency, and drop metrics expose RF and transport issues before subscriber experience scores fall; focuses on RAN KPIs rather than core signaling traces (Cat 5.11).
...
- `stats` rolls up events into metrics; results are split **by gnb_id, cell_id, _time** so each row reflects one combination of those dimensions.
- `eventstats` rolls up events into metrics; results are split **by cell_id** so each row reflects one combination of those dimensions.
...
- Pipeline stage (see **5G NR gNodeB native OS monitoring toolsing**): table _time, gnb_id, cell_id, avg_dl, avg_ul, avg_drop, avg_lat, thr_ratio
...
gNodeB noise during beam sweep trials, C-RAN re-homing, MIMO parameter tests, or power-saving features that the RAN optimization squad scheduled.
