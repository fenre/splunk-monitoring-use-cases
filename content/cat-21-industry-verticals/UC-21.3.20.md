<!-- AUTO-GENERATED from UC-21.3.20.json — DO NOT EDIT -->
...
*We watch hospital sign-in and single sign-on health. We help you catch identity problems before clinicians are locked out at the worst moment.*
...
Service and risk leaders use signals from DIPS Arena User Authentication and SSO Monitoring to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `stats` rolls up events into metrics; results are split **by auth_flow, _time** so each row reflects one combination of those dimensions.
...
### Step 3 — Validate
Compare failure rates, latency, and success counts with the identity service or federation console and Microsoft Entra sign-in audit for the same window. Spot-check a few indexed events against the DIPS (or EHR) vendor audit export so field names and flows line up with how clinicians actually sign in.
...
Auth spikes during password synchronizations, phone reprovisioning, Entra or federation maintenance, or SMART app updates that the identity team announced.
