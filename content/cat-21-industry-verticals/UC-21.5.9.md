<!-- AUTO-GENERATED from UC-21.5.9.json — DO NOT EDIT -->
...
*We watch effluent and permit limits. We help you catch excursions before environmental penalties or public concern.*
...
Service and risk leaders use signals from Environmental Compliance Effluent Monitoring to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `stats` rolls up events into metrics; results are split **by outfall_id, parameter** so each row reflects one combination of those dimensions.
...
Effluent blips on composite vs grab sampling differences, diurnal plant load, or lab hold times during chain-of-custody handoffs the permit still covers.
