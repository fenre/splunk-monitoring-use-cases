<!-- AUTO-GENERATED from UC-21.6.8.json — DO NOT EDIT -->
...
*We watch store HVAC and energy use. We help you catch waste before comfort and cost targets slip.*
...
Service and risk leaders use signals from Store HVAC and Energy Consumption Optimization to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `eventstats` rolls up events into metrics; results are split **by store_id, wday, hour** so each row reflects one combination of those dimensions.
...
HVAC and kWh noise during heat waves, setpoint or schedule changes, after-hours cleaning, or equipment commissioning the facilities runbook includes.
