<!-- AUTO-GENERATED from UC-21.3.9.json — DO NOT EDIT -->
...
*We watch trial data paths for odd edits or gaps. We help you catch integrity issues before a study is questioned.*
...
Service and risk leaders use signals from Clinical Trial Data Integrity Monitoring to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `stats` rolls up events into metrics; results are split **by study_id, site_id** so each row reflects one combination of those dimensions.
...
Integrity flags on blinded data loads, redaction for export, query replay after an EDC vendor patch, or monitor-driven protocol amendments IRB already approved.
