<!-- AUTO-GENERATED from UC-21.3.25.json — DO NOT EDIT -->
...
*We watch how long clinical documents take to render. We help you catch slowness before handoffs wait on the chart.*
...
Service and risk leaders use signals from DIPS Arena Clinical Document Generation Latency to move crews, budget, and customer or regulator communications while the situation is still controllable, so the organization steers from live data rather than a post-incident report.
...
- `stats` rolls up events into metrics; results are split **by document_type, _time** so each row reflects one combination of those dimensions.
...
Document latency during template merges, CDA assembly upgrades, e-sign or rendering library updates, or mass fax-to-chart replays the document team requeued.
