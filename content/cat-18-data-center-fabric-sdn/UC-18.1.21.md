...
• `stats` rolls up events into metrics; results are split **by _time, tenant, epg, action** so each row reflects one combination of those dimensions.
• `stats` rolls up events into metrics; results are split **by tenant, epg** so each row reflects one combination of those dimensions.
...
Step 3 — Validate
Confirm with the APIC GUI under Operations (endpoint and policy views); cross-check EPG, BD, and leaf context for the same MAC or IP and time range.
