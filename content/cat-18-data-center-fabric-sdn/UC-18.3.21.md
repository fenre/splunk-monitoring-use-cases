...
• `stats` rolls up events into metrics; results are split **by leaf, esi, event** so each row reflects one combination of those dimensions.
...
Step 3 — Validate
Compare with ACI/NSX tooltips for the same TEP, segment, and flood domain during the same interval.
