...
• `stats` rolls up events into metrics; results are split **by action, template_name, site_name, status, user** so each row reflects one combination of those dimensions.
...
Step 3 — Validate
Compare with Cisco NDO (deployment status, template, and per-site diffs) for the same template and site.
