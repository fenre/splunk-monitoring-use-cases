...
---
id: "13.5.7"
title: "Splunk APM Service Map Health (RED Metrics)"
...
• `stats` rolls up events into metrics; results are split **by _time, service_name, environment** so each row reflects one combination of those dimensions.
• `eventstats` rolls up events into metrics; results are split **by service_name** so each row reflects one combination of those dimensions.
...
Table (service health matrix — green/yellow/red by RED metric), Line chart (RED metrics per service over 24 hours), Gauge (error rate per critical service), Heatmap (service × time error rate).
