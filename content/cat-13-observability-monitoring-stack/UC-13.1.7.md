...
KV Store failures break lookups, app functionality, and ES correlation. Health monitoring prevents cascading application issues.
...
• `stats` rolls up events into metrics; results are split **by host, log_level, message** so each row reflects one combination of those dimensions.
...
Status grid (SHC member × KV Store health), Table (KV Store errors), Line chart (replication lag).
