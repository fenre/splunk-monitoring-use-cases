...
• `stats` rolls up events into metrics; results are split **by detector_name, detector_id, creator** so each row reflects one combination of those dimensions.
...
Step 3 — Validate
Match the time range and labels with the vendor APM or monitoring UI and walk one full example in their drill-down.
