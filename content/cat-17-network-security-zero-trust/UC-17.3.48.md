...
- `stats` rolls up events into metrics; results are split **by dlp_rule_name, data_type, src_user_name, action, dst** so each row reflects one combination of those dimensions.
- Orders rows with `sort` — combine with `head`/`tail` for top-N patterns.

### Step 3 — Validate
Compare with Check Point SmartConsole or Infinity portal for the same rules, hits, and time window.
