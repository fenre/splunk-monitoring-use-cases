<!-- AUTO-GENERATED from UC-12.5.3.json — DO NOT EDIT -->
...
*We watch Flux reconciliation so clusters do not drift silently from the desired state in Git.*

---

## Description

Unhealthy Flux `Kustomization`/`HelmRelease` resources stop automated delivery; monitoring reconciliation ensures continuous GitOps and catches controller or source errors early.
...
- `stats` rolls up events into metrics; results are split **by namespace, name, kind, message** so each row reflects one combination of those dimensions.
...
Table (failed resources), Single value (failed reconciliations), Timeline (controller errors), Bar chart (failures by kind).

## Known False Positives

Short-lived sync issues during cluster upgrades, credential rotations, or Git provider incidents.
