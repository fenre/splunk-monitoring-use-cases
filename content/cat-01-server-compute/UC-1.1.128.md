<!-- AUTO-GENERATED from UC-1.1.128.json — DO NOT EDIT -->
...
> **Criticality:** Critical &middot; **Difficulty:** Intermediate &middot; **Pillar:** Observability &middot; **Type:** Capacity

*We watch inode use, not just gigabytes free, so the server cannot run out of file slots when space still looks okay.*
...
- `stats` rolls up events into metrics; results are split **by host, MountedOn** so each row reflects one combination of those dimensions.
...
Tiny `tmpfs` or `squashfs` images can have low inode totals that sit high in percent without production risk. Field names vary (`IUsePct` vs `IUse%`)—align extractions. Snapshot-heavy backup volumes can have inode pressure that is still in a change window.
