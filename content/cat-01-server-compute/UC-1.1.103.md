<!-- AUTO-GENERATED from UC-1.1.103.json — DO NOT EDIT -->
...
*We pull IPMI sensor health from the management controller so we see overheating or bad power before the operating system panics.*
...
- `stats` rolls up events into metrics; results are split **by host, sensor_name** so each row reflects one combination of those dimensions.
...
Transient fan or voltage warnings during chassis hot-swap or PDU work can clear in minutes. Some vendors map “non-critical” strings that still match `WARNING` in your parser—align enum values with `ipmitool` output for your BMC firmware.
