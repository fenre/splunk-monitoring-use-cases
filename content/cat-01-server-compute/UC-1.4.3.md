...
*We look for out-of-band messages that say a power feed or supply is missing or bad, so you can fix it while the box still has another power path to lean on.*
...
Vendors use different phrasing (for example `PS Redundancy Lost` vs. `Power Supply Failed`). Redundant A/B maintenance and hot-swap can generate duplicate SEL rows. Validate against the out-of-band console or a physical site check before a full incident bridge.
