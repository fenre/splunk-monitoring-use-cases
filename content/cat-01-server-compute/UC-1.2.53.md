...
*We help you know when full-disk protection on the server is in recovery or trouble, so someone can re-verify keys before the volume is wide open with no secrets.*
...
Re-image, re-key, and TPM/bios update flows generate recovery and suspend entries. A recovery key *use* in the field is the hot signal for stolen device workflows.
