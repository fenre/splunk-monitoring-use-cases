<!-- AUTO-GENERATED from UC-1.1.98.json — DO NOT EDIT -->
...
*We watch TLB shootdowns so we know when CPUs are invalidating each other’s memory mappings too often—a hidden cause of slowdowns on big servers.*
...
Kernel compile-time symbols and CPU counts change how `tlb_shootdown_rate` is derived; compare hosts with similar core counts. Large-page or VM balloon operations can legitimately raise shootdowns during narrow windows. Replace the literal `threshold` token in SPL with a numeric macro before production use.
