...
*Thin pool exhaustion causes I/O errors on all logical volumes in the pool, causing application failures — so you find out before users do when something is slowing down or breaking.*
...
Snapshots, clones, and VM images swing thin-pool use. A jump can be expected provisioning, not a leak.
