...
*We watch the hidden pipes programs use to talk to each other on Windows so we can notice odd pipe activity that often rides along with sneaky tools moving between machines.*
...
Defender, SQL, backup, and remote management tools. Baseline by `Image` and pipe name allowlists; expect noise on app and RDS session hosts.
