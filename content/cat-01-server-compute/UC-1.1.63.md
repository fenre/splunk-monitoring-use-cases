...
*We help you know when a network card is quietly throwing away pieces of traffic, which is often the first sign of a busy or mis-sized connection between the card and the rest of the network.*
...
A single non-zero value from a one-time micro-burst; counters that only increase on reboot; bond/team parents vs slave stats double-counting if you do not pick the right **interface** names—tighten to `| where total_dropped>threshold` and compare delta over time, not the absolute if counters never reset.
