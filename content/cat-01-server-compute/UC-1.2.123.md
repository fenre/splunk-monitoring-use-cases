...
*We look for the extra rights and hand-offs that turn a normal session into a much more powerful one, so a small foothold is harder to grow into a full take-over in one step.*
...
Backup and defrag with SeManageVolumePrivilege; vendors with documented needs. Enrich with process tree from same `_time` window.
