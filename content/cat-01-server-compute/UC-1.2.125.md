...
*We look at the shared disk space that many virtual machines use at once, so a storage hiccup on that shared volume is seen before a whole cluster of services falls over together.*
...
Storage maintenance, redirect-on-write, and node drain; chkdsk on CSV. Check cluster `Quorum` and `PhysicalDisk` resource state in parallel.
