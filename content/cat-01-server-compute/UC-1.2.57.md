...
*We look for a program on the server with an unusually high number of active threads, which can mean a bug, a runaway, or a capacity ceiling hit when you use the main Perfmon search as the SLO.*
...
IIS, Java, and some database engines run hot thread counts in normal steady state. Baseline the same `instance` over weeks before a static cap.
