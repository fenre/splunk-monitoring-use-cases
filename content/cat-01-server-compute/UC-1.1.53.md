...
*We catch when the system is starting to turn away work at the front door of busy network listeners, so you can fix sizing or load before people see long waits and errors.*
...
Kernels that do not publish the field, leaving a literal zero and hiding issues—validate field mapping; microbursts on restart that clear immediately; load tests in lower environments you should suppress by host prefix.
