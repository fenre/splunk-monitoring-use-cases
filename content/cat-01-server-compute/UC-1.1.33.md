...
*Inode exhaustion causes file creation failures even when disk space remains available, stopping applications — so you find out before users do when something is slowing down or breaking.*
...
Many small files can use most inodes while block space is still free. Set thresholds with `df -i` baselines and directory clean-up playbooks, not only block-usage SLOs.
