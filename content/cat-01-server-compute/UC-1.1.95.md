...
*We track how fast the system opens new TCP connections so we notice traffic spikes that might mean a misbehaving app or an early-stage flood.*
...
Batch jobs, OS updates pulling many packages, or a new service that legitimately opens many short-lived TCP connections can raise `TcpActiveOpens` without an attack. Cold start after reboot also shifts the baseline until `streamstats` has enough history.
