...
*We look for changes to the Windows boot settings and boot commands so we can see when something other than a planned fix or build job is about to change how the system starts or recovers.*
...
MECM, imaging, and vendor repair media running `bcdedit` during build; break-glass recovery. Parent image and host role allowlists; require ticket for servers.
