<!-- AUTO-GENERATED from UC-1.1.105.json — DO NOT EDIT -->
...
*We watch fan RPM from IPMI so we catch failed or lazy fans while the server still has room to cool down.*
...
- `stats` rolls up events into metrics; results are split **by host, fan_name** so each row reflects one combination of those dimensions.
...
PWM fans idling low in a cold datacenter row can sit under 20% by design; pair with temperature and redundant fan loss. IPMI may report “na” or zero when a sensor is absent—exclude those rows in parsing.
