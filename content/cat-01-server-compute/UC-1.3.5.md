...
> **Criticality:** Low &middot; **Difficulty:** Intermediate &middot; **Pillar:** Security &middot; **Type:** Fault

*We count which apps are crashing and how often, so you can see unstable software or a troubled computer before it ruins someone’s workday or loses their data.*
...
A new app version can produce a burst of identical crashes; treat top-level counts with the build number in view. A developer repeatedly stopping a process in Xcode can add noise. Filter out per-user `~/Library` paths for fleet dashboards if you only care about system-wide or shared apps.
