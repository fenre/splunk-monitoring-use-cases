...
*We watch whether the system’s special fast memory pools grow in an unusual way over time, so we can step in if the machine is quietly eating memory in the background.*
...
A one-off spike right after a major package upgrade, driver reload, or cache-heavy batch job that legitimately pin more slab pages; a logging change that alters or drops `slab_size` and makes the sum jump without real growth.
