...
*Interface errors (CRC, drops, overruns) indicate bad cables, failing NICs, or duplex mismatches — so you find out before users do when something is slowing down or breaking.*
...
Non-zero error counters can appear after a cable wiggle, driver reload, or known firmware bug. Use deltas over time and physical checks, not a single non-zero read.
