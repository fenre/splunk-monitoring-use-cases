<!-- AUTO-GENERATED from UC-1.1.117.json — DO NOT EDIT -->

---
id: "1.1.117"
title: "Configuration File Change Tracking (/etc)"
...
*We get alerts when key files under `/etc` change from Linux audit so surprise edits do not go unnoticed.*

---

## Description

Configuration file changes can indicate system compromise or unauthorized configuration modifications.
...
Puppet, Ansible, or `dnf` can rewrite `/etc` during approved changes—join to change records. The glob `path="/etc/*"` may not match every auditd `path` field format; use `path=/etc/...` with `match` in `where` if needed.
