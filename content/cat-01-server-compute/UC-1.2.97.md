...
*We watch the print system for the same kinds of driver and remote call patterns that keep showing up in big security problems, so print servers are not a quiet open door into the rest of the network.*
...
Mass driver push after print-nightmare style patching; MFP rollouts. Baseline on print servers; separate file servers without spooler from role holders.
