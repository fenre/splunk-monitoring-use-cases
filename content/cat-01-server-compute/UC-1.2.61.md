...
*We help you know when the space-savings process on a volume is in trouble, which can cost money on disk and time on rehydrate jobs before restore.*
...
Optimization after large data moves or a full volume can be slow and look like failure until completion. Distinguish ‘still running’ from ‘gave up’ with the vendor event text.
