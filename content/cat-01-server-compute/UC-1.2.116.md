...
*We look for new behind-the-scenes Windows management hooks that can run code without a normal login, because those are a quiet way to stay on a machine for a long time.*
...
SCCM inventory, monitoring agents, and DSC. Whitelist by namespace; alert on `ROOT\subscription` and permanent consumers outside IT templates.
