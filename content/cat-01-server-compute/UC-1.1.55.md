...
*We watch the machine’s name-lookup diary for a jump in “couldn’t look that up” lines, so teams can fix the phone book before apps start failing in big numbers.*
...
Intentional NXDOMAIN from security or sinkhole tools; build jobs that resolve thousands of not-yet-registered names; false `eval` of `count` in older copies of this use case—this version uses the `failures` field directly from `stats`.
