<!-- AUTO-GENERATED from UC-1.1.123.json — DO NOT EDIT -->
...
*We use Linux PSI metrics so cgroups that wait on CPU, memory, or disk show up before user-facing latency spikes.*
...
- `stats` rolls up events into metrics; results are split **by host, cgroup, resource** so each row reflects one combination of those dimensions.
...
Kernel <=5.0 may lack full PSI; some platforms expose only system-wide files. `some` vs `full` line semantics differ for I/O—match your doc before thresholding. Container churn can briefly spike one cgroup while siblings are fine.
