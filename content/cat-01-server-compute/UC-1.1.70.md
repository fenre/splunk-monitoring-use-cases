...
*We help you know when the master list of user names on a machine is edited, so only the right people and tools are the ones doing it.*
...
Configuration management tools that legitimately rewrite **passwd** every hour; duplicate **PATH** + **SYSCALL** pairs—`dedup` or key on **key=** only; container hosts where only the image builder should touch **passwd**—alert there with higher severity.
