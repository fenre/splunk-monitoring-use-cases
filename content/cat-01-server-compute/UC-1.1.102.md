<!-- AUTO-GENERATED from UC-1.1.102.json — DO NOT EDIT -->
...
*We scan kernel logs for memory hardware errors so we can replace bad DIMMs before random crashes or data corruption spread.*
...
| rex field=_raw max_match=1 "(?<error_type>correctable|uncorrectable|CE|UE)"
...
- `stats` rolls up events into metrics; results are split **by host, error_type** so each row reflects one combination of those dimensions.
...
Correctable ECC errors are often auto-scrubbed and may be acceptable up to a vendor threshold; tune for “uncorrectable” vs “correctable”. MCE strings from non-memory subsystems can match the same keywords—verify DIMM vs CPU cache in the full message.
