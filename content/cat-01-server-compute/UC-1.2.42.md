<!-- AUTO-GENERATED from UC-1.2.42.json — DO NOT EDIT -->
...
*We watch how much time the .NET part of a program spends in garbage collection, which can slow web and service apps on Windows even when the server CPU looks only moderately busy.*
...
High GC% often foreshadows OOM, restarts, and slow pages—an app-centric lens on a server metric you already collect.
...
```spl
| tstats `summariesonly` avg(Performance.mem_used) as m
...
Deploys, JIT, and big heap Gen2 collections can spike time-in-GC. Use release timing and the same w3wp/svc instance context before a dev escalation.
