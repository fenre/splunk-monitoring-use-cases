...
*We help you notice when someone signs in from more different places than they usually do in a day, which can be travel—or someone else using their name.*
...
Any corporate **NAT** that rotates exit IPs; **CI** users; **SRE** jump hosts that legitimately have dozens of home IPs a day; **zero**-history first day of deployment (no baseline) — the SPL guards with `isnotnull(baseline_ips)` but still think about warm-up time.
