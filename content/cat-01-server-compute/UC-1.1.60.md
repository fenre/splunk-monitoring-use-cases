...
*We help you know when the same computer’s network doors are not all the same size, which can make some big packets fit on one path but get chopped up or stuck on another.*
...
Intentional designs: jumbo on storage VLAN, 1500 on management; down interfaces still reporting an old MTU; containers or VRFs with deliberate differences—add a **lookup** of `expected_mismatch_ok` for those hosts.
