...
*We list machines still talking the oldest Windows file-sharing dialect so the team can turn it off on purpose instead of by surprise when a new flaw appears.*
...
Legacy storage and lab gear; one-off copy jobs during protocol migration. Inventory `ClientName` and retire per subnet; do not clear alerts until clients are upgraded or firewalled.
