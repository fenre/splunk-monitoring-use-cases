...
> **Criticality:** Medium &middot; **Difficulty:** Intermediate &middot; **Pillar:** Security &middot; **Type:** Fault

*We help you know when the same app is crashing over and over on a server, not just a one-time blip, so you can fix the root cause before users revolt.*
...
Browser and Office update waves throw WER noise. Count by the same `crash_app` + version in your main search before a mass pager.
