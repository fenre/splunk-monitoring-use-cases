...
> **Criticality:** High &middot; **Difficulty:** Advanced &middot; **Pillar:** Observability &middot; **Type:** Fault

*We look for files with no valid owner so odd leftovers from bad restores or intruders do not sit on the disk undetected.*
...
NFS, overlayfs, and unpacked tarballs can surface `nouser` files during migrations without an attack. Excluding `/proc` and ephemeral mounts in the `find` keeps noise down—mirror those excludes in the alert.
