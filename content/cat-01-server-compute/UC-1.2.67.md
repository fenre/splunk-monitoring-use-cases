...
*We watch for strange Kerberos login-ticket patterns on Windows that can mean someone forged a 'golden ticket' and could act as anyone in the directory, so the team can respond while damage is still limited.*
...
Legacy applications or domains that still negotiate RC4; lab or isolated forests with relaxed crypto policy; mixed-mode or migration windows. Baseline TGT volume per account and corroborate with ticket lifetime and source DC before calling incident.
