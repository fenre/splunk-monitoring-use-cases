...
*We track how much CPU time goes to interrupt handling so storms from networking or storage show up before average utilization looks bad.*
...
Packet captures, storage benchmarks, or DPDK can legitimately run CPUs hot with high softirq. `irq_stats` field definitions differ when sourced from `mpstat` vs parsed `/proc`—ensure percentages are 0–100. Replace any literal `threshold` tokens in derived searches with real macros.
