...
> **Criticality:** High &middot; **Difficulty:** Intermediate &middot; **Pillar:** Observability &middot; **Type:** Availability, Fault

*We read systemd’s restart counts and unit states so flapping services get fixed before they take down the whole host.*
...
`NRestarts` is cumulative in many versions—reset expectations after upgrades. Short bursts from health checks on aggressive `RestartSec` can exceed five without user impact. Scope the search to a lookup of `Unit` names that actually matter to SLAs.
