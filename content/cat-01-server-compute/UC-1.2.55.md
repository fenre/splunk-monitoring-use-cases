...
*We look for a small set of very powerful system rights being used in a way that does not look like a normal system tool, which can be part of a break-in path.*
...
Backup, storage, and defrag tools can trigger privilege-heavy access. A raw match on the privilege *string* is only the start of triage—add process and signer path.
