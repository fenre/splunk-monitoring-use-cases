...
*We track when setuid programs run from Linux audit data so only the binaries and users we expect can elevate privileges.*
...
Legitimate setuid programs (`sudo`, `passwd`, `ping`) are expected on some paths—maintain an allowlist of `exe` values. `threshold` in SPL is a token you must replace with a numeric or macro. Auditd field names (`suid` vs `auid`) vary by rule set—tune to your `auditd` version.
