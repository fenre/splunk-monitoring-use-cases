...
*We look at background file transfers that use the quiet download channel, so a big or odd transfer does not look like a normal patch job when it is not.*
...
SCCM/Intune, Windows Update, and Edge updates; VDI gold builds. Correlates parent `Image` and user account; velocity-only alerts.
