...
*We notice when a virtual machine keeps losing CPU time to other work on the same physical host — a sign the host is crowded or overbooked before users only see a slow app.*
...
Steal time bounces with short host contention, not only chronic overcommit. Correlate with hypervisor CPU headroom and neighbor VMs before rightsizing.
