...
*We see when the server is short on memory or swapping a lot, so you can add RAM or stop a runaway app before things freeze up.*
...
Memory and paging can spike during patch installs, large queries, and backup windows. Windows may commit aggressively under load without immediate user impact.
