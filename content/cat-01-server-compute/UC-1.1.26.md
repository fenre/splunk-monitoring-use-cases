<!-- AUTO-GENERATED from UC-1.1.26.json — DO NOT EDIT -->
...
*Frequency scaling changes indicate thermal throttling or power management adjustments affecting workload performance — so you find out before users do when something is slowing down or breaking.*
...
- `stats` rolls up events into metrics; results are split **by host, path** so each row reflects one combination of those dimensions.
...
Governor and P-state changes are normal on idle and power-capped systems. Escalate sustained misconfiguration or unexpected limits, not every transition.
