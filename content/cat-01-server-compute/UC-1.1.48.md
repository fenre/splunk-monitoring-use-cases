...
*We catch when one side of a big server is starved of memory while the other still has room, so you can fix placement before jobs run slow in hard-to-explain ways.*
...
Non-NUMA or single-socket machines mis-tagged in inventory; short-lived skew right after boot or a large one-node allocation that later balances; `min_free` at or near zero causing unstable ratios—consider a `min_free` floor in `eval`.
