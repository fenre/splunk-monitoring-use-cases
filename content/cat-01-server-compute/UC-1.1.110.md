...
*We list people who have not logged in for a long time so we can disable stale Linux accounts that attackers could try next.*
...
Break-glass or shared functional accounts may show no SSH “Accepted” lines if people only use `sudo` or console—join with directory data. Short `last_login` windows after a password reset or key-only logins without matching `linux_secure` patterns can false-negative; validate against identity tools.
