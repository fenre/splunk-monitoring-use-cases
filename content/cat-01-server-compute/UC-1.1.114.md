...
*We count open files per process on Linux so runaway programs cannot exhaust the file descriptor table and crash the box.*
...
Databases, reverse proxies, and `java` apps legitimately keep thousands of FDs; threshold per `process` or use percent of `ulimit -n`. TA sampling may count one `lsof` line per FD—confirm whether `count` in SPL matches that model.
