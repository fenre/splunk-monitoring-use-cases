...
*We pay attention when the Linux tools inside Windows show up in places you do not expect, so a hidden shell or unapproved app store install does not sit there unnoticed on servers.*
...
Dev and data science build hosts; WSL2 updates. Tag approved `host` groups; do not run this UC on explicit Linux-dev estates without retuning.
