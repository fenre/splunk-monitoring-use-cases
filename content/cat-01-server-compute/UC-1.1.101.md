...
*We watch how often the Linux kernel switches between processes so we can catch surprising churn that usually means something is thrashing or stuck in a loop.*
...
Short time windows after boot, during `fork` storms from CI, or when `vmstat` sampling cadence changes can make `stdev` unstable. Hosts with very low `cs` can have tiny standard deviations so the 3-sigma rule trips on noise—set a floor on `stddev` or minimum sample count if needed.
