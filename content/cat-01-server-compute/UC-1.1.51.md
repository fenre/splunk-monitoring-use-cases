...
*We watch for signs the computer is resending a lot of network data it already tried to send, which usually means a rough connection or a sick link somewhere along the way.*
...
Wi‑Fi or wide-area test hosts with inherently noisy retrans; counter resets on driver reloads that make `sum(retransSegs)` jump; a too-short `window` that never stabilizes the baseline on bursty services.
