<!-- AUTO-GENERATED from UC-1.1.8.json — DO NOT EDIT -->
...
*We watch for many failed remote login attempts so you can spot someone trying to guess their way in before a weak or stolen password gives them a foothold.*
...
| tstats `summariesonly` count as attempts
  ...
  where nodename=Authentication Authentication.action=failure
  by Authentication.src Authentication.dest span=5m
| where attempts > 10
```
...
- Filters the current rows with `where attempts > 10` after aggregating failed authentication events by source and destination in each 5m bucket.
...
Repeated failures from security scans, mis-typed passwords, automation with stale keys, or shared jump hosts can look like attacks. Pair with asset purpose, pen-test windows, and source reputation before paging out of hours.
