...
*We measure how long CPUs stay in each sleep state so we can spot policy or firmware changes that save power at the cost of latency.*
...
Laptop or datacenter power policies legitimately change C-state mix after BIOS, `cpupower`, or `intel_pstate` updates. The SPL references `total_time` only if your collector emits it—without that field, `idle_pct` is empty rather than wrong.
