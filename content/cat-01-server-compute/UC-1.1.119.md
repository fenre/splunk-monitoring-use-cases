<!-- AUTO-GENERATED from UC-1.1.119.json — DO NOT EDIT -->
...
*We count zombie processes from `ps` data so a buggy service cannot leave behind endless dead children and starve the process table.*
...
Short parentless `Z` states during container stop or CI teardown can clear quickly—use a sustained window. Field name for state may be `STAT` in some `ps` extractions, not `state`—align props to the TA.
