<!-- AUTO-GENERATED from UC-1.1.22.json — DO NOT EDIT -->
...
*We spot modifications to kernel parameters that affect system behavior, security posture, or performance tuning — so you find out before users do when something is slowing down or breaking.*
...
- `stats` rolls up events into metrics; results are split **by host, path, exe, auid** so each row reflects one combination of those dimensions.
...
Hardening, containers, and orchestration can change `sysctl` often. Distinguish policy drift from expected automation using host class and change tickets.
