...
> **Criticality:** High &middot; **Difficulty:** Intermediate &middot; **Pillar:** Observability &middot; **Type:** Performance &middot; **Wave:** Walk

*We watch for storage that is taking too long to answer, even when the machine does not look busy — that is often where slowness starts before anything obvious breaks.*
...
High wait or service times can track heavy batch windows, backup jobs, storage firmware upgrades, or transient path issues. Cross-check with application latency and array or hypervisor health.
