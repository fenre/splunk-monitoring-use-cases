...
*We watch how much data is moving through each network card on the server, and we help you know when a card is so busy that apps may slow down.*
...
Line chart (bandwidth by interface), Dual-axis (bandwidth + errors), Table (top talkers).

## Known False Positives

Backups, VM migration, and patch pushes can max an interface. Baseline the same host’s last month before calling it an incident.
