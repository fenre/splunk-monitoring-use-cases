...
*We watch the directory settings that let one service act as another user, because those are a favorite way to move sideways once someone has a toehold on a web or app server.*
...
Legitimate constrained delegation for web/SQL; cluster resource SPNs. Use attribute-level diff and service owner + CAB prior to false SOC pages.
