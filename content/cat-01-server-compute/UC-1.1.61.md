...
*We help you know when a computer is keeping an unusually large pile of “recently finished” network connections, which can get in the way of opening new ones when traffic is very busy.*
...
Load generators that open huge fan-out short-lived connections on purpose; `warning_level=32000` may be too low on massive connection brokers—raise per service tier; `search` that misses your field name if the parser used `ST` instead of `state`.
