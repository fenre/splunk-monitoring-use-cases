...
*We look at the card that runs your disk storage so you know when it is unhappy or a disk group is in trouble before a full storage failure.*
...
Patrol read, copyback, and foreign import operations can set temporary attention flags. A restart after a kernel driver update can reset counters—compare to the vendor’s management utility before a hardware RMA. Vendor strings for `status` differ; tune **where** to your parser.
