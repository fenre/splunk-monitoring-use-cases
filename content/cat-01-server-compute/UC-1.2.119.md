...
*We look at the list of things Windows runs at login from the registry, so a new auto-start is not the first time you hear about a tool that should not be there on a server.*
...
SSO and VPN client updates under HKCU; gold-image noise. Use path allowlists and compare to prior week baseline.
