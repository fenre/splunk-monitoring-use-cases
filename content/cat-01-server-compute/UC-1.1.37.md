<!-- AUTO-GENERATED from UC-1.1.37.json — DO NOT EDIT -->
...
*Stale NFS handles cause application hangs and I/O failures that severely impact users — so you find out before users do when something is slowing down or breaking.*
...
- `stats` rolls up events into metrics; results are split **by host, nfs_server** so each row reflects one combination of those dimensions.
...
NFS blips during network or storage work look like stale handles in logs. Confirm app-level I/O errors, not a single message.
