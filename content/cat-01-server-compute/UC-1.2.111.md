...
*We show when the host firewall rules change, so a rule someone thought was still there for remote support or file sharing is not silently open to the world after an edit.*
...
GPO-refresh of approved rules; WDAC co-management. Use GPO version and `who` fields; require ticket for non-GPO-sourced local edits on servers.
