...
*We track how many pages the kernel moves in and out of swap so we see memory stress early—not only when the disk is full.*
...
Heavy `si`+`so` can appear during transparent huge page compaction or one-off `swapoff` even when steady-state memory is fine—use duration filters. The `coalesce(baseline_std, 50)` floor embeds a magic number; replace with a macro for your page size and polling cadence.
