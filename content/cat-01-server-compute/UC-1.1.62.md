...
*We watch how much data each network card is moving in each few-minute slice, and we raise a hand when that slice is so busy it’s probably time to add speed or move work.*
...
Counter resets (driver reload) that make **last-first** look huge; very short intervals inside the bin; interfaces that were down part of the window—consider `| where dbytes_in>=0 AND dbytes_out>=0` and a lower bound on `mbps` noise.
