...
*We look at failures when one program on Windows asks another to start through the old shared-component path, so both everyday misconfiguration and attack-style abuse are easier to see.*
...
Remote WMI/COM for monitoring, SCOM, and WMI filters; Exchange health. Whitelist by `AppID` to known services with tickets.
