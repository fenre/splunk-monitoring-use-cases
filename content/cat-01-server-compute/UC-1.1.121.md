<!-- AUTO-GENERATED from UC-1.1.121.json — DO NOT EDIT -->

---
id: "1.1.121"
title: "Bootloader Configuration Changes"
...
> **Criticality:** Critical &middot; **Difficulty:** Beginner &middot; **Pillar:** Observability &middot; **Type:** Performance

*We alert on changes under `/boot` from audit records so the machine cannot boot a surprise kernel or initramfs without us knowing.*

---

## Description

Bootloader changes can enable persistence mechanisms or bypass security controls at boot time.
...
- `stats` rolls up events into metrics; results are split **by host, path, auid** so each row reflects one combination of those dimensions.
...
Kernel updates and `grub2-mkconfig` runs touch bootloader files in normal patch cycles—suppress with change windows. The `path~` regex may need adjustment for your distro’s UEFI path (`/boot/efi/EFI/...`).
