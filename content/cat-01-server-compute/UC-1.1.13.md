...
*Zombie processes indicate parent processes not properly reaping children — so you find out before users do when something is slowing down or breaking.*
...
A few defunct children are normal. Counts can spike if `ps` samples during a fork-heavy job. Focus on growth over time and parent processes that never reap.
