...
*We alert when the kernel says the CPU had to slow down for heat so we can fix fans, airflow, or paste before performance tanks.*

---

## Description

Thermal throttling reduces CPU performance to prevent overheating, indicating cooling system issues.
...
The search uses loose `OR`/`AND` precedence; strings that mention both “CPU” and “throttling” in unrelated contexts (some drivers, benchmarks) can match. Short laptop thermal events under heavy load may be expected—correlate with sustained duration and temperature sensors.
