...
*We help you find one program that is using a big share of the machine’s memory for a while, which is a common path to crashes when memory runs out.*
...
**Elasticsearch** / **JVM** heaps that are large by design; **k8s** pause containers; **40** may be too low on 1**TB** RAM hosts—scale with a **lookup** of `host_ram_gb`.
