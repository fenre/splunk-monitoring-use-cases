...
*We watch for signs that a container or service group is bumping into its memory cap, so you can add memory or move work before things get stopped unexpectedly.*
...
Test clusters that intentionally trigger cgroup OOM; log-format changes that still contain the word cgroup but refer to a healthy accounting line; missing `cgroup_id` parsing so every row lands in null—fix extractions first.
