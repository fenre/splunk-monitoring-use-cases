...
*We help you see when a safety profile blocks an action more than a few times, which is a good time to see if a program changed or if someone is poking at things they shouldn’t.*
...
Developers who intentionally trigger denies in a lab host that shares an index; profiles that `complain` first (no deny) — you may need a separate use case; short spikes during `aa-enforce` rollouts—suppress by **change** window.
