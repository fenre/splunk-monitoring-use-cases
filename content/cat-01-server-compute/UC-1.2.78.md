...
*We watch for use of a special break-glass administrator on domain controllers so we can tell when someone is not doing a normal directory sign-in during everyday work.*
...
Planned DSRM password rotations with a change ticket; DR or AD recovery exercises; rare vendor work at the DC console. Baseline expected `IpAddress` for physical/KVM access; alert on remote or unknown sources.
