<!-- AUTO-GENERATED from UC-1.1.111.json — DO NOT EDIT -->
...
*We flag files that anyone can change so a mistake or intruder cannot leave a trapdoor in critical directories.*
...
Paths under `/tmp`, container layers, and package managers can legitimately be world-writable; exclude with lookups. `find` output volume can be large—cap depth or pre-filter in the collector. Occasional 777 on installer staging dirs during upgrades is normal if time-boxed.
