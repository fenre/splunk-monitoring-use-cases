...
> **Criticality:** Medium &middot; **Difficulty:** Intermediate &middot; **Pillar:** Observability &middot; **Type:** Configuration

*Clock drift causes authentication failures (Kerberos), log correlation issues, transaction ordering problems, and certificate validation failures — so you find out before users do when something is slowing down or breaking.*
...
NTP or chrony can step the clock and change stratum briefly during source changes, VM moves, or maintenance. Alert on persistent drift, not a single sample.
