...
*We look at how current Windows update is on each computer, so out-of-date machines are visible for both security follow-up and honest answers in an audit, not a surprise after an incident.*
...
Air-gapped or change-frozen servers; WSUS deferrals; feature updates that need multiple reboots. Compare to maintenance groups; allow longer `DaysSinceUpdate` for non-Internet hosts only with exception records.
