...
*We help you see who used the “run as superuser” path and what they tried to run, so odd surprises can be checked while they still matter.*
...
Legitimate one-off `sudo` during vendor support windows; `user` field parsing that maps **root** differently; CI users that are supposed to `sudo` dozens of times per hour—move those to an explicit allow class.
