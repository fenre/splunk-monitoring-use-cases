<!-- AUTO-GENERATED from UC-9.1.5.json — DO NOT EDIT -->
...
*We use identity and sign-in data in Splunk so we can notice unusual logins, access changes, and privileged use while it still matters — Kerberos Ticket Anomalies*
...
- `stats` rolls up events into metrics; results are split **by Account_Name, Service_Name** so each row reflects one combination of those dimensions.
...
Service account renewal noise, virtual desktop and jump host auth patterns, or scanning tools; tune to Tier-0 account lists and source subnets.
