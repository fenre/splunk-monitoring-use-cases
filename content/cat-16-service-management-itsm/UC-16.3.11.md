...
*We check whether work that must run on a schedule actually did, so finance, ops, and customers get files when they expect them.*
...
Misses from data delays, holiday calendars, clock skew, or jobs that re-run but still finish in the allowed window.
