<!-- AUTO-GENERATED from UC-16.3.4.json — DO NOT EDIT -->
...
*We time how long work waits when it is passed along, so bridges and handoffs are not the hidden place where work dies.*
...
- `stats` rolls up events into metrics; results are split **by from_group, to_group** so each row reflects one combination of those dimensions.
...
Handoff and escalation delays when a bridge runs long, named owners change shift, or many teams join one ticket.
