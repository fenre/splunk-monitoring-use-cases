<!-- AUTO-GENERATED from UC-16.2.6.json — DO NOT EDIT -->
...
*We notice when the real world and the book slowly disagree, so drift does not build until nobody trusts the data.*
...
- `stats` rolls up events into metrics; results are split **by parent, child, relationship_type** so each row reflects one combination of those dimensions.
...
False duplicates and drift when naming rules change, discovery merges, or the same system has more than one nickname.
