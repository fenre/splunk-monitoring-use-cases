<!-- AUTO-GENERATED from UC-16.3.10.json — DO NOT EDIT -->
...
> **Criticality:** Critical &middot; **Difficulty:** Advanced &middot; **Pillar:** Observability &middot; **Type:** Availability, Compliance

*We add up the pieces that make a service, so a single number tells the business if the whole stack met its promise.*
...
- `stats` rolls up events into metrics; results are split **by component_name** so each row reflects one combination of those dimensions.
...
Dips in composite scores with partial monitoring, one-way work, and feeds that run a few minutes behind the story in chat.
