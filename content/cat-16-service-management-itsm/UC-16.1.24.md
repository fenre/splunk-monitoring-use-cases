<!-- AUTO-GENERATED from UC-16.1.24.json — DO NOT EDIT -->
...
*We help keep a serious incident in step between systems, so status and assignees do not fight each other in two places.*
...
- `stats` rolls up events into metrics; results are split **by has_snow_ticket severity** so each row reflects one combination of those dimensions.
...
Short mismatches from rate limits, network gaps, and bulk updates; line up a few record pairs on the same day in both systems.
