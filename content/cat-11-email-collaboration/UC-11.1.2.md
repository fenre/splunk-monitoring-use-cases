<!-- AUTO-GENERATED from UC-11.1.2.json — DO NOT EDIT -->
...
*We see who opened or changed mailboxes, so we can tell normal support access from snooping or takeover.*
...
```spl
index=o365 sourcetype="o365:management:activity" Workload="Exchange" Operation IN ("MailItemsAccessed","Send","SendAs")
...
- `stats` rolls up events into metrics; results are split **by UserId, Operation, MailboxOwnerUPN** so each row reflects one combination of those dimensions.
...
Legitimate delegate access, shared mailboxes, and service account activity in busy support teams.
