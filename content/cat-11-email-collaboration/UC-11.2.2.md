<!-- AUTO-GENERATED from UC-11.2.2.json — DO NOT EDIT -->
...
*We watch whether Gmail is delivering, deferring, or flagging messages, so mail problems show up in one place.*
...
- `stats` rolls up events into metrics; results are split **by message_info.disposition** so each row reflects one combination of those dimensions.
...
| tstats `summariesonly` count
  ...
  by All_Email.src_user All_Email.action All_Email.recipient All_Email.subject span=1h
| sort -count
```
...
Newsletter spikes, campaign sends, and bulk internal notices that change disposition mix.
