<!-- AUTO-GENERATED from UC-11.1.8.json — DO NOT EDIT -->
...
*We catch sneaky auto-forward and inbox rules, so a stolen account cannot silently ship mail somewhere else.*
...
### Step 3 — Validate
Compare with Exchange Admin Center mailbox rules and Microsoft 365 Defender message investigation for the same users.
...
```spl
| tstats `summariesonly` count from datamodel=Change.All_Changes
  by All_Changes.user All_Changes.object All_Changes.action
| sort -count
```
...
Legitimate forwarding rules for out-of-office automation, helpdesk routing, or executive assistant access.
