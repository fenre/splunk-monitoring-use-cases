<!-- AUTO-GENERATED from UC-11.2.10.json — DO NOT EDIT -->
...
*We see which managed browsers are out of policy, so unpatched or non-compliant clients do not sit there for weeks.*
...
- `stats` rolls up events into metrics; results are split **by device_id, org_unit** so each row reflects one combination of those dimensions.
...
Staged rollouts, delayed check-ins, and lab devices that remain non-compliant for a known window.
