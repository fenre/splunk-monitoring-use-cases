...
• `eval` defines or adjusts **hops** — often to normalize units, derive a ratio, or prepare for thresholds.
• Filters the current rows with `where hops>12` — typically the threshold or rule expression for this monitoring goal.
• Pipeline stage (see **Lateral authentication chains across OT VLANs (TSA IR readiness)**): table src_ip, hops, duration, ot_zone
