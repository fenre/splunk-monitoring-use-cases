---
id: "22.20.3"
title: "CMMC Level 2 practice evidence — CUI control area 3"
status: "verified"
criticality: "low"
...
```spl
index=endpoint sourcetype=windefender:management earliest=-14d
| eval uc_tag="22.20.3"
| where removable_media_policy!="block"
| stats dc(host) as hosts by ou
| sort - hosts
```
...
The first pipeline stage scopes events using **index**: endpoint; **sourcetype**: windefender:management. If that sourcetype is not mentioned in Data sources, double-check parsing or update the documentation to match the feed you actually ingest.

**Pipeline walkthrough**

• Scopes the data: index=endpoint, sourcetype=windefender:management, time bounds. Cross-check against **Data sources** above so indexes and sourcetypes match your ingestion.
• `eval` defines or adjusts **uc_tag** — often to normalize units, derive a ratio, or prepare for thresholds.
• Filters the current rows with `where removable_media_policy!="block"` — typically the threshold or rule expression for this monitoring goal.
• `stats` rolls up events into metrics; results are split **by ou** so each row reflects one combination of those dimensions (useful for per-host, per-user, or per-entity comparisons for this use case).
...
Stacked bar, Table, Time chart, Single value. Key fields: `hosts`, `ou`, `uc_tag`.
