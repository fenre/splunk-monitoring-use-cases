...
- Filters the current rows with `where isnotnull(vendor_site)` — typically the threshold or rule expression for this monitoring goal.
- `stats` rolls up events into metrics; results are split **by Account_Name, ComputerName, vendor_site** so each row reflects one combination of those dimensions.
