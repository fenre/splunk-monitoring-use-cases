...
• `eval` defines or adjusts **bind_ok** — often to normalize units, derive a ratio, or prepare for thresholds.
• Filters the current rows with `where bind_ok=0` — typically the threshold or rule expression for this monitoring goal.
• Pipeline stage (see **ELN signatures missing certificate or hash binding (21 CFR Part 11)**): table signer_id, document_id, certificate_serial
