...
- `stats` rolls up events into metrics; results are split **by terminatingRuleId, coalesce(uri,httpRequest.uri) as path, clientIp** so each row reflects one combination of those dimensions.
