...
- `eval` defines or adjusts **stages_ok** — often to normalize units, derive a ratio, or prepare for thresholds.
- Filters the current rows with `where stages_ok=0 AND category IN ("Normal","Standard")` — typically the threshold or rule expression for this monitoring goal.
