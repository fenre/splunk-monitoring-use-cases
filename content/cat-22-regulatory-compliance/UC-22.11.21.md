...
- `eval` defines or adjusts **key_id** — applying business logic to classify, threshold, or derive compliance-relevant indicators.
- Discretizes time or numeric ranges with `bin`/`bucket`.
- `stats` aggregates events by **_time, key_id**, producing one row per unique combination for trend analysis and threshold evaluation.
- Enriches events using `lookup` (lookup definition + optional OUTPUT fields).
- `eval` defines or adjusts **expected_rotations** — applying business logic to classify, threshold, or derive compliance-relevant indicators.
- Filters the current rows with `where rotations<1` — typically the threshold or rule expression for this monitoring goal.

Output fields: _time, expected_rotations, key_id, rotations.
