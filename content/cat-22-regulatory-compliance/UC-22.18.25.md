...
• `eval` defines or adjusts **jump** — often to normalize units, derive a ratio, or prepare for thresholds.
• Filters the current rows with `where jump > prev*0.5` — typically the threshold or rule expression for this monitoring goal.
• Pipeline stage (see **Wonderware flow/pressure tag jumps over 50 percent (API RP 1164)**): table _time, tagname, value, prev
