...
• `stats` rolls up events into metrics; results are split **by uname** so each row reflects one combination of those dimensions (useful for per-host, per-user, or per-entity comparisons for this use case).
...
• Pipeline stage (see **Segregation of duties conflicts across SAP / Oracle financial roles (SOX §404)**): table uname, roles, conflict_flag
