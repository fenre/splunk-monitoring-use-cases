...
- Filters the current rows with `where gxp_flag="true"` — typically the threshold or rule expression for this monitoring goal.
- `stats` rolls up events into metrics; results are split **by TargetUserName, EventCode** so each row reflects one combination of those dimensions.
