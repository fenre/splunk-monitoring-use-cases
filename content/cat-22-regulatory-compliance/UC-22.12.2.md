...
- Filters the current rows with `where isnull(closed) AND age_days>45` — typically the threshold or rule expression for this monitoring goal.
- `stats` rolls up events into metrics; results are split **by assignment_group, opened_by** so each row reflects one combination of those dimensions.
