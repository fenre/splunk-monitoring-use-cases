...
Control MFA-PCI-01 has a failure rate of 5% over the last 7 days vs baseline 0.5±0.2%; the UC fires.

### Negative scenario

Within-band variability; no alert.
...
- `eventstats` rolls up events into metrics; results are split **by control_id** so each row reflects one combination of those dimensions.
...
### Step 3 — Validate
Compare with the GRC system and the control testing or evidence calendar so stale dates are not confused with real control failure.
...
Evidence stale dates during quarterly cycles, control owner transitions, or platform migrations.
