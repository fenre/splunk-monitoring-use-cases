...
- `stats` rolls up events into metrics; results are split **by sha256, path** so each row reflects one combination of those dimensions.
- `eventstats` rolls up events into metrics; results are split **by sha256** so each row reflects one combination of those dimensions.
- Filters the current rows with `where first_seen=g AND first_seen > relative_time(now(),"-30d@d")` — typically the threshold or rule expression for this monitoring goal.
