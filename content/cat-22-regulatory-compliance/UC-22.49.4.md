...
An S3 bucket with data_domain=customer has a lifecycle of 7 years but the policy catalogue requires 3 years; the UC fires.

### Negative scenario

Bucket lifecycle matches catalogue; no alert.
...
- `eval` defines or adjusts **drift_days** — applying business logic to classify, threshold, or derive compliance-relevant indicators.
- Filters the current rows with `where drift_days>0` — typically the threshold or rule expression for this monitoring goal.

Output fields: configured_days, data_domain, drift_days, policy_days, store.
