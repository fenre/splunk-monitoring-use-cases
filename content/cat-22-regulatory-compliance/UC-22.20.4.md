---
id: "22.20.4"
title: "CMMC Level 2 practice evidence — CUI control area 4"
status: "verified"
...
Demonstrates contractor cybersecurity hygiene for "CMMC Level 2 practice evidence — CUI control area 4" using CUI-scoped telemetry mapped to CMMC 2.0 Level 2 expectations.

## Value

Gives the responsible team auditor-ready evidence for CMMC compliance by automating regulatory compliance evidence using indexes physical and sourcetypes genetec:access. Replaces manual evidence gathering with a continuous Splunk search that produces structured, time-stamped artefacts suitable for regulatory examination and internal audit review.
...
| eval uc_tag="22.20.4"
| where building="CUI-Lab" AND badge_status!="active"
...
• Filters the current rows with `where building="CUI-Lab" AND badge_status!="active"` — typically the threshold or rule expression for this monitoring goal.
• `stats` rolls up events into metrics; results are split **by cardholder, reader_name** so each row reflects one combination of those dimensions (useful for per-host, per-user, or per-entity comparisons for this use case).
...
Stacked bar, Table, Time chart, Single value. Key fields: `cardholder`, `reader_name`, `uc_tag`.
