...
- Filters the current rows with `where cui_flag="true" AND Logon_Type IN ("10","2")` — typically the threshold or rule expression for this monitoring goal.
- `stats` rolls up events into metrics; results are split **by Account_Name, IpAddress** so each row reflects one combination of those dimensions.
