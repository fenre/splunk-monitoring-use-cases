...
• `stats` rolls up events into metrics; results are split **by Account_Name, IpAddress** so each row reflects one combination of those dimensions (useful for per-host, per-user, or per-entity comparisons for this use case).
