---
id: "22.20.9"
title: "CMMC Level 2 practice evidence — CUI control area 9"
status: "verified"
criticality: "high"
...
Demonstrates contractor cybersecurity hygiene for "CMMC Level 2 practice evidence — CUI control area 9" using CUI-scoped telemetry mapped to CMMC 2.0 Level 2 expectations.

## Value

Gives the responsible team auditor-ready evidence for CMMC compliance by automating regulatory compliance evidence using indexes os and sourcetypes chef:compliance. Replaces manual evidence gathering with a continuous Splunk search that produces structured, time-stamped artefacts suitable for regulatory examination and internal audit review.
...
```spl
index=os sourcetype=chef:compliance earliest=-1d
| eval uc_tag="22.20.9"
| where profile="cmmc_level2" AND control_status="failed"
| stats count by control_id, hostname
```
...
The first pipeline stage scopes events using **index**: os; **sourcetype**: chef:compliance. If that sourcetype is not mentioned in Data sources, double-check parsing or update the documentation to match the feed you actually ingest.

**Pipeline walkthrough**

• Scopes the data: index=os, sourcetype=chef:compliance, time bounds. Cross-check against **Data sources** above so indexes and sourcetypes match your ingestion.
• `eval` defines or adjusts **uc_tag** — often to normalize units, derive a ratio, or prepare for thresholds.
• Filters the current rows with `where profile="cmmc_level2" AND control_status="failed"` — typically the threshold or rule expression for this monitoring goal.
• `stats` rolls up events into metrics; results are split **by control_id, hostname** so each row reflects one combination of those dimensions (useful for per-host, per-user, or per-entity comparisons for this use case).
...
Stacked bar, Table, Time chart, Single value. Key fields: `control_id`, `hostname`, `uc_tag`.
