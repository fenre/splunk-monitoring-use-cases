...
• `stats` rolls up events into metrics; results are split **by user, session_id** so each row reflects one combination of those dimensions (useful for per-host, per-user, or per-entity comparisons for this use case).
• Joins to a subsearch with `join` — set `max=` to match cardinality and avoid silent truncation.
• Pipeline stage (see **Shared OT maintenance account attribution via PAM (TSA access)**): table user, session_id, targets, ips
