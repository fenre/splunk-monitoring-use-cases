...
• `eval` defines or adjusts **hash_ok** — often to normalize units, derive a ratio, or prepare for thresholds.
• Filters the current rows with `where hash_ok=0` — typically the threshold or rule expression for this monitoring goal.
• Pipeline stage (see **Veeva document hash mismatch (21 CFR Part 11)**): table document_id, version, user_id
