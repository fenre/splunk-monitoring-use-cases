...
- `stats` rolls up events into metrics; results are split **by sending_ip, sending_facility, sending_app** so each row reflects one combination of those dimensions.
...
Medical device logins from biomedical engineering, scheduled patient transitions, or vendor maintenance.
