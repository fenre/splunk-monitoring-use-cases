...
The first pipeline stage scopes events using **index**: app; **sourcetype**: obiee:usage. If that sourcetype is not mentioned in Data sources, double-check parsing or update the documentation to match the feed you actually ingest.

**Pipeline walkthrough**

- Scopes the data: index=app, sourcetype=obiee:usage, time bounds. Cross-check against **Data sources** above so indexes and sourcetypes match your ingestion.
- `eval` defines or adjusts **uc_tag** — often to normalize units, derive a ratio, or prepare for thresholds.
- `stats` rolls up events into metrics; results are split **by user, report_path, action** so each row reflects one combination of those dimensions.
