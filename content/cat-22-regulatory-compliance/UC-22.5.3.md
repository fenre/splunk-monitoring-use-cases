<!-- AUTO-GENERATED from UC-22.5.3.json — DO NOT EDIT -->

---
id: "22.5.3"
title: "MiFID II Best Execution Monitoring"
criticality: "high"
splunkPillar: "Security"
---

# UC-22.5.3 · MiFID II Best Execution Monitoring

> **Criticality:** High &middot; **Difficulty:** Advanced &middot; **Pillar:** Security &middot; **Type:** Compliance, Performance

*We compares execution quality and routing latency across venues (price improvement, fees, speed) using structured order/execution JSON from OMS/EMS to support best execution oversight.*

---

## Description

Compares execution quality and routing latency across venues (price improvement, fees, speed) using structured order/execution JSON from OMS/EMS to support best execution oversight.

## Value

Gives the responsible team auditor-ready evidence for MiFID II compliance by automating regulatory compliance evidence using indexes trading and sourcetypes _json. Replaces manual evidence gathering with a continuous Splunk search that produces structured, time-stamped artefacts suitable for regulatory examination and internal audit review.

## Implementation

(1) Publish execution reports to HEC with consistent timestamps and normalized units (`effective_spread_bps`, `fee_bps`); (2) refresh baselines weekly; (3) exclude auctions/halts using flags in the JSON; (4) quarterly export for RTS 28 reporting. Indexes required: `index=trading`; Sourcetypes: `sourcetype=_json`.

## Detailed Implementation

### Prerequisites
- Install and configure the required add-on or app: Splunk HTTP Event Collector (core platform) with JSON parsing, Financial Information eXchange (FIX) Log Parsing (Splunkbase 431) (optional).
- Ensure the following data sources are available: `index=trading` `sourcetype="_json"` `source="http:bestex"` (order_id, exec_id, venue, symbol, last_px, effective_spread_bps, fee_bps, exec_latency_ms, decision_time, report_time).
- For app installation, inputs.conf, and Splunk directory layout, see the Implementation guide: docs/implementation-guide.md

### Step 1 — Configure data collection
(1) Publish execution reports to HEC with consistent timestamps and normalized units (`effective_spread_bps`, `fee_bps`); (2) refresh baselines weekly; (3) exclude auctions/halts using flags in the JSON; (4) quarterly export for RTS 28 reporting.

### Step 2 — Create the search and alert
Run the following SPL in Search (then save as report or alert; adjust time range and threshold as needed):

```spl
index=trading sourcetype="_json" source="http:bestex" earliest=-7d
| eval all_in_bps=effective_spread_bps+fee_bps
| stats median(all_in_bps) as p50_cost, median(exec_latency_ms) as p50_latency, count as fills by venue, symbol
| eventstats median(p50_cost) as global_p50 by symbol
| eval venue_delta=round(p50_cost-global_p50, 2)
| sort symbol, venue_delta
| table symbol, venue, fills, p50_cost, p50_latency, venue_delta
```

#### Understanding this SPL

**MiFID II Best Execution Monitoring (Art. 27)** — Compares execution quality and routing latency across venues (price improvement, fees, speed) using structured order/execution JSON from OMS/EMS to support best execution oversight.

Documented **Data sources**: `index=trading` `sourcetype="_json"` `source="http:bestex"` (order_id, exec_id, venue, symbol, last_px, effective_spread_bps, fee_bps, exec_latency_ms, decision_time, report_time). **App/TA** (typical add-on context): Splunk HTTP Event Collector (core platform) with JSON parsing, Financial Information eXchange (FIX) Log Parsing (Splunkbase 431) (optional). The SPL below should target the same indexes and sourcetypes you configured for that feed—rename `index=` / `sourcetype=` if your deployment differs.

The first pipeline stage scopes events using **index**: trading; **sourcetype**: _json. That sourcetype matches what this use case lists under Data sources.

**Pipeline walkthrough**

- Scopes the data: index=trading, sourcetype="_json", time bounds. Cross-check against **Data sources** above so indexes and sourcetypes match your ingestion.
- `eval` defines or adjusts **all_in_bps** — applying business logic to classify, threshold, or derive compliance-relevant indicators.
- `stats` aggregates events by **venue, symbol**, producing one row per unique combination for trend analysis and threshold evaluation.
- `eventstats` rolls up events into metrics; results are split **by symbol** so each row reflects one combination of those dimensions.
- `eval` defines or adjusts **venue_delta** — applying business logic to classify, threshold, or derive compliance-relevant indicators.
- Orders rows with `sort` — combine with `head`/`tail` for top-N patterns.

Enable Data Model Acceleration (and metric indexes for `mstats`) for the models or datasets referenced above; otherwise `tstats`/`mstats` may return no results from summaries.

Output fields: all_in_bps, fills, p50_cost, p50_latency, symbol, venue, venue_delta.

### Step 3 — Validate
Compare with audit evidence in the source system (DLP, IDP, or application) and, if this use case underpins a control test, the ServiceNow GRC console or the evidence pack for a known time window.


### Step 4 — Operationalize
Add the search to a dashboard or set up alert actions (email, webhook, PagerDuty, etc.) as required. Document the use case in your runbook and assign an owner. Consider visualizations: Scatter (latency vs cost), Leaderboard table by venue, Box-style panels via stats.

## SPL

```spl
index=trading sourcetype="_json" source="http:bestex" earliest=-7d
| eval all_in_bps=effective_spread_bps+fee_bps
| stats median(all_in_bps) as p50_cost, median(exec_latency_ms) as p50_latency, count as fills by venue, symbol
| eventstats median(p50_cost) as global_p50 by symbol
| eval venue_delta=round(p50_cost-global_p50, 2)
| sort symbol, venue_delta
| table symbol, venue, fills, p50_cost, p50_latency, venue_delta
```

## Visualization

Scatter (latency vs cost), Leaderboard table by venue, Box-style panels via stats.

## Known False Positives

Reporting gaps during venue maintenance, scheduled feed restatements, or vendor data corrections.

## References

- [Log Parsing](https://splunkbase.splunk.com/app/431)
