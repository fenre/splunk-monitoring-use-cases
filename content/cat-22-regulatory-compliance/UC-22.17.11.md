...
Supports Part 11 technical controls evidence for "Audit trails — control theme 1" by correlating validated system audit trails with identity and change records.
...
• `stats` rolls up events into metrics; results are split **by injection_id, user_id** so each row reflects one combination of those dimensions (useful for per-host, per-user, or per-entity comparisons for this use case).
