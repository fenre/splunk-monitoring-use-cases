...
- `stats` aggregates events by **institution_id, period**, producing one row per unique combination for trend analysis and threshold evaluation.
- Filters the current rows with `where sub!="true"` — typically the threshold or rule expression for this monitoring goal.
Output fields: institution_id, period, sub.
