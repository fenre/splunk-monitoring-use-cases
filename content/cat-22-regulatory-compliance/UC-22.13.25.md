...
- `stats` aggregates events by **visitor_id, reader_id, psp_name**, producing one row per unique combination for trend analysis and threshold evaluation.
- Filters the current rows with `where count>=1` — typically the threshold or rule expression for this monitoring goal.

Output fields: count, psp_name, reader_id, visitor_id.
