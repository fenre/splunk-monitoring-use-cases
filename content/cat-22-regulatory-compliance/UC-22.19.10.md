...
- Filters the current rows with `where cidrip="0.0.0.0/0"` — typically the threshold or rule expression for this monitoring goal.
- `stats` rolls up events into metrics; results are split **by userIdentity.arn, groupId** so each row reflects one combination of those dimensions.
