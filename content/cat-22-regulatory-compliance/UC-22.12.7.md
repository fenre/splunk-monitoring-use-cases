...
- Filters the current rows with `where account_type="service" OR match(Account_Name,"(?i)_svc|svc_")` — typically the threshold or rule expression for this monitoring goal.
- `stats` rolls up events into metrics; results are split **by Account_Name, ComputerName, Logon_Type** so each row reflects one combination of those dimensions.
