...
- `stats` rolls up events into metrics; results are split **by session_server_principal_name, _time, client_ip** so each row reflects one combination of those dimensions.
- `eventstats` rolls up events into metrics; results are split **by session_server_principal_name** so each row reflects one combination of those dimensions.
