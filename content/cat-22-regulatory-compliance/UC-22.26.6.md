...
- `stats` rolls up events into metrics; results are split **by _time, asset, user** so each row reflects one combination of those dimensions.
- `eventstats` rolls up events into metrics; results are split **by asset** so each row reflects one combination of those dimensions.
