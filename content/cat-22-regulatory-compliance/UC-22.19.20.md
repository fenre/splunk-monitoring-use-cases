...
• `eval` defines or adjusts **cnt** — often to normalize units, derive a ratio, or prepare for thresholds.
• Filters the current rows with `where cnt>12` — typically the threshold or rule expression for this monitoring goal.
• Pipeline stage (see **SAP users with excessive role stacking (FISMA / FedRAMP)**): table uname, roles
