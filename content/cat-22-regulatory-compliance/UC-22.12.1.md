...
- Filters the current rows with `where isnull(ritm)` — typically the threshold or rule expression for this monitoring goal.
- `stats` aggregates events by **acct, host**, producing one row per unique combination for trend analysis and threshold evaluation.
...
Output fields: acct, host, ritm, uc_tag.
