...
- `stats` rolls up events into metrics; results are split **by stream_name, violation, host** so each row reflects one combination of those dimensions.
...
Encryption or transport alerts during migration cutovers, lab environments, or compensating controls for legacy apps that are approved in the record of processing.
