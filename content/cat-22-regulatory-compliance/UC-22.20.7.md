---
id: "22.20.7"
title: "CMMC Level 2 practice evidence — CUI control area 7"
status: "verified"
criticality: "low"
...
```spl
index=grc sourcetype=cmmc:spra earliest=-365d
| eval uc_tag="22.20.7"
| where status!="approved"
| stats count by practice_id, reviewer
```
...
The first pipeline stage scopes events using **index**: grc; **sourcetype**: cmmc:spra. If that sourcetype is not mentioned in Data sources, double-check parsing or update the documentation to match the feed you actually ingest.

**Pipeline walkthrough**

• Scopes the data: index=grc, sourcetype=cmmc:spra, time bounds. Cross-check against **Data sources** above so indexes and sourcetypes match your ingestion.
• `eval` defines or adjusts **uc_tag** — often to normalize units, derive a ratio, or prepare for thresholds.
• Filters the current rows with `where status!="approved"` — typically the threshold or rule expression for this monitoring goal.
• `stats` rolls up events into metrics; results are split **by practice_id, reviewer** so each row reflects one combination of those dimensions (useful for per-host, per-user, or per-entity comparisons for this use case).
...
Stacked bar, Table, Time chart, Single value. Key fields: `practice_id`, `reviewer`, `uc_tag`.
