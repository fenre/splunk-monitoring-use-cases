---
id: "22.20.6"
title: "CMMC Level 2 practice evidence — CUI control area 6"
status: "verified"
criticality: "medium"
...
Demonstrates contractor cybersecurity hygiene for "CMMC Level 2 practice evidence — CUI control area 6" using CUI-scoped telemetry mapped to CMMC 2.0 Level 2 expectations.
...
```spl
index=endpoint sourcetype=crowdstrike:detections earliest=-7d tactic="Execution"
| eval uc_tag="22.20.6"
| lookup cui_hosts.csv ComputerName OUTPUT cui_flag
| where cui_flag="true"
| stats count by technique, device_id
```
...
• `stats` rolls up events into metrics; results are split **by technique, device_id** so each row reflects one combination of those dimensions (useful for per-host, per-user, or per-entity comparisons for this use case).
...
Stacked bar, Table, Time chart, Single value. Key fields: `device_id`, `technique`, `uc_tag`.
