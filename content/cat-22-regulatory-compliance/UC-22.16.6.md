...
The first pipeline stage scopes events using **index**: network; **sourcetype**: cisco:ios. If that sourcetype is not mentioned in Data sources, double-check parsing or update the documentation to match the feed you actually ingest.

**Pipeline walkthrough**

- Scopes the data: index=network, sourcetype=cisco:ios, time bounds. Cross-check against **Data sources** above so indexes and sourcetypes match your ingestion.
...
- `stats` rolls up events into metrics; results are split **by mac, host** so each row reflects one combination of those dimensions.
