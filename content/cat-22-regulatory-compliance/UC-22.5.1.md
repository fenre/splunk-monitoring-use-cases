<!-- AUTO-GENERATED from UC-22.5.1.json — DO NOT EDIT -->

---
id: "22.5.1"
title: "MiFID II Trade and Transaction Reporting Completeness"
criticality: "critical"
splunkPillar: "Security"
---

# UC-22.5.1 · MiFID II Trade and Transaction Reporting Completeness

> **Criticality:** Critical &middot; **Difficulty:** Advanced &middot; **Pillar:** Security &middot; **Type:** Compliance, Performance

*We look for reporting gaps (missing submissions vs expected trading-day volume) and ARM/APA rejection spikes to support completeness and accuracy controls for transaction reporting oversight.*

---

## Description

Detects reporting gaps (missing submissions vs expected trading-day volume) and ARM/APA rejection spikes to support completeness and accuracy controls for transaction reporting oversight.

## Value

Gives the responsible team auditor-ready evidence for MiFID II compliance by automating regulatory compliance evidence using indexes trading and sourcetypes _json. Replaces manual evidence gathering with a continuous Splunk search that produces structured, time-stamped artefacts suitable for regulatory examination and internal audit review.

## Implementation

(1) Send ARM/APA acknowledgements and gateway rejects to HEC with a dedicated token; (2) standardize JSON keys (`reject_code`, `report_status`); (3) baseline "expected volume" can be replaced with a lookup of expected daily counts by `venue` and instrument class; (4) schedule daily for compliance desk review. Indexes required: `index=trading`; Sourcetypes: `sourcetype=_json`.

## Detailed Implementation

### Prerequisites
- Install and configure the required add-on or app: Splunk HTTP Event Collector (core platform) with JSON parsing, Financial Information eXchange (FIX) Log Parsing (Splunkbase 431) (optional).
- Ensure the following data sources are available: `index=trading` `sourcetype="_json"` `source="http:trx_reporting"` (transaction_report_id, trade_date, venue, report_status, reject_code).
- For app installation, inputs.conf, and Splunk directory layout, see the Implementation guide: docs/implementation-guide.md

### Step 1 — Configure data collection
(1) Send ARM/APA acknowledgements and gateway rejects to HEC with a dedicated token; (2) standardize JSON keys (`reject_code`, `report_status`); (3) baseline "expected volume" can be replaced with a lookup of expected daily counts by `venue` and instrument class; (4) schedule daily for compliance desk review.

### Step 2 — Create the search and alert
Run the following SPL in Search (then save as report or alert; adjust time range and threshold as needed):

```spl
index=trading sourcetype="_json" source="http:trx_reporting" earliest=-30d@d
| eval rejected=if(isnotnull(reject_code) AND reject_code!="", 1, 0)
| eval accepted=if(report_status IN ("ACCEPTED","ACKED","CONFIRMED") AND rejected=0, 1, 0)
| bin _time span=1d
| stats count as sent, sum(accepted) as accepted, sum(rejected) as rejects, dc(transaction_report_id) as distinct_reports by _time, venue
| eventstats avg(sent) as baseline by venue
| eval volume_gap=if(sent<baseline*0.75, 1, 0)
| where volume_gap=1 OR rejects>0
| table _time, venue, sent, accepted, rejects, volume_gap
| sort _time, venue
```

#### Understanding this SPL

**MiFID II Trade and Transaction Reporting Completeness (Art. 26)** — Detects reporting gaps (missing submissions vs expected trading-day volume) and ARM/APA rejection spikes to support completeness and accuracy controls for transaction reporting oversight.

Documented **Data sources**: `index=trading` `sourcetype="_json"` `source="http:trx_reporting"` (transaction_report_id, trade_date, venue, report_status, reject_code). **App/TA** (typical add-on context): Splunk HTTP Event Collector (core platform) with JSON parsing, Financial Information eXchange (FIX) Log Parsing (Splunkbase 431) (optional). The SPL below should target the same indexes and sourcetypes you configured for that feed—rename `index=` / `sourcetype=` if your deployment differs.

The first pipeline stage scopes events using **index**: trading; **sourcetype**: _json. That sourcetype matches what this use case lists under Data sources.

**Pipeline walkthrough**

- Scopes the data: index=trading, sourcetype="_json", time bounds. Cross-check against **Data sources** above so indexes and sourcetypes match your ingestion.
- `eval` defines or adjusts **rejected** — applying business logic to classify, threshold, or derive compliance-relevant indicators.
- `eval` defines or adjusts **accepted** — applying business logic to classify, threshold, or derive compliance-relevant indicators.
- Discretizes time or numeric ranges with `bin`/`bucket`.
- `stats` aggregates events by **_time, venue**, producing one row per unique combination for trend analysis and threshold evaluation.
- `eventstats` rolls up events into metrics; results are split **by venue** so each row reflects one combination of those dimensions.
- `eval` defines or adjusts **volume_gap** — applying business logic to classify, threshold, or derive compliance-relevant indicators.
- Filters the current rows with `where volume_gap=1 OR rejects>0` — typically the threshold or rule expression for this monitoring goal.

- Orders rows with `sort` — combine with `head`/`tail` for top-N patterns.

Enable Data Model Acceleration (and metric indexes for `mstats`) for the models or datasets referenced above; otherwise `tstats`/`mstats` may return no results from summaries.

Output fields: _time, accepted, distinct_reports, rejected, rejects, sent, venue, volume_gap.

### Step 3 — Validate
Compare with audit evidence in the source system (DLP, IDP, or application) and, if this use case underpins a control test, the ServiceNow GRC console or the evidence pack for a known time window.


### Step 4 — Operationalize
Add the search to a dashboard or set up alert actions (email, webhook, PagerDuty, etc.) as required. Document the use case in your runbook and assign an owner. Consider visualizations: Timechart (accepted vs rejects), Single value (gap days counter), Table (worst venues by reject rate).

## SPL

```spl
index=trading sourcetype="_json" source="http:trx_reporting" earliest=-30d@d
| eval rejected=if(isnotnull(reject_code) AND reject_code!="", 1, 0)
| eval accepted=if(report_status IN ("ACCEPTED","ACKED","CONFIRMED") AND rejected=0, 1, 0)
| bin _time span=1d
| stats count as sent, sum(accepted) as accepted, sum(rejected) as rejects, dc(transaction_report_id) as distinct_reports by _time, venue
| eventstats avg(sent) as baseline by venue
| eval volume_gap=if(sent<baseline*0.75, 1, 0)
| where volume_gap=1 OR rejects>0
| table _time, venue, sent, accepted, rejects, volume_gap
| sort _time, venue
```

## Visualization

Timechart (accepted vs rejects), Single value (gap days counter), Table (worst venues by reject rate).

## Known False Positives

Reporting gaps during venue maintenance, scheduled feed restatements, or vendor data corrections.

## References

- [Log Parsing](https://splunkbase.splunk.com/app/431)
