...
- `stats` aggregates events by **host, name, auid**, producing one row per unique combination for trend analysis and threshold evaluation.
- Correlates data from a subsearch to enrich the primary result set with additional context.
- `eval` defines or adjusts **near_deploy** — applying business logic to classify, threshold, or derive compliance-relevant indicators.
...
Output fields: auid, deploy_t, host, name, near_deploy, process, t0.
...
FIM alerts from authorized patches, application updates, or vendor-provided config changes.
