...
- `stats` rolls up events into metrics; results are split **by u_ir_stage, state** so each row reflects one combination of those dimensions.
...
Response time may extend during shift handoffs, holiday weekends, or major incident bursts.
