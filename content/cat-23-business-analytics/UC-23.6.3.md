...
• `stats` rolls up events into metrics; results are split **by cost_centre, gl_account** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare budget and actuals to the ERP, planning, or FP&A models for the same chart of account mapping and period.
