...
• `stats` rolls up events into metrics; results are split **by course_name, compliance_status** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
• `eventstats` rolls up events into metrics; results are split **by course_name** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare training completion to your learning management system and HRIS compliance views for the same courses and due dates.
