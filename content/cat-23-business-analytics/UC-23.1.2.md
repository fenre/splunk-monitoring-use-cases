...
• `stats` rolls up events into metrics; results are split **by session** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare abandonment rate, session counts, and cart value to your shop platform reports and the web analytics dashboard (GA, Adobe). Reconcile a sample of sessions where cart_add fired but purchase did not to confirm timing and event naming.
