...
• `stats` rolls up events into metrics; results are split **by entity_a, entity_b, severity** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare intercompany exception counts and balances to the ERP intercompany workbench and the close checklist your accounting team already uses.
