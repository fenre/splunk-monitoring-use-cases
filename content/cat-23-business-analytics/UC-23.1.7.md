...
• `stats` rolls up events into metrics; results are split **by search_term** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare search queries, zero-result rates, and clicks to your site search analytics (Algolia, Elasticsearch UI, CMS) and the web analytics dashboard (GA, Adobe) for the same site section.
