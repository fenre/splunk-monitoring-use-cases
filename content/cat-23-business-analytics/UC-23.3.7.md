...
• `stats` rolls up events into metrics; results are split **by event_id** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare registrants, attendance, and pipeline to the event platform, the marketing platform analytics, and the CRM dashboard for the same program codes.
