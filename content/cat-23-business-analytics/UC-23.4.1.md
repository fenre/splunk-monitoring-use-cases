...
• `stats` rolls up events into metrics; results are split **by department, tenure_band** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Reconcile people counts and time ranges to the HR system of record (Workday, SuccessFactors, or your payroll and HRIS reports). For sensitive areas, use aggregated views that match what HR already publishes for leadership.
