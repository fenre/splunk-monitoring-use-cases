...
• `stats` rolls up events into metrics; results are split **by reporting_framework** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare metric completeness and sign-off to the ESG project plan, data-owner attestations, and the disclosure checklist your sustainability team already maintains.
