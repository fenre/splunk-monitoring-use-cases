...
• `stats` rolls up events into metrics; results are split **by assignment_group, breach_risk** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare backlog age and forecast to the support tool WFM and queue settings for the same contracts and business hours.
