...
• `stats` rolls up events into metrics; results are split **by resource_host** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare third-party and API timing to your tag management or RUM vendor and the web analytics dashboard (GA, Adobe). Verify tag inventory and page groups match what this search assumes.
