...
• `stats` rolls up events into metrics; results are split **by department, week** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare absence and leave totals to the HRIS and time system for the same policies, regions, and pay periods. Respect privacy: use aggregates appropriate for your governance.
