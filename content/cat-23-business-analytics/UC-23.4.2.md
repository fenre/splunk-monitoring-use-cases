...
• `stats` rolls up events into metrics; results are split **by department** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare time-to-hire and stage counts to your applicant tracking system and HRIS reports for the same requisition set and date range.
