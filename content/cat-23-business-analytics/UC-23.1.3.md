...
• `stats` rolls up events into metrics; results are split **by perf_bucket** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare page timing and revenue-correlated segments to the web analytics dashboard (GA, Adobe) and any real user monitoring or speed tool your team trusts. Align the same page definitions and date range before you trust week-over-week trends.
