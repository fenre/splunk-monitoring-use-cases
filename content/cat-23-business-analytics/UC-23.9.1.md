...
• `stats` rolls up events into metrics; results are split **by scope, source** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
• Orders rows with `sort` — combine with `head`/`tail` for top-N patterns.
• Pipeline stage (see **Carbon Footprint Tracking and Reduction Progress**): table scope, source, total_tonnes


Step 3 — Validate
Compare emissions totals to facility data, ESG workbooks, and the numbers your sustainability and finance teams use for published reporting. When factors or boundaries change, re-baseline the comparison so it matches the disclosure rules.
