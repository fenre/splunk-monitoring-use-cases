...
• `stats` rolls up events into metrics; results are split **by channel, issue_type** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare effort scores and channel mix to survey tools and the support tool for the same tickets and time range.
