...
• `stats` rolls up events into metrics; results are split **by customer_id** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare churn risk scores and inputs to the CRM or subscription platform (renewal dates, ARR), billing status in your payment system, and the support tool (Zendesk, ServiceNow) for a sample of at-risk customers. Calibrate score weights to historical churn, not one noisy week.
