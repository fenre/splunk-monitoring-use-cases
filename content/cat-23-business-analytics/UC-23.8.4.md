...
• `stats` rolls up events into metrics; results are split **by quarter** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare growth, margin, and the rule-of-40 components to the ERP, planning model, and investor reporting for the same fiscal rules.
