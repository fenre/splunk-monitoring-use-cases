...
• `stats` rolls up events into metrics; results are split **by line_id, week** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare capacity and demand to the ERP, planning tool, and production schedule for the same sites and time buckets.
