...
• `stats` rolls up events into metrics; results are split **by customer_tier** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare perfect-order components to the ERP, WMS, and customer case data for a sample of orders in the same window.
