...
• `stats` rolls up events into metrics; results are split **by _time, site, system** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
• `stats` rolls up events into metrics; results are split **by site, system** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
• `stats` rolls up events into metrics; results are split **by site, square_metres** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare kWh, benchmarks, and site totals to utility invoices, the BMS, and the ESG energy workbook for the same meters and period.
