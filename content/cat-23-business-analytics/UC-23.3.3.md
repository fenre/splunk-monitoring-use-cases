...
• `stats` rolls up events into metrics; results are split **by campaign_id** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare opens, clicks, and bounces to the marketing platform analytics (mail automation: Mailchimp, Hubspot, Eloqua) and your email deliverability reports for the same sends.
