...
• `stats` rolls up events into metrics; results are split **by discount_band** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare discount usage and price bands to the CRM dashboard and any pricing-approval or CPQ system for the same period. Reconcile a sample of quotes and orders.
