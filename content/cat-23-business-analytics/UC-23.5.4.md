...
• `stats` rolls up events into metrics; results are split **by carrier, service_level** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare delivery SLAs and last-mile times to the transport or TMS system and customer-facing delivery promises for the same lanes.
