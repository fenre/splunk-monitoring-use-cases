...
• `stats` rolls up events into metrics; results are split **by forecast_category** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare pipeline totals, stages, and amounts to the CRM dashboard (Salesforce, HubSpot, or your system) for the same dates. Spot-check a few opportunities that changed stage to confirm fields and close dates match what finance and sales see in the CRM.
