...
• `stats` rolls up events into metrics; results are split **by _time, site** so each row reflects one combination of those dimensions (useful for comparisons across those breakdowns for this use case).
...
Step 3 — Validate
Compare water usage to meter reads, utility bills, and the ESG water table for the same sites and units.
