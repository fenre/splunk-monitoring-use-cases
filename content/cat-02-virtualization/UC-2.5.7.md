<!-- AUTO-GENERATED from UC-2.5.7.json — DO NOT EDIT -->
...
*We keep an eye on iGEL Device Resource Utilization and raise the alarm before it drags down real work or real outages start.*
...
- `stats` rolls up events into metrics; results are split **by mem_tier, flash_tier** so each row reflects one combination of those dimensions.
...
Proxmox and hypervisor health can go yellow during Ceph rebalancing, zfs scrubs, or one-node maintenance; check cluster quorum and job progress before reassigning the alert.
