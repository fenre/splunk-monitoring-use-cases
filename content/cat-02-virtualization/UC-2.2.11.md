<!-- AUTO-GENERATED from UC-2.2.11.json — DO NOT EDIT -->
...
*We keep an eye on storage Spaces Direct (S2D) Health and raise the alarm before it drags down real work or real outages start.*
...
- `stats` rolls up events into metrics; results are split **by pool_name, host** so each row reflects one combination of those dimensions.
...
Hyper-V counters can wobble when guests reboot, when integration services update, or during quick live migration; align spikes with host maintenance and live migration events.
