...
*We keep an eye on cloudformation stack drift. Drift means infrastructure no longer matches its declared template — manual changes have been made. This breaks IaC and. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
Scheduled template drift runs right after a pipeline deploy, before auto-remediation re-applies the template.
