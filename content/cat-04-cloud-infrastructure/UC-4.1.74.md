<!-- AUTO-GENERATED from UC-4.1.74.json — DO NOT EDIT -->
...
*We keep an eye on iam access analyzer findings. Unexpected paths for data or access that the analyzer found so the team can close them before a stranger stumbles in. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
- `stats` rolls up events into metrics; results are split **by detail.resourceType, detail.principal.awsAccountId, detail.isPublic** so each row reflects one combination of those dimensions.
...
| tstats `summariesonly` count
  ...
  where match(Alerts.app, "(?i)access.analyzer|iam") OR match(Alerts.signature, "(?i)external|public|access")
  by Alerts.severity Alerts.src span=1h
| sort -count
```
...
Intentional public buckets or roles for a static site, partner data exchange, or test account that the policy still documents.
