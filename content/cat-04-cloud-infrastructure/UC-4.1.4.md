...
*We keep an eye on iam policy changes. IAM policy changes affect who can do what across the entire AWS account. Unauthorized policy attachments can grant admin. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
CI/CD or IaC that attaches known managed policies; emergency access policies already on the change record.
