...
*We keep an eye on aws backup restore job failures. Backups and restores that do not complete correctly so we are not one restore away from a bad surprise. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*

---

## Description

Restore job failures prevent recovery during DR. Monitoring ensures backup and restore pipeline is healthy.
...
Table (job, resource, state, message), Timeline (failed restores), Single value (failed jobs last 24h).

## Known False Positives

Jobs that you cancelled on purpose, DR drills, and pre-prod runs that you expect to fail a gate.
