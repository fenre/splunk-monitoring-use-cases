...
*We keep an eye on rds performance insights trending. How heavy the database is over time (Performance Insights) so tuning waits and right-sizing is based on data, not guesswork. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
Large ad hoc queries, ETL, or an index build that you expect to spike DB load during the same maintenance window as the alert.
