...
*We keep an eye on lambda layer version compliance. Outdated layers carry vulnerable dependencies; enforcing approved layer ARNs avoids shadow IT libraries in functions. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
CI/CD and hotfix deploys that swap Lambda layers; compare layer ARNs to the service owner before flagging as drift.
