...
*We keep an eye on ec2 spot instance interruption notices. The notice that a cheap spot node may be taken back so the app can drain work or request capacity elsewhere in time. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
Intentional spot reclaim during game-day, batch jobs, and capacity tests where interruption is the design.
