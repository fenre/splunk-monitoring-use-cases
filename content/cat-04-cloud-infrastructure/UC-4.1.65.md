...
*We keep an eye on guardduty severity analysis. The serious GuardDuty finding types the service raises, in one place, so a small team does not miss something loud in the console. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
Benign port scans, red-team, authorized network scanners, and new accounts/regions before suppressions and baselines are tuned.
