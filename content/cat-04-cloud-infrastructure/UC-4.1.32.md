...
*We keep an eye on nat gateway bytes processed and connection tracking. How much is leaving the private network through the NAT and how many connections it carries, so a quiet cutover does not show up only as a ticket. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
Large data transfers, backups, and regional failovers you scheduled; one-off spikes during migration, not a silent exfil by default.
