...
*We keep an eye on root account usage. Any sign-in using the special root identity, which should be almost never because normal admins should use named roles with limits. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
Initial account setup, rare billing or support cases your policy still routes through root; documented break-glass; MFA and payment instrument changes in small businesses.
