...
> **Criticality:** High &middot; **Difficulty:** Intermediate &middot; **Pillar:** Security &middot; **Type:** Capacity

*Hitting account or region quotas (cloud servers instance limit, VPCs, EBS volumes) blocks provisioning and causes runtime failures.*
...
Planned load tests, new regions, and one-off batch jobs can legitimately run near limits. We treat a breach as normal when it is tied to a change ticket and a known window, and we reset after the window ends.
