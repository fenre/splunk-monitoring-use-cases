<!-- AUTO-GENERATED from UC-4.1.59.json — DO NOT EDIT -->
...
*We use Splunk to watch s3 suspicious access patterns—unusual listbucket volume, access from new regions, or anonymous reads often precede data exfiltration; pattern detection reduces dwell time.—so the team is told early, not only after a customer or auditor notices a gap.*
...
- `stats` rolls up events into metrics; results are split **by userIdentity.arn, requestParameters.bucketName** so each row reflects one combination of those dimensions.
...
Approved changes, pre-production, and known vendor windows already named in the runbook for this s3 suspicious access patterns.
