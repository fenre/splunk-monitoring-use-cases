<!-- AUTO-GENERATED from UC-4.1.42.json — DO NOT EDIT -->
...
*We keep an eye on step functions execution failures. The key performance numbers the cloud already exposes so the team can plan capacity and notice problems before user-visible outages. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*

---

## Description

Failed or aborted executions break workflows. Tracking failure rate and failed execution IDs enables debugging and retry.
...
Line chart (failed, aborted by workflow), Table (state machine, count), Single value.

## Known False Positives

Planned deploys, game-day tests, and vendor maintenance that you expect to move CPU, error, or lag metrics; compare with the change window.
