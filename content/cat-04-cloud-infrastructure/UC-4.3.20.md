<!-- AUTO-GENERATED from UC-4.3.20.json — DO NOT EDIT -->
...
*Cloud Armor blocks and overload attacks metrics indicate attack traffic and policy effectiveness.*
...
- `stats` rolls up events into metrics; results are split **by jsonPayload.enforcedSecurityPolicy.outcome jsonPayload.enforcedSecurityPolicy.name** so each row reflects one combination of those dimensions.
...
This pattern can surge during change windows, in sandboxes, and when a platform is in maintenance, so we line alerts up with the calendar and take a second look before we call it an incident.
