...
*We keep an eye on security hub alert aggregation. Security Hub results pulled into one stream so a single place can own triage, not a dozen product consoles. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
Suppressed or accepted findings, duplicate product integrations, and member-account lag that catches up a few minutes later.
