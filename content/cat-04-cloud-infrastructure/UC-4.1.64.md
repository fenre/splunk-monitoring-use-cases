<!-- AUTO-GENERATED from UC-4.1.64.json — DO NOT EDIT -->
...
*We keep an eye on eks control plane audit. Kubernetes audit logs capture who changed roles, secrets, and workloads; essential for forensics and SOC2 evidence on EK. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
Platform or GitOps that edits `clusterroles` or `secrets` during a cluster upgrade; compare with the release runbook.
