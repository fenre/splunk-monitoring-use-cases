...
*We keep an eye on vpc flow log analysis. The traffic the network chose to block so we can see odd ports, regions, or quiet probes before they become a breach. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
Legitimate NACLs or security groups that block expected scanners; canary and migration traffic you already whitelisted in the runbook.
