<!-- AUTO-GENERATED from UC-4.3.24.json — DO NOT EDIT -->
...
*Serverless cold start impact on request latency.*
...
- `stats` rolls up events into metrics; results are split **by resource.labels.service_name, metric_type, bin(_time, 5m)** so each row reflects one combination of those dimensions.
- `eval` defines or adjusts **cold_start_indicator** — often to normalize units, derive a ratio, or prepare for thresholds.
- `stats` rolls up events into metrics; results are split **by resource.labels.service_name** so each row reflects one combination of those dimensions.
...
**GCP Cloud Run Cold Start Rate** — Serverless cold start impact on request latency.
