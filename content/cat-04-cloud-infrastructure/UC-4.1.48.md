...
*We keep an eye on athena query execution failures and bytes scanned. Failed Athena work that might hide reporting gaps or a permissions mistake before it hits the board meeting. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
Analysts stopping large queries, bad SQL in a lab, and role tests that you expect to fail in sandboxes.
