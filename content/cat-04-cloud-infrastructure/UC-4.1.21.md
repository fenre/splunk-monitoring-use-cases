...
*We keep an eye on alb/nlb access logs and 5xx errors. Web requests that the edge blocked or that returned server error codes so the team can tell attacks and broken deploys apart. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
Known pen-test, dark-traffic, or canary 5xx during deploy; WAF in count or shadow mode you expect to list blocks for.
