...
*We keep an eye on aws lambda cold start monitoring. Cold start frequency and duration impact user experience. High cold start rates or long init times cause request latency spikes an. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
Known slow cold starts, scheduled batch jobs, and canary invocations in lower envs you already noise-reduced.
