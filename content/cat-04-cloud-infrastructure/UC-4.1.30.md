...
*We keep an eye on cloudtrail log file delivery failures. Failed CloudTrail delivery means audit gaps. Attackers may target trail deletion or S3 permissions to hide activity. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
Intentional trail or bucket hardening in the same change ticket; always confirm a matching ticket before paging.
