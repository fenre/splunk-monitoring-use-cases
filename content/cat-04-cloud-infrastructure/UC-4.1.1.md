...
*We keep an eye on unauthorized api calls. Repeated access-denied replies from the cloud control plane can mean someone probing permissions, broken automation, or a stolen key that no longer fits. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
Pentest or vulnerability scanning that intentionally hits forbidden APIs; new least-privilege rollouts; automation that lost IAM scope after a change window.
