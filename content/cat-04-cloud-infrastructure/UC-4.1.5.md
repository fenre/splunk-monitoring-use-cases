<!-- AUTO-GENERATED from UC-4.1.5.json — DO NOT EDIT -->
...
*We keep an eye on console login without mfa. Successful sign-ins to the AWS web console that did not use a second step, so a stolen password is still dangerous for admin work. We want the right people to notice in time, not only when a customer or an auditor stumbles on it first.*
...
Federated SSO through SAML or OIDC that enforces MFA at the IdP, so the AWS record may not show the same second factor detail.
