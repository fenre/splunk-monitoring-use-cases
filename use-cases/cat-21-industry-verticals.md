...
### 21.11 Regulatory and Compliance Frameworks

**Primary App/TA:** Splunk Enterprise Security, Splunk App for Compliance, custom compliance dashboards, Edge Processor (PII masking), SOAR for incident workflow.

**Data Sources:** Cross-system: security logs, access logs, change management, data flow logs, incident management, vulnerability scanners, backup systems, network flows with geo-enrichment.

---

### UC-21.11.1 · GDPR PII Detection in Application Log Data (Art. 5/6)
- **Criticality:** 🔴 Critical
- **Difficulty:** 🟠 Advanced
- **Monitoring type:** Security, Compliance
- **Value:** Articles 5 and 6 require data minimization and lawful processing; detecting PII in application logs shows where processing may lack a documented basis or retention discipline before supervisory review.
- **App/TA:** Splunk App for Compliance, Edge Processor
- **Data Sources:** `index=compliance` `sourcetype="compliance:pii"` (app_name, field_name, detection_rule, confidence, masked_sample)
- **SPL:**
```spl
index=compliance sourcetype="compliance:pii"
| stats count as hits, dc(app_name) as apps, values(detection_rule) as rules by field_name
| where hits > 100 OR mvcount(rules) > 3
| sort - hits
| table field_name, hits, apps, rules
```
- **Implementation:** Route application stdout and API logs through Edge Processor with regex/ML PII classifiers; tag detections without storing raw secrets. Map `app_name` to RoPA entries for gap triage. Alert product owners when new PII-bearing fields appear in non-approved paths.
- **Visualization:** Bar chart (hits by field), Treemap (apps × field), Table (top violations).
- **CIM Models:** N/A

---

### UC-21.11.2 · GDPR Data Subject Access Request Fulfillment Tracking (Art. 15-22)
- **Criticality:** 🟠 High
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Compliance, Performance
- **Value:** Articles 15–22 impose timelines for access, rectification, erasure, and portability; tracking request lifecycle proves regulator-ready evidence of timely fulfillment.
- **App/TA:** Splunk App for Compliance, SOAR
- **Data Sources:** `index=compliance` `sourcetype="dsar:request"` (request_id, request_type, opened_epoch, due_epoch, completed_epoch, status)
- **SPL:**
```spl
index=compliance sourcetype="dsar:request"
| eval due_s=if(isnotnull(due_epoch), due_epoch, opened_epoch+2592000)
| eval completed=if(lower(status)=="completed" OR lower(status)=="closed", completed_epoch, null())
| eval breach=if(isnotnull(completed), completed>due_s, now()>due_s)
| where breach=1
| eval days_late=if(isnotnull(completed), round((completed-due_s)/86400,1), round((now()-due_s)/86400,1))
| table request_id, request_type, status, days_late
```
- **Implementation:** Ingest DSAR tickets from the privacy portal or ITSM with consistent `due_epoch` (calendar month default if unset). Join SOAR playbooks for automated reminders. Exclude duplicate child tasks via `request_id` parent key if modeled.
- **Visualization:** Single value (open overdue), Timeline (requests), Table (breach detail).
- **CIM Models:** N/A

---

### UC-21.11.3 · GDPR Breach Notification Timeline Monitoring (Art. 33, 72-hour rule)
- **Criticality:** 🔴 Critical
- **Difficulty:** 🟠 Advanced
- **Monitoring type:** Compliance, Security
- **Value:** Article 33 requires supervisory notification without undue delay and, where feasible, within 72 hours of awareness; measuring detection-to-notification intervals supports defensible timelines.
- **App/TA:** Splunk Enterprise Security, SOAR
- **Data Sources:** `index=compliance` `sourcetype="breach:notification"` (incident_id, detected_epoch, authority_notified_epoch, data_subject_scope, severity)
- **SPL:**
```spl
index=compliance sourcetype="breach:notification"
| eval notify_lag_h=if(isnotnull(authority_notified_epoch), (authority_notified_epoch-detected_epoch)/3600, (now()-detected_epoch)/3600)
| where notify_lag_h > 72 OR isnull(authority_notified_epoch)
| eval status=if(isnull(authority_notified_epoch), "PENDING_AUTHORITY_NOTIFY", "LATE_NOTIFY")
| table incident_id, detected_epoch, authority_notified_epoch, notify_lag_h, severity, status
```
- **Implementation:** Feed SOAR case milestones into `breach:notification`; stamp `detected_epoch` from first credible DLP or IR ticket. Legal holds may adjust clocks—store `clock_pause_reason` if modeled. Escalate when 72 hours elapses without `authority_notified_epoch`.
- **Visualization:** Gauge (hours to deadline), Timeline (milestones), Table (open breaches).
- **CIM Models:** N/A

---

### UC-21.11.4 · GDPR Data Retention Policy Enforcement (Art. 5(1)(e))
- **Criticality:** 🟠 High
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Compliance, Capacity
- **Value:** Article 5(1)(e) limits storage to what is necessary for the purposes; automated checks against declared retention policies reduce “forever copy” risk in data lakes and backups.
- **App/TA:** Splunk App for Compliance
- **Data Sources:** `index=compliance` `sourcetype="retention:policy"` (dataset_id, policy_days, oldest_record_epoch, last_purge_epoch, owner)
- **SPL:**
```spl
index=compliance sourcetype="retention:policy"
| eval age_days=(now()-oldest_record_epoch)/86400
| eval overdue=age_days > policy_days
| eval purge_lag_days=if(isnotnull(last_purge_epoch), (now()-last_purge_epoch)/86400, null())
| where overdue=1 OR (isnotnull(purge_lag_days) AND purge_lag_days > policy_days*1.1)
| table dataset_id, policy_days, age_days, purge_lag_days, owner
```
- **Implementation:** Nightly inventory jobs push oldest-object metadata per dataset; align backup retention with primary store policies. Document legal holds that extend retention in a separate field to suppress false positives.
- **Visualization:** Bar chart (age vs policy), Table (overdue datasets), Single value (count breaching).
- **CIM Models:** N/A

---

### UC-21.11.5 · GDPR Consent Management Audit Trail (Art. 7)
- **Criticality:** 🟠 High
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Compliance, Audit
- **Value:** Article 7 requires demonstrable consent, including withdrawal as easy as giving consent; correlating CMP events with downstream processing proves the audit trail regulators expect.
- **App/TA:** Splunk App for Compliance
- **Data Sources:** `index=compliance` `sourcetype="consent:audit"` (user_hash, purpose_id, action, channel, epoch, cmp_version)
- **SPL:**
```spl
index=compliance sourcetype="consent:audit"
| bin _time span=1d
| stats count as events, dc(user_hash) as users by purpose_id, action, _time
| eventstats sum(eval(if(action=="withdraw",events,0))) as withdraws, sum(eval(if(action=="accept",events,0))) as accepts by purpose_id, _time
| eval withdraw_ratio=if(accepts+withdraws>0, withdraws/(accepts+withdraws), null())
| where withdraw_ratio > 0.4 OR events > 100000
| table _time, purpose_id, action, events, users, withdraw_ratio
```
- **Implementation:** Stream CMP webhooks and mobile SDK events into HEC with hashed subject IDs. Version `cmp_version` to detect stale banners. Join to marketing sends only in aggregated form to avoid re-identification.
- **Visualization:** Line chart (accept vs withdraw), Stacked bar (by purpose), Table (spikes).
- **CIM Models:** N/A

---

### UC-21.11.6 · GDPR Cross-Border Data Transfer Monitoring (Art. 44-49)
- **Criticality:** 🔴 Critical
- **Difficulty:** 🟠 Advanced
- **Monitoring type:** Security, Compliance
- **Value:** Articles 44–49 govern transfers outside the EEA; monitoring flows to non-adequate destinations with safeguards (SCCs, BCRs) supports transfer impact assessments and breach of contractual clauses.
- **App/TA:** Splunk Enterprise Security, custom geo dashboards
- **Data Sources:** `index=compliance` `sourcetype="dataflow:transfer"` (flow_id, src_region, dst_region, safeguard_type, bytes, app_name)
- **SPL:**
```spl
index=compliance sourcetype="dataflow:transfer"
| eval eea=if(match(dst_region,"(?i)^(DE|FR|IE|IT|ES|NL|BE|AT|SE|DK|FI|PT|PL|CZ|RO|HU|BG|HR|SI|SK|EE|LV|LT|CY|LU|MT|GR)$"),1,0)
| where eea=0 AND (len(safeguard_type)==0 OR lower(safeguard_type) IN ("none","unknown"))
| bin _time span=1h
| stats sum(bytes) as total_b, dc(flow_id) as flows by dst_region, app_name, _time
| where total_b > 1073741824
| eval total_gb=round(total_b/1073741824,2)
| table _time, dst_region, app_name, total_gb, flows
```
- **Implementation:** Enrich NetFlow, cloud egress, and API gateway logs with region codes and SCC metadata. Replace the inline EEA regex with your legal team’s adequacy list (KVStore lookup) if needed. Tune byte thresholds per environment; exclude CDN cache hits if tagged.
- **Visualization:** Choropleth (dst_region), Sankey (region flows), Table (top risky flows).
- **CIM Models:** N/A

---

### UC-21.11.7 · NIS2 Incident Detection and 24-Hour Early Warning Reporting (Art. 23)
- **Criticality:** 🔴 Critical
- **Difficulty:** 🟠 Advanced
- **Monitoring type:** Security, Compliance
- **Value:** Article 23 sets early warning and incident notification duties for essential and important entities; proving detection time and 24-hour early-warning dispatch supports competent authority timelines.
- **App/TA:** Splunk Enterprise Security, SOAR
- **Data Sources:** `index=compliance` `sourcetype="incident:nis2"` (incident_id, detected_epoch, early_warning_sent_epoch, csirt_id, sector)
- **SPL:**
```spl
index=compliance sourcetype="incident:nis2"
| eval early_h=if(isnotnull(early_warning_sent_epoch), (early_warning_sent_epoch-detected_epoch)/3600, (now()-detected_epoch)/3600)
| where early_h > 24 OR isnull(early_warning_sent_epoch)
| eval risk=if(early_h>48,"BREACH_24H_WINDOW","AT_RISK")
| table incident_id, sector, detected_epoch, early_warning_sent_epoch, early_h, risk
```
- **Implementation:** Map ES notable events to NIS2 case objects; SOAR posts `early_warning_sent_epoch` when the portal API confirms receipt. Use sector-specific playbooks for OT versus corporate IT. Document false-positive closures without early warning.
- **Visualization:** Timeline (detection to warning), Single value (incidents past 24h), Table (open).
- **CIM Models:** N/A

---

### UC-21.11.8 · NIS2 Supply Chain Security Monitoring (Art. 21(2)(d))
- **Criticality:** 🟠 High
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Security, Compliance
- **Value:** Article 21(2)(d) addresses supply-chain risks for network and information systems; tracking vendor incidents, expiring assessments, and anomalous third-party access reduces systemic exposure.
- **App/TA:** Splunk Enterprise Security, third-party risk feeds
- **Data Sources:** `index=compliance` `sourcetype="supply:chain"` (vendor_id, assessment_due_epoch, open_finding_count, privileged_session_flag, last_incident_epoch)
- **SPL:**
```spl
index=compliance sourcetype="supply:chain"
| eval days_to_due=round((assessment_due_epoch-now())/86400,0)
| eval overdue_assessment=days_to_due < 0
| eval stale_incident=if(isnotnull(last_incident_epoch), (now()-last_incident_epoch)/86400 < 1 AND open_finding_count > 0, false())
| where overdue_assessment=1 OR open_finding_count > 5 OR (privileged_session_flag==1 AND open_finding_count > 0)
| table vendor_id, days_to_due, open_finding_count, privileged_session_flag, last_incident_epoch
```
- **Implementation:** Integrate GRC vendor scores and IAM logs for vendor jump-box sessions. Refresh `assessment_due_epoch` from contract metadata. Correlate with news OSINT for supply-chain breaches via optional lookup.
- **Visualization:** Table (vendor risk), Bar chart (findings by vendor), Status grid.
- **CIM Models:** N/A

---

### UC-21.11.9 · NIS2 Vulnerability Disclosure and Patch Management Tracking (Art. 21(2)(e))
- **Criticality:** 🟠 High
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Security, Compliance
- **Value:** Article 21(2)(e) covers handling of vulnerabilities and disclosure; SLAs on critical CVE remediation and coordinated disclosure timestamps demonstrate operational due diligence.
- **App/TA:** Splunk Enterprise Security, vulnerability management TA
- **Data Sources:** `index=compliance` `sourcetype="vuln:patch"` (asset_id, cve_id, severity, disclosed_epoch, patched_epoch, sla_days)
- **SPL:**
```spl
index=compliance sourcetype="vuln:patch" severity="critical"
| eval age_days=if(isnotnull(patched_epoch), (patched_epoch-disclosed_epoch)/86400, (now()-disclosed_epoch)/86400)
| eval sla_breach=age_days > sla_days OR (isnull(patched_epoch) AND age_days > sla_days)
| where sla_breach=1
| stats count as open_crit, max(age_days) as max_age by asset_id
| where open_crit > 0
| table asset_id, open_crit, max_age
```
- **Implementation:** Ingest VM scanner and patch tools with normalized CVE and `disclosed_epoch` from NVD sync. Set `sla_days` by tier (e.g., 7 for critical Internet-facing). Exclude decommissioned assets with a status field.
- **Visualization:** Histogram (age distribution), Table (worst assets), Single value (SLA breaches).
- **CIM Models:** N/A

---

### UC-21.11.10 · NIS2 Business Continuity and Crisis Management Monitoring (Art. 21(2)(c))
- **Criticality:** 🔴 Critical
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Availability, Compliance
- **Value:** Article 21(2)(c) expects business continuity and crisis management; monitoring RTO/RPO test outcomes and live failover events shows resilience in practice, not only on paper.
- **App/TA:** Splunk ITSI (optional), custom BCP dashboards
- **Data Sources:** `index=compliance` `sourcetype="continuity:status"` (service_id, test_type, planned_epoch, actual_rto_min, actual_rpo_min, target_rto_min, target_rpo_min, success_flag)
- **SPL:**
```spl
index=compliance sourcetype="continuity:status"
| where lower(success_flag)!="true" OR actual_rto_min > target_rto_min OR actual_rpo_min > target_rpo_min
| eval gap_rto=actual_rto_min - target_rto_min
| eval gap_rpo=actual_rpo_min - target_rpo_min
| table service_id, test_type, planned_epoch, actual_rto_min, target_rto_min, gap_rto, gap_rpo, success_flag
```
- **Implementation:** Ingest DR drill results from tabletop tools and live failover telemetry from orchestration. Align `service_id` with BIA tiers. Alert when annual test window is missed (separate saved search on last `planned_epoch`).
- **Visualization:** Bar chart (gaps), Timeline (tests), Table (failed services).
- **CIM Models:** N/A

---

### UC-21.11.11 · NIS2 Network and Information Systems Access Control Audit (Art. 21(2)(i))
- **Criticality:** 🟠 High
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Security, Audit
- **Value:** Article 21(2)(i) requires policies and procedures on access control; continuous review of privileged access, MFA gaps, and after-hours admin sessions supports incident readiness.
- **App/TA:** Splunk Enterprise Security
- **Data Sources:** `index=compliance` `sourcetype="access:audit"` (user_id, asset_id, action, mfa_status, session_start_epoch, privileged_flag)
- **SPL:**
```spl
index=compliance sourcetype="access:audit" privileged_flag=1
| eval hour=strftime(session_start_epoch,"%H")
| where mfa_status!="success" OR (hour < 6 OR hour > 22)
| bin _time span=1d
| stats count as sessions, dc(user_id) as users, dc(asset_id) as assets by _time, user_id
| where sessions > 10
| table _time, user_id, sessions, users, assets
```
- **Implementation:** Normalize AD, PAM, and cloud IAM logs into `access:audit`; map `privileged_flag` from group membership. Tune off-hours window per locale. Join to UEBA for impossible travel if available.
- **Visualization:** Heatmap (hour × user), Table (sessions), Bar chart (MFA failures).
- **CIM Models:** N/A

---

### UC-21.11.12 · DORA ICT Risk Management Dashboard (Art. 5-16)
- **Criticality:** 🟠 High
- **Difficulty:** 🟠 Advanced
- **Monitoring type:** Risk, Compliance
- **Value:** Articles 5–16 establish ICT risk management for financial entities; aggregating risk register updates, control testing, and residual scores gives boards a single operational view.
- **App/TA:** Splunk App for Compliance, custom executive dashboard
- **Data Sources:** `index=compliance` `sourcetype="ict:risk"` (entity_id, risk_id, inherent_score, residual_score, control_count, last_test_epoch)
- **SPL:**
```spl
index=compliance sourcetype="ict:risk"
| eval test_age_days=(now()-last_test_epoch)/86400
| where residual_score > 12 OR test_age_days > 365 OR control_count < 2
| stats avg(residual_score) as avg_res, max(residual_score) as max_res, count as risks by entity_id
| sort - max_res
| table entity_id, risks, avg_res, max_res
```
- **Implementation:** Ingest GRC exports or APIs with quarterly refresh; tie `risk_id` to ICT asset inventory. Use entity_id for group reporting. Drill to control tests via separate index if split.
- **Visualization:** Scatter (inherent vs residual), Bar chart (by entity), Single value (high risks).
- **CIM Models:** N/A

---

### UC-21.11.13 · DORA ICT Incident Classification and Reporting (Art. 17-23)
- **Criticality:** 🔴 Critical
- **Difficulty:** 🟠 Advanced
- **Monitoring type:** Security, Compliance
- **Value:** Articles 17–23 define classification and regulatory reporting of ICT-related incidents; aligning ES severity to DORA major/significant categories streamlines filings to authorities.
- **App/TA:** Splunk Enterprise Security, SOAR
- **Data Sources:** `index=compliance` `sourcetype="incident:dora"` (case_id, dora_class, opened_epoch, regulator_filed_epoch, impact_customers, downtime_min)
- **SPL:**
```spl
index=compliance sourcetype="incident:dora"
| eval major=impact_customers>50000 OR downtime_min>300
| eval filing_due_h=if(major==1, 4, 72)
| eval filed_lag_h=if(isnotnull(regulator_filed_epoch), (regulator_filed_epoch-opened_epoch)/3600, (now()-opened_epoch)/3600)
| where filed_lag_h > filing_due_h OR isnull(regulator_filed_epoch)
| table case_id, dora_class, major, filing_due_h, filed_lag_h, impact_customers
```
- **Implementation:** Map notables to `dora_class` via lookup from ES severity and business impact. SOAR records `regulator_filed_epoch` from filing portal ACK. Validate thresholds with legal (customer counts, downtime).
- **Visualization:** Timeline (classification), Table (late filings), Single value (open major).
- **CIM Models:** N/A

---

### UC-21.11.14 · DORA Digital Operational Resilience Testing (Art. 24-27)
- **Criticality:** 🟠 High
- **Difficulty:** 🟠 Advanced
- **Monitoring type:** Compliance, Performance
- **Value:** Articles 24–27 mandate threat-led penetration testing and resilience testing cycles; tracking test scope, remediation of findings, and retest dates proves the testing program is executed.
- **App/TA:** Splunk App for Compliance
- **Data Sources:** `index=compliance` `sourcetype="resilience:test"` (test_id, test_type, scope_system, start_epoch, end_epoch, finding_open_count, retest_due_epoch)
- **SPL:**
```spl
index=compliance sourcetype="resilience:test"
| eval retest_late=retest_due_epoch < now() AND finding_open_count > 0
| where finding_open_count > 0 OR retest_late=1
| eval days_to_retest=round((retest_due_epoch-now())/86400,0)
| table test_id, test_type, scope_system, finding_open_count, retest_due_epoch, days_to_retest, retest_late
```
- **Implementation:** Ingest red-team and TLPT reports from GRC; link findings to Jira with `retest_due_epoch`. Exclude cancelled tests with status field. Correlate scope to critical functions per DORA inventory.
- **Visualization:** Gantt-style timeline (tests), Bar chart (open findings), Table (overdue retest).
- **CIM Models:** N/A

---

### UC-21.11.15 · DORA Third-Party ICT Provider Concentration Risk (Art. 28-44)
- **Criticality:** 🔴 Critical
- **Difficulty:** 🟠 Advanced
- **Monitoring type:** Risk, Compliance
- **Value:** Articles 28–44 address third-party ICT risk and concentration; monitoring dependency ratios and substitutability metrics supports exit plans and contractual audit rights.
- **App/TA:** Splunk App for Compliance, vendor dashboards
- **Data Sources:** `index=compliance` `sourcetype="vendor:concentration"` (provider_id, critical_function_id, spend_pct, workload_pct, contract_end_epoch, exit_plan_exists)
- **SPL:**
```spl
index=compliance sourcetype="vendor:concentration"
| stats sum(workload_pct) as concentration, max(exit_plan_exists) as has_exit, values(critical_function_id) as functions by provider_id
| eval at_risk=concentration > 25 OR has_exit=0
| where at_risk=1
| table provider_id, concentration, has_exit, functions
```
- **Implementation:** Load contract and architecture CMDB extracts; `workload_pct` should sum to ~100 per function when scoped. Flag providers above board-approved thresholds. Refresh quarterly from procurement.
- **Visualization:** Treemap (provider share), Network diagram (dependencies), Table (top concentration).
- **CIM Models:** N/A

---

### UC-21.11.16 · DORA Cross-Region Disaster Recovery Compliance (Art. 11-12)
- **Criticality:** 🟠 High
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Availability, Compliance
- **Value:** Articles 11–12 address ICT systems, protocols, and resilience; demonstrating geographically separated recovery capabilities and replication lag supports exit-from-service and continuity expectations.
- **App/TA:** Splunk ITSI, infrastructure monitoring
- **Data Sources:** `index=compliance` `sourcetype="dr:compliance"` (app_id, primary_region, dr_region, replication_lag_sec, last_dr_test_epoch, rpo_target_sec)
- **SPL:**
```spl
index=compliance sourcetype="dr:compliance"
| eval lag_breach=replication_lag_sec > rpo_target_sec
| eval test_stale=(now()-last_dr_test_epoch)/86400 > 365
| where lag_breach=1 OR test_stale=1
| eval gap_sec=replication_lag_sec - rpo_target_sec
| table app_id, primary_region, dr_region, replication_lag_sec, rpo_target_sec, gap_sec, test_stale
```
- **Implementation:** Pull replication metrics from DB and storage APIs; align `rpo_target_sec` with BIA. Ensure `dr_region` meets regulatory separation rules via lookup. Pair with active-active health checks.
- **Visualization:** Line chart (lag trend), Table (breaches), Map (regions).
- **CIM Models:** N/A

---

### UC-21.11.17 · CCPA Consumer Data Access and Deletion Request Tracking (§1798.100-105)
- **Criticality:** 🟠 High
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Compliance, Performance
- **Value:** Civil Code §§1798.100–1798.105 require disclosure and deletion workflows with defined response periods; monitoring request queues proves operational compliance with statutory timelines.
- **App/TA:** Splunk App for Compliance
- **Data Sources:** `index=compliance` `sourcetype="privacy:request"` (request_id, type, state_resident, opened_epoch, due_epoch, completed_epoch, status)
- **SPL:**
```spl
index=compliance sourcetype="privacy:request" state_resident="CA"
| eval due=if(isnotnull(due_epoch), due_epoch, opened_epoch+1296000)
| eval done=if(match(lower(status),"complete|closed"), completed_epoch, null())
| eval late=if(isnotnull(done), done>due, now()>due)
| where late=1
| eval days=if(isnotnull(done), round((done-due)/86400,1), round((now()-due)/86400,1))
| table request_id, type, days, status
```
- **Implementation:** Ingest privacy portal and legal ops queues; default 45-day `due` from `opened_epoch` when `due_epoch` absent per §1798.130. Extension flags may add 45 days—model as `extension_epoch` if needed.
- **Visualization:** Single value (late count), Timeline, Table (requests).
- **CIM Models:** N/A

---

### UC-21.11.18 · CCPA Data Sale Opt-Out Enforcement Monitoring (§1798.120)
- **Criticality:** 🔴 Critical
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Compliance, Security
- **Value:** Section 1798.120 grants consumers the right to opt out of sale or sharing; detecting downstream data broker sends after opt-out timestamps shows systemic control failures.
- **App/TA:** Splunk App for Compliance, consent platform
- **Data Sources:** `index=compliance` `sourcetype="consent:optout"` (consumer_id_hash, optout_epoch, downstream_event_epoch, partner_id, sale_flag)
- **SPL:**
```spl
index=compliance sourcetype="consent:optout" sale_flag=1
| where downstream_event_epoch > optout_epoch
| stats count as violations, dc(partner_id) as partners, min(downstream_event_epoch) as first_violation by consumer_id_hash
| where violations >= 1
| table consumer_id_hash, violations, partners, first_violation
```
- **Implementation:** Join CMP opt-out feed with server-side forwarding logs to partners; hash `consumer_id_hash` consistently. Tag `sale_flag` per CPRA definitions (including cross-context behavioral ads if modeled as sale/share).
- **Visualization:** Bar chart (violations by partner), Table (consumers affected), Single value (violations).
- **CIM Models:** N/A

---

### UC-21.11.19 · CCPA Sensitive Personal Information Processing Audit (§1798.121)
- **Criticality:** 🟠 High
- **Difficulty:** 🟠 Advanced
- **Monitoring type:** Compliance, Audit
- **Value:** Section 1798.121 limits use of sensitive personal information to purposes disclosed and permitted; auditing access paths and purpose codes prevents impermissible secondary use.
- **App/TA:** Splunk App for Compliance, Edge Processor
- **Data Sources:** `index=compliance` `sourcetype="spi:processing"` (resource, purpose_code, spi_category, consumer_limit_applied_flag, event_epoch)
- **SPL:**
```spl
index=compliance sourcetype="spi:processing"
| where consumer_limit_applied_flag=0 AND match(spi_category,"(geo|health|finance|biometric)")
| bin _time span=1d
| stats count as events, dc(resource) as resources by purpose_code, spi_category, _time
| where events > 1000
| table _time, purpose_code, spi_category, events, resources
```
- **Implementation:** Classify application and data warehouse queries with SPI tags from data catalog; flag when `purpose_code` is not in the permitted list for that category. Coordinate with Limit the Use workflows.
- **Visualization:** Heatmap (purpose × category), Table (top misuse), Line chart (events).
- **CIM Models:** N/A

---

### UC-21.11.20 · MiFID II Trade and Transaction Reporting Completeness (Art. 26)
- **Criticality:** 🔴 Critical
- **Difficulty:** 🟠 Advanced
- **Monitoring type:** Compliance, Performance
- **Value:** Article 26 mandates complete and accurate transaction reporting to regulators; reconciling internal trade capture to outbound ARM submissions catches omissions that carry enforcement risk.
- **App/TA:** Splunk App for Compliance, trade surveillance
- **Data Sources:** `index=compliance` `sourcetype="trade:report"` (trade_id, venue, internal_ts, arm_ack_ts, report_status, reject_reason)
- **SPL:**
```spl
index=compliance sourcetype="trade:report"
| where lower(report_status)!="ack" OR isnotnull(reject_reason)
| eval lag_sec=if(isnotnull(arm_ack_ts), arm_ack_ts-internal_ts, now()-internal_ts)
| where lag_sec > 300 OR match(lower(report_status),"reject|error|fail")
| table trade_id, venue, report_status, reject_reason, lag_sec
```
- **Implementation:** Ingest OMS fills and ARM gateway ACK/NACK logs with shared `trade_id`. Alert on reject storms by `reject_reason`. Align clocks via NTP monitoring separately.
- **Visualization:** Line chart (reject rate), Table (stuck trades), Bar chart (by venue).
- **CIM Models:** N/A

---

### UC-21.11.21 · MiFID II Communications Recording and Retention Audit (Art. 16(7))
- **Criticality:** 🟠 High
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Compliance, Audit
- **Value:** Article 16(7) requires records of services and transactions, including electronic communications, to be kept; verifying capture completeness and retention windows supports firm and regulator exams.
- **App/TA:** Splunk App for Compliance, recording platform
- **Data Sources:** `index=compliance` `sourcetype="comms:recording"` (user_id, channel, session_id, record_start_epoch, record_end_epoch, retention_until_epoch, capture_ok_flag)
- **SPL:**
```spl
index=compliance sourcetype="comms:recording"
| where capture_ok_flag=0 OR isnull(record_end_epoch) OR retention_until_epoch < now()+7776000
| eval gap_sec=if(isnotnull(record_end_epoch), record_end_epoch-record_start_epoch, null())
| where isnull(gap_sec) OR gap_sec < 1
| stats count as failures by user_id, channel
| where failures > 3
| table user_id, channel, failures
```
- **Implementation:** Integrate turret, mobile recording, and Teams compliance APIs; map traders to `user_id`. Flag sessions with zero duration or missing end. Retention should exceed 5 years where MiFID applies—tune `retention_until_epoch` per jurisdiction.
- **Visualization:** Table (users at risk), Bar chart (failures by channel), Timeline.
- **CIM Models:** N/A

---

### UC-21.11.22 · MiFID II Best Execution Monitoring (Art. 27)
- **Criticality:** 🟠 High
- **Difficulty:** 🟠 Advanced
- **Monitoring type:** Performance, Compliance
- **Value:** Article 27 requires firms to take all sufficient steps to obtain best execution; comparing venue quality and slippage against policy benchmarks demonstrates ongoing monitoring of execution factors.
- **App/TA:** Splunk App for Compliance, market data TA
- **Data Sources:** `index=compliance` `sourcetype="execution:report"` (order_id, venue, arrival_price, executed_price, benchmark_price, policy_deviation_bps, instrument)
- **SPL:**
```spl
index=compliance sourcetype="execution:report"
| eval slippage_bps=abs((executed_price-arrival_price)/arrival_price)*10000
| eval bench_gap=abs(executed_price-benchmark_price)/benchmark_price*10000
| where policy_deviation_bps > 2 OR bench_gap > 5 OR slippage_bps > 10
| bin _time span=1h
| stats avg(policy_deviation_bps) as avg_dev, count as orders by venue, instrument, _time
| where avg_dev > 1.5
| table _time, venue, instrument, avg_dev, orders
```
- **Implementation:** Load TCA feeds and smart order router decisions; align `benchmark_price` with RTS 27/28 reporting where applicable. Segment by asset class; exclude illiquid names with a liquidity flag if present.
- **Visualization:** Scatter (slippage vs benchmark), Line chart (avg_dev), Table (outliers).
- **CIM Models:** N/A

---

### UC-21.11.23 · ISO 27001 Annex A Control Effectiveness Monitoring
- **Criticality:** 🟠 High
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Compliance, Audit
- **Value:** Annex A controls must be implemented and reviewed; tracking control test pass rates and open deficiencies by domain shows whether the ISMS is operating as designed between certification audits.
- **App/TA:** Splunk App for Compliance
- **Data Sources:** `index=compliance` `sourcetype="isms:control"` (control_id, domain, test_result, test_epoch, deficiency_open_flag)
- **SPL:**
```spl
index=compliance sourcetype="isms:control"
| bin _time span=1w
| stats count as tests, sum(eval(if(test_result=="fail" OR deficiency_open_flag=1,1,0))) as fails by domain, _time
| eval fail_rate=round(100*fails/tests,1)
| where fail_rate > 5 OR fails > 0
| table _time, domain, tests, fails, fail_rate
```
- **Implementation:** Import internal audit and continuous control testing results; map `control_id` to Annex A themes. Feed weekly for trending. Exclude planned exceptions with approved waiver IDs if modeled.
- **Visualization:** Line chart (fail rate), Stacked bar (domain), Table (failing controls).
- **CIM Models:** N/A

---

### UC-21.11.24 · ISO 27001 Information Security Event Log Review Compliance (A.12.4)
- **Criticality:** 🟡 Medium
- **Difficulty:** 🟢 Beginner
- **Monitoring type:** Compliance, Audit
- **Value:** Control A.12.4 expects logging and monitoring with review of security events; proving that high-priority queues were reviewed within SLA supports audit evidence for log review procedures.
- **App/TA:** Splunk Enterprise Security, ITSM
- **Data Sources:** `index=compliance` `sourcetype="logreview:compliance"` (queue_id, event_id, created_epoch, reviewed_epoch, reviewer_id, sla_hours)
- **SPL:**
```spl
index=compliance sourcetype="logreview:compliance"
| eval sla_sec=sla_hours*3600
| eval reviewed_late=if(isnotnull(reviewed_epoch), reviewed_epoch-created_epoch > sla_sec, now()-created_epoch > sla_sec)
| where reviewed_late=1
| stats count as backlog by queue_id
| where backlog > 0
| table queue_id, backlog
```
- **Implementation:** Ingest SOC ticketing when an analyst marks review complete; alternatively sample ES notable disposition timestamps. Set `sla_hours` by severity tier. Management attestation can be a separate sourcetype.
- **Visualization:** Single value (backlog), Bar chart (by queue), Table.
- **CIM Models:** N/A

---

### UC-21.11.25 · ISO 27001 Access Rights Review and Recertification (A.9.2.5)
- **Criticality:** 🟠 High
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Compliance, Security
- **Value:** Control A.9.2.5 requires regular review of user access rights; tracking overdue recertification campaigns and orphan privileged accounts reduces insider and stale-access risk.
- **App/TA:** Splunk Enterprise Security, IAM integrations
- **Data Sources:** `index=compliance` `sourcetype="access:recertification"` (campaign_id, account_id, owner_id, due_epoch, decision_epoch, decision)
- **SPL:**
```spl
index=compliance sourcetype="access:recertification"
| eval pending=if(isnull(decision_epoch) OR decision=="pending",1,0)
| where pending=1 AND due_epoch < now()
| stats count as overdue by campaign_id, owner_id
| where overdue > 0
| sort - overdue
| table campaign_id, owner_id, overdue
```
- **Implementation:** Export IGA recert tasks via API; map service accounts with `owner_id` for accountability. Auto-complete decisions from HR termination feed in a separate pipeline. Alert when `overdue` exceeds team capacity.
- **Visualization:** Bar chart (overdue by owner), Table (campaigns), Single value (total overdue).
- **CIM Models:** N/A

---

### UC-21.11.26 · NIST CSF Maturity Posture Dashboard (Identify/Protect/Detect/Respond/Recover)
- **Criticality:** 🟡 Medium
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Risk, Compliance
- **Value:** The CSF Core functions organize outcomes; rolling up tool coverage and process maturity scores by function shows leadership where investment lags policy (e.g., Identify vs Detect).
- **App/TA:** Splunk App for Compliance, ES posture
- **Data Sources:** `index=compliance` `sourcetype="csf:posture"` (function_name, subcategory, maturity_tier, evidence_count, last_assessed_epoch)
- **SPL:**
```spl
index=compliance sourcetype="csf:posture"
| stats avg(maturity_tier) as avg_tier, sum(evidence_count) as evidence by function_name
| eval gap=if(avg_tier < 3, 1, 0)
| sort function_name
| table function_name, avg_tier, evidence, gap
```
- **Implementation:** Load self-assessment or automated control evidence counts per CSF subcategory; refresh after major architecture changes. Normalize `maturity_tier` to 1–4 scale. Optional drill to MITRE mappings in separate dashboard.
- **Visualization:** Radar chart (functions), Bar chart (avg tier), Table.
- **CIM Models:** N/A

---

### UC-21.11.27 · NIST CSF Detect Function Coverage Gap Analysis
- **Criticality:** 🟠 High
- **Difficulty:** 🟠 Advanced
- **Monitoring type:** Security, Compliance
- **Value:** The Detect function (DE) expects anomalies and continuous monitoring; comparing expected log sources and detection rules to actual ingested volume exposes blind spots before adversaries do.
- **App/TA:** Splunk Enterprise Security
- **Data Sources:** `index=compliance` `sourcetype="csf:coverage"` (asset_class, expected_sourcetypes, observed_sourcetype, de_rule_count, ingest_mb_24h)
- **SPL:**
```spl
index=compliance sourcetype="csf:coverage"
| eval expected=mvindex(split(expected_sourcetypes,","),0)
| where isnull(observed_sourcetype) OR de_rule_count=0 OR ingest_mb_24h < 1
| stats values(asset_class) as assets, max(de_rule_count) as rules, sum(ingest_mb_24h) as ingest by observed_sourcetype
| fillnull value="MISSING" observed_sourcetype
| table observed_sourcetype, assets, rules, ingest
```
- **Implementation:** Build CMDB-driven expected sourcetype list per asset class; compare to `_internal` or metadata index for actual ingestion. Join ES correlation search counts per data model. Exclude dev stacks by index filter.
- **Visualization:** Table (gaps), Bar chart (missing by class), Matrix heatmap.
- **CIM Models:** N/A

---

### UC-21.11.28 · SOC 2 Trust Services Criteria Continuous Control Monitoring (CC6-CC8)
- **Criticality:** 🟠 High
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Compliance, Audit
- **Value:** CC6–CC8 address logical access, system operations, and change management; continuous evidence of control operation reduces year-end sampling gaps for SOC 2 examinations.
- **App/TA:** Splunk App for Compliance
- **Data Sources:** `index=compliance` `sourcetype="tsc:control"` (control_ref, cc_family, evidence_event_count, exception_count, period_start_epoch, period_end_epoch)
- **SPL:**
```spl
index=compliance sourcetype="tsc:control" cc_family="CC6" OR cc_family="CC7" OR cc_family="CC8"
| stats sum(evidence_event_count) as ev, sum(exception_count) as ex by control_ref, cc_family
| eval ex_rate=if(ev+ex>0, round(100*ex/(ev+ex),2), null())
| where ex > 0 AND ex_rate > 1
| sort - ex_rate
| table control_ref, cc_family, ev, ex, ex_rate
```
- **Implementation:** Map automated evidence (e.g., privileged login reviews, change tickets) to TSC references used by auditors. Tag `cc_family` consistently with AICPA mapping. Weekly rollup for executive sign-off packs.
- **Visualization:** Bar chart (exception rate), Table (controls), Line chart (trend).
- **CIM Models:** N/A

---

### UC-21.11.29 · SOC 2 System Availability and Incident Response Evidence Collection (A1)
- **Criticality:** 🔴 Critical
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Availability, Compliance
- **Value:** Availability criterion A1 expects commitments on system availability; capturing uptime, incident timelines, and customer-facing impact supports SOC 2 and customer SLA reporting.
- **App/TA:** Splunk ITSI, incident management
- **Data Sources:** `index=compliance` `sourcetype="availability:evidence"` (service_name, window_start_epoch, window_end_epoch, uptime_pct, incident_id, customer_impact_min)
- **SPL:**
```spl
index=compliance sourcetype="availability:evidence"
| eval breach=uptime_pct < 99.9 OR customer_impact_min > 15
| where breach=1
| stats avg(uptime_pct) as avg_up, sum(customer_impact_min) as impact by service_name
| sort avg_up
| table service_name, avg_up, impact
```
- **Implementation:** Aggregate synthetic or APM uptime with maintenance windows excluded via tag. Join P1/P2 incidents from ITSM for `customer_impact_min`. Align fiscal month boundaries with finance for rebate calculations if applicable.
- **Visualization:** Line chart (uptime), Bar chart (impact minutes), SLA gauge.
- **CIM Models:** N/A

---

### UC-21.11.30 · SOC 2 Confidentiality Classification and DLP Event Audit (C1)
- **Criticality:** 🟠 High
- **Difficulty:** 🔵 Intermediate
- **Monitoring type:** Security, Compliance
- **Value:** Confidentiality criterion C1 addresses protection of confidential information; auditing DLP policy hits against classification labels shows controls are applied to data commensurate with commitments.
- **App/TA:** Splunk Enterprise Security, DLP connectors
- **Data Sources:** `index=compliance` `sourcetype="dlp:event"` (doc_label, policy_name, action, user_id, channel, blocked_flag)
- **SPL:**
```spl
index=compliance sourcetype="dlp:event"
| where match(doc_label,"(?i)public|internal") AND blocked_flag=0 AND match(lower(action),"allow|permit")
| bin _time span=1d
| stats count as events, dc(user_id) as users by doc_label, policy_name, _time
| where events > 50
| table _time, doc_label, policy_name, events, users
```
- **Implementation:** Normalize CASB, endpoint DLP, and email gateway events; ensure `doc_label` comes from classification tools. Tune to catch high-volume “allow” of sensitive labels to external domains. Redact filenames in display fields.
- **Visualization:** Heatmap (label × policy), Bar chart (events), Table (users).
- **CIM Models:** N/A
