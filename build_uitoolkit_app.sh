...
# Run from repo root. Requires: Node 18+, yarn or npm, python3

set -e
REPO_ROOT="$(cd "$(dirname "$0")" && pwd)"
UITOOLKIT="$REPO_ROOT/imucl_uitoolkit"
...
COPYFILE_DISABLE=1 tar --format ustar -cvzf "$SPL_NAME" -C "$TAR_DIR" imucl
rm -rf "$TAR_DIR"

echo "Done. Created: $REPO_ROOT/$SPL_NAME"
