...
| Grandma explanation | **Grandma explanation:** | Optional, v7.1+. Per-UC plain-language "explain it to my grandma" summary in `we` voice: 20–400 chars, no Splunk / SPL / CIM / MITRE / TA / add-on acronyms, no index / sourcetype / tstats, no regulation clause numbers. Authored in the JSON sidecar as `grandmaExplanation` (the markdown-line form is optional). Rendered as the primary UC text across the non-technical view (UC cards, search results, subcategory lists, recently-added, and the top of the UC detail panel). Populated by `python -m splunk_uc generate-grandma-explanations` (legacy `python3 -m splunk_uc generate-grandma-explanations` shim still works during soak) from the existing title/description/value copy; CI enforces that every UC has it via `python -m splunk_uc generate-grandma-explanations --check`. See [docs/grandma-explanations.md](grandma-explanations.md) for authoring and tone rules. |
...
| CIM Models | **CIM Models:** | Splunk CIM<sup class="ref">[<a href="#ref-3">3</a>]</sup> data model names the use case relies on (comma-separated, e.g. `Performance`, `Network_Traffic`, `Change`). See [CIM and data models](cim-and-data-models.md). |
...
| MITRE ATT&CK<sup class="ref">[<a href="#ref-1">1</a>]</sup> | **MITRE ATT&CK:** | Technique IDs, comma-separated (e.g. T1562.008, T1110.003). Shown as links in the dashboard. |
...
## Cost impact (optional, v1.8+)

Structured metadata for ranking or filtering UCs by Splunk licensing impact. All sub-fields are optional; **`cost.tier`** is the primary coverage key tracked by `python -m splunk_uc audit-cost-coverage`. Values are hand-authored (no generator for v1). Per-region pricing and FX rates are intentionally out of scope.

| Field | JSON key | Description |
|-------|----------|-------------|
| Tier | `cost.tier` | Licensing-impact band: `low`, `medium`, `high`, or `extreme`. Primary sort/filter key for recommender and MCP consumers (F-2/F-3). |
| Ingest (GB/day) | `cost.ingest_gb_per_day` | Worst-case indexed volume estimate at reference scale. Machine-only; omit when unknown. |
| Search load | `cost.search_load` | Compute intensity: `light`, `moderate`, `heavy`, or `real-time`. |
| Search load notes | `cost.search_load_notes` | Plain-English nuance for search load (max 400 chars). |
| tstats eligible | `cost.tstats_eligible` | `true` when the primary SPL can run via accelerated CIM tstats. |
| Storage class | `cost.storage_class` | Expected retention tier: `hot`, `warm`, `cold`, or `frozen`. |
| Retention (days) | `cost.retention_days` | Expected retention horizon for underlying data. |
| Sources | `cost.sources` | Labels for cost drivers (e.g. HEC volume, DMA acceleration). |
| Last estimated | `cost.last_estimated` | ISO date when the profile was last reviewed. |
| Estimated by | `cost.estimated_by` | Provenance: `maintainer`, `ai-advisory`, `vendor`, or `customer-report`. |

### Example (in a UC sidecar)
...
"cost": {
  ...
  "search_load_notes": "Hourly scheduled stats over one index; no real-time correlation.",
  "tstats_eligible": true,
  "storage_class": "warm",
  "retention_days": 90,
  "sources": ["index volume", "scheduled search"],
...
<a id="ref-6"></a>**[6]** docs.splunk.com. *DMA docs*. Retrieved May 11, 2026, from https://docs.splunk.com/Documentation/Splunk/latest/Knowledge/Acceleratedatamodels

<a id="ref-7"></a>**[7]** schema.ocsf.io. *OCSF*. Retrieved May 11, 2026, from https://schema.ocsf.io/

<a id="ref-8"></a>**[8]** github.com. *Splunk Security Essentials*. Retrieved May 11, 2026, from https://github.com/splunk/security_content
