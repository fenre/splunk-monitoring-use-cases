...
| Grandma explanation | **Grandma explanation:** | Optional, v7.1+. Per-UC plain-language "explain it to my grandma" summary in `we` voice: 20–400 chars, no Splunk / SPL / CIM / MITRE / TA / add-on acronyms, no index / sourcetype / tstats, no regulation clause numbers. Authored in the JSON sidecar as `grandmaExplanation` (the markdown-line form is optional). Rendered as the primary UC text across the non-technical view (UC cards, search results, subcategory lists, recently-added, and the top of the UC detail panel). Populated by `python -m splunk_uc generate-grandma-explanations` (legacy `scripts/generate_grandma_explanations.py` shim still works during soak) from the existing title/description/value copy; CI enforces that every UC has it via `python -m splunk_uc generate-grandma-explanations --check`. See [docs/grandma-explanations.md](grandma-explanations.md) for authoring and tone rules. |
