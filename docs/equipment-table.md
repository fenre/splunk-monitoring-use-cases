...
3. Regenerate the sidecars: **`python3 scripts/generate_equipment_tags.py`**. Review the diff; the generator only touches the two equipment arrays on each sidecar so `git diff --stat content/` stays scoped.
