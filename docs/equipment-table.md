...
| `/api/v1/equipment/app-index.json` | Curated Splunkbase<sup class="ref">[<a href="#ref-8">8</a>]</sup> add-ons + DSA ingest ids per equipment slug (~127 entries). See [Equipment App Map](equipment-app-map.md). |
...
6. Regenerate the equipment app map when Splunkbase/DSA mappings change:
   **`python3 -m splunk_uc generate-equipment-app-map`**. See
   [Equipment App Map](equipment-app-map.md).
7. Run the audit: **`python3 -m splunk_uc audit-compliance-mappings`** (validates schema, regulations, and the `equipment-orphan` lint).
