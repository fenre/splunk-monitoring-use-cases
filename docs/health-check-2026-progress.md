...
| F14 | M | `api/v1/_evidence-packs-bak/`, `_draft_uc_*`, `_fix_*` clutter | **DONE** (2026-05-13, reclassified) | `api/v1/_evidence-packs-bak/` gone ✓. The 17 surviving `scripts/_*.py` files (5 `_catalog_*`, 7 `_meraki_*`, `_draft_uc_18_1_15`, `_fix_broken_fixture_refs`, `_patch_catalog_guide_fields`, `_regulation_wisdom`, `_wire_batch7`) are not clutter but content-burndown one-shots formally ratified by the v8.2.0 CHANGELOG narrative ("What stays in `scripts/`" §Deliberate, "Deliberately **not** migrated (documented exemption)" §Migration). The `_` prefix is the convention that marks them as one-shots; they live alongside the tier-uplift / content-fix family (`uplift_*.py`, `assurance_gap_fix.py`, `fix_cim_dataset_hallucinations.py`, `rewrite_meraki_*.py`, `regen_di_for_ucs.py`, `enrich_di_gold*.py`, `uc_quality_fix.py`) and are deleted *wholesale* at the end of their respective content burndowns, not piecemeal. F14 is therefore closed-by-decision: the original "clutter" was `_evidence-packs-bak/` and that is gone; the surviving `scripts/_*.py` files are a recognised category, not a finding. |
