...
| **P14** Content stewardship | PARTIAL | **Per-category CODEOWNERS routing scaffolded 2026-05-13** — `.github/CODEOWNERS` now carries one `/content/cat-NN-<slug>/` row per category (all 23), with a new structural test (`tests/build/test_codeowners.py`, 6 cases) that locks the invariant so the file cannot silently drift back to a single catch-all. Until co-maintainers join the project, every row points at the lead maintainer; the *structure* is in place, so swapping in a domain owner is a one-line change. **Still open**: per-category scorecards (the second half of P14). |
