...
1. **Monday cluster, then Tuesday backstop.** The three weekly
   maintenance probes (`link-check`, `codeql`, `regulatory-watch`)
   fire on Monday between 06:00 and 09:00 UTC so a maintainer's first
   triage window of the week sees the whole batch. `gitleaks` and
   `splunkbase-sync` are staggered into Tuesday so the triage backlog
   never doubles up.
...
| `actions/upload-artifact`                           | `043fb46d1a93c77aae656e7c1c64a875d1fc6a0a` | `v7.0.1` | `validate`, `pages`, `release`, `link-check`, `regulatory-watch`, `splunkbase-sync`, `build-reproducibility`, `uc-tests` |
