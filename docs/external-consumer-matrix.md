...
| 8 | **`data/equipment-app-map.json` + `/api/v1/equipment/app-index.json`** | [`docs/equipment-app-map.md`](equipment-app-map.md), [`schemas/equipment-app-map.schema.json`](../schemas/equipment-app-map.schema.json) | ➕ Additive-only within v1 | Equipment picker phases — expanding slug coverage, adding Splunkbase/DSA refs | Removing a slug entry, narrowing `apps[]` without deprecation | Additive slug/app entries ship freely; removals follow the `/api/v1/` deprecation policy. Gated by `audit-equipment-app-map --check`. |
