...
| **Channels** (Pages, jsDelivr, Cloudflare, npm, PyPI, Splunkbase<sup class="ref">[<a href="#ref-7">7</a>]</sup>, MCP) | Read-only consumers of `dist/` | n/a |
...
* `data/` holds cross-UC data: regulations, crosswalks (ATT&CK<sup class="ref">[<a href="#ref-2">2</a>]</sup>, D3FEND<sup class="ref">[<a href="#ref-3">3</a>]</sup>, OSCAL, OLIR),
...
This document is the **locked v7.0.0 contract** for the catalogue's
current architecture. Forward-looking architectural work lives in
two places, not in a separate `architecture-2027.md` placeholder:

* **[`ROADMAP.md`](../ROADMAP.md)** — release-aligned forward plan
  (current target, in-progress, backlog), kept in sync with `VERSION`
  by the `audit-roadmap-consistency` audit.
* **[`docs/adr/`](adr/)** — Architecture Decision Records numbered
  by acceptance. The cadence is healthy: ADR-0010, ADR-0011, and
  ADR-0012 all landed 2026-05-13 (sample-regime split, schema-lineage
  governance, sample-data canonical shape). New ADR numbers are
  assigned at authorship time — never reserved in advance — so that
  the lineage stays true to the order in which decisions were
  actually made (see [`ADR-0011 §"Alternatives considered"`](adr/0011-schema-lineage-governance.md)
  point C for why placeholder reservation is rejected).

The Repo Health Check tracker at
[`docs/health-check-2026-progress.md`](health-check-2026-progress.md)
is the active "what's next" backlog the project executes from. When
it lands a deliverable, it folds its narrative back into either an
ADR (one-decision-per-doc), a section update inside this document (a
locked-contract revision), or a `[Unreleased]` `CHANGELOG.md` entry
that becomes part of the next minor release.
