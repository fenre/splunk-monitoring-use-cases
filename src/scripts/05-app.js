...
function _invAppIndexURL() {
  var rel = (typeof window.EQUIPMENT_APP_INDEX_URL === 'string' && window.EQUIPMENT_APP_INDEX_URL)
    ? window.EQUIPMENT_APP_INDEX_URL.trim()
    : 'api/v1/equipment/app-index.json';
  if (rel.charAt(0) === '/') return rel;
  var base = (typeof window.__SITE_BASE_PATH === 'string' && window.__SITE_BASE_PATH)
    ? window.__SITE_BASE_PATH.replace(/\/+$/, '')
    : '';
