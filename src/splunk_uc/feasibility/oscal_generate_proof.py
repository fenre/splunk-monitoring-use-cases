...
scripts/feasibility/oscal_generate_proof.py to
src/splunk_uc/feasibility/oscal_generate_proof.py. parents[3] resolves:
oscal_generate_proof.py -> feasibility/ -> splunk_uc/ -> src/ -> repo
...
Takes ``content/cat-22-regulatory-compliance/UC-22.35.1.json`` (the draft UC
exemplar in the JSON SSOT) and emits a minimal NIST OSCAL v1.1.1 Component
Definition, then hands the output off to ``scripts/feasibility/oscal_validate.mjs``
(Node + Ajv) for schema validation.
...
Run:
  python -m splunk_uc feasibility-oscal-generate-proof
"""
...
def iso_timestamp() -> str:
    return dt.datetime(2026, 4, 16, 0, 0, 0, tzinfo=dt.UTC).isoformat(timespec="seconds")
...
def build_implemented_requirements(
    uc_full_id: str, compliance: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    ...
    for entry in compliance:
        ...
        req: dict[str, Any] = {
            ...
            "props": [
                {
                    "name": "assurance",
                    "ns": "https://fenre.github.io/splunk-monitoring-use-cases/ns/compliance",
                    "value": entry["assurance"],
                },
                {
                    "name": "mode",
                    "ns": "https://fenre.github.io/splunk-monitoring-use-cases/ns/compliance",
...
def build_component_definition(uc: dict[str, Any]) -> dict[str, Any]:
    uc_id = uc["id"]
        ...
        "control-implementations": [
            {
                ...
                "description": (
                    ...
                    f"at content/cat-22-regulatory-compliance/UC-{uc_id}.json."
                ),
    ...
    back_matter: dict[str, Any] | None = {"resources": resources} if resources else None
...
def main(argv: list[str] | None = None) -> int:
    """Dispatcher entry-point. ``argv`` accepted for the registry contract; the OSCAL proof takes no flags."""
    del argv
