...
scripts/feasibility/olir_ingest_proof.py to
src/splunk_uc/feasibility/olir_ingest_proof.py. parents[3] resolves:
olir_ingest_proof.py -> feasibility/ -> splunk_uc/ -> src/ -> repo
root. The legacy ``parents[2]`` chain assumed depth two and is now
wrong by one level. The legacy shim re-exports ``main`` and ``run``
so any direct CLI invocation in maintainer notes still works during
the soak period.
...
Run:
  python -m splunk_uc feasibility-olir-ingest-proof
"""
...
def fetch(url: str, dest: pathlib.Path) -> dict[str, Any]:
    """Download ``url`` to ``dest`` unless cached; return provenance record.

    HTTPS-only is enforced at the call site (every URL in ``SOURCES``
    above starts with ``https://``); bandit's S310 warning about
    untrusted ``urlopen`` is therefore a false positive here.
    """
    if not url.startswith("https://"):
        raise ValueError(f"feasibility ingest URLs must be HTTPS: {url!r}")
...
def flatten_controls(group: dict[str, Any], acc: list[dict[str, Any]], path: list[str]) -> None:
...
def normalise_oscal_catalog(source_id: str, catalog_json: dict[str, Any]) -> dict[str, Any]:
...
def run() -> int:
    ...
    for source in SOURCES:
        ...
        link_rels: dict[str, int] = {}
        for c in normalised["controls"]:
            for link in c["links"]:
                link_rels[link["rel"]] = link_rels.get(link["rel"], 0) + 1
        if link_rels:
            top = ", ".join(
                f"{rel}={n}" for rel, n in sorted(link_rels.items(), key=lambda x: -x[1])[:4]
            )
...
def main(argv: list[str] | None = None) -> int:
    """Dispatcher entry-point. ``argv`` accepted for the registry contract; the OLIR proof takes no flags."""
    del argv
    return run()
