...
def _render_phase23_section(ucs: list[dict[str, Any]], subcat_titles: dict[str, str]) -> str:
    parts: list[str] = [
        FENCE_BEGIN,
        "",
        "<!--",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if fence_re.search(md_text):
        new_md = (
            fence_re.sub(lambda _m: section, md_text, count=1)
            if section
            else fence_re.sub("", md_text, count=1)
        )
    elif section:
    ...
    if args.check and changed:
        ...
        return 1
    print(
        f"Phase 2.3 generator: {len(changed)} file(s) "
