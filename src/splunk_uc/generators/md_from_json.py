...
def _format_detailed_implementation(text: str) -> str:
    ...
    for line in lines:
        ...
        # Match em dash (U+2014), en dash (U+2013), or hyphen-minus.
        m = re.match(r"^(Step\s+\d+)\s*[—–-]\s*(.+)", stripped)  # noqa: RUF001
        if m:
...
def render_md(uc: dict[str, Any], json_filename: str) -> str:
    ...
    if monitoring:
        meta_parts.append(f"**Type:** {', '.join(monitoring)}")
    if wave:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate .md files from UC JSON source-of-truth.")
    parser.add_argument(
        "--check",
        action="store_true",
