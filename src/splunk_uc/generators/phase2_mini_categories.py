...
def _dump_sidecar(obj: dict[str, Any]) -> str:
...
def _render_markdown_block(uc: dict[str, Any]) -> str:
...
def _group_by_subcat(ucs: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    ...
    return dict(
        sorted(grouped.items(), key=lambda kv: (int(kv[0].split(".")[0]), int(kv[0].split(".")[1])))
    )


def _render_phase22_section(ucs: list[dict[str, Any]], subcat_titles: dict[str, str]) -> str:
    parts: list[str] = [
        FENCE_BEGIN,
        "",
        "<!--",
...
def _backfill_sidecar_cim(
    sidecar_path: Path, cim_list: list[str], check: bool, changed: list[Path]
) -> None:
...
def _new_sidecar_from_uc(uc: dict[str, Any], existing_path: Path | None = None) -> dict[str, Any]:
    side: dict[str, Any] = {"$schema": "../../schemas/uc.schema.json"}
    side.update(uc)
    ...
    if existing_path is not None and existing_path.exists():
        ...
        derived_carry_over: list[dict[str, Any]] = [
            e
            for e in existing.get("compliance", []) or []
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--check", action="store_true", help="Do not write any files; exit 1 on drift.")
    args = ap.parse_args(argv)

    data = json.loads(DATA_FILE.read_text(encoding="utf-8"))
    cim_map: dict[str, list[str]] = data.get("cim_models_backfill", {})
    new_ucs: list[dict[str, Any]] = data.get("new_ucs", [])
    ...
    if fence_re.search(new_md):
        ...
        new_md = (
            fence_re.sub(lambda _m: section, new_md, count=1)
            if section
            else fence_re.sub("", new_md, count=1)
        )
    elif section:
    ...
    if args.check and changed:
        ...
        return 1
    print(
        f"Phase 2.2 generator: {len(changed)} file(s) "
