...
Usage
-----
    python3 -m splunk_uc generate-phase3-3-derivatives            # write
    python3 -m splunk_uc generate-phase3-3-derivatives --check    # drift
...
class FrameworkIndex:
    ...
    def resolve_id(self, regulation: str) -> str | None:
        return self._alias.get(regulation.strip().lower())

    def framework(self, framework_id: str) -> dict[str, Any] | None:
    ...
    def first_version(self, framework_id: str) -> str | None:
    ...
    def __init__(self, index: FrameworkIndex) -> None:
        ...
        for deriv_id, spec in sorted(index.derives_from.items()):
            ...
            if not parent_id or not parent_version:
                raise SystemExit(
                    f"regulations.json: derivesFrom[{deriv_id!r}] missing parent or parentVersion"
                )
            ...
            self._by_parent.setdefault((parent_id, parent_version), []).append(
                {
...
def _build_inherited_entry(
    ...
    if not isinstance(parent_assurance, str):
        return None
    ...
    if target["inheritance_mode"] == "identity":
        ...
        rationale += (
            "Clause mapped per data/regulations.json derivesFrom graph; "
            "inherited assurance degraded one step."
        )
    divergence_note = target.get("divergence_note")
    if divergence_note:
        rationale += " Divergence recorded — SME review should confirm scope fit."
...
def _rewrite_sidecar(
    ...
    * ``derived_count`` -- Number of inherited entries the generator
    ...
    native_keys = {_entry_key(e) for e in native if isinstance(e, dict)}
