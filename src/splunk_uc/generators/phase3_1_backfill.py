...
Usage
-----
    python3 -m splunk_uc generate-phase3-1-backfill            # write changes
    python3 -m splunk_uc generate-phase3-1-backfill --check    # drift-detect
...
def _process(check_only: bool) -> int:
    ...
    for uc_id in sorted(per_uc, key=lambda s: [int(p) if p.isdigit() else p for p in s.split(".")]):
    ...
    if missing:
        ...
        for uc_id in missing:
            print(f"  - {uc_id} ({_sidecar_path(uc_id).relative_to(REPO_ROOT)})", file=sys.stderr)
        return 2
    ...
    if check_only:
        if drift:
            ...
            return 1
        print(f"Phase 3.1 backfill: OK ({len(mappings)} mappings, {len(per_uc)} UCs, no drift).")
        return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Drift-detection mode. Exits non-zero if any tracked sidecar "
