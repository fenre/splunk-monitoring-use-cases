...
class LedgerInput:
    ...
    mode: str
    assurance: str
    derivation_source: dict[str, Any] | None
...
# ----------------------------------------------------------------------
def load_signoffs() -> dict[str, list[dict[str, Any]]]:
    """Load peer / legal / sme signoff arrays. Missing files -> empty list."""
    out: dict[str, list[dict[str, Any]]] = {"peer": [], "legal": [], "sme": []}
...
def signoff_status_for(
    entry: LedgerInput,
    signoffs: dict[str, list[dict[str, Any]]],
...
def build_ledger_inputs(
    ...
    for sidecar_path in iter_uc_sidecars():
        ...
        for c in compliance:
            ...
            if not regulation_name:
                continue
            try:
                regulation_id = resolve_regulation_id(regulation_name, regulation_index)
    ...
    if unresolved:
        print("FATAL: unresolved regulation names in UC sidecars:", file=sys.stderr)
...
def build_ledger() -> dict[str, Any]:
    ...
    # across *all* sidecars instead of O(#commits x #sidecars)).
    unique_paths = sorted({e.uc_path for e in inputs})
    _populate_git_caches_bulk(unique_paths)

    records = [build_ledger_entry(e, signoffs, baselines, head_commit) for e in inputs]
    ...
    if args.check:
        ...
        if _structural_diff(current, new_body):
            print(
                ...
                f"Run `python -m splunk_uc generate-mapping-ledger` (without --check) "
