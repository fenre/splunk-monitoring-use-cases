...
* ``equipment/index.json``              - equipment slug -> UC ids + regulation
                                          ...
                                          ``generate-equipment-tags`` verb
                                          (``python -m splunk_uc generate-equipment-tags``).
...
# ``equipment_lib`` still lives under ``scripts/`` until P6 Tier 2 of the
# scripts-taxonomy migration ports it into the package as well.  Insert
# ``scripts/`` on ``sys.path`` so the sibling import keeps working from
# inside the package.  The insertion is idempotent.  We use it only to
# look up display labels for the equipment/index.json facade — the
# authoritative per-UC equipment tags come from the sidecars (compliance
...
def _uc_compact(
    uc: Mapping[str, Any],
    alias_to_id: Mapping[str, str] | None = None,
) -> dict[str, Any]:
...
def _regulations_index(regs: Mapping[str, Any]) -> dict[str, Any]:
    frameworks = []
    for fw in regs.get("frameworks", []):
        versions = sorted({v.get("version") for v in fw.get("versions", []) if v.get("version")})
        frameworks.append(
            {
                "id": fw.get("id"),
...
def _oscal_payloads() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    index_payload = {
        ...
        "endpoints": {
            ...
            "componentDefinitions": (f"/api/{API_VERSION}/oscal/component-definitions/index.json"),
        },
    }
...
def _canonicalise_cim(name: str) -> str | None:
    if not name:
        return None
    root = name.split(".", 1)[0]
...
def _recommender_uc_thin(
    ...
    uc_id = str(uc.get("i", ""))
    # ``sidecar_sb_map`` originates in JSON; runtime entries may be any
    # type, so type the iterable as ``Sequence[Any]`` and rely on the
    # isinstance(Mapping) guard below to filter junk.
    sb_raw: Sequence[Any] = ()
    ...
    return {
        ...
        "monitoringType": sorted([m for m in (uc.get("mtype", []) or []) if isinstance(m, str)]),
        "splunkPillar": str(uc.get("pillar", "") or ""),
        "app": _recommender_apps(uc),
        "cimModels": _recommender_cim_models(uc),
        "mitreAttack": sorted([m for m in (uc.get("mitre", []) or []) if isinstance(m, str)]),
        "equipment": sorted([e for e in (uc.get("e", []) or []) if isinstance(e, str)]),
        "equipmentModels": sorted([em for em in (uc.get("em", []) or []) if isinstance(em, str)]),
        "sb": sb_compact,
    }
...
def _recommender_payloads(
    ...
    return {
        "sourcetype-index": {
            ...
            "ucCount": len({uid for ids in sourcetype_bucket.values() for uid in ids}),
    ...
    * ``vendor`` — publisher (Splunk, Cisco, etc.)
    ...
    body: dict[str, Any] = {
        ...
        "apps": {
            ...
            for sb_id, entry in sorted(merged.items(), key=lambda kv: int(kv[0]))
        },
    ...
    body["etag"] = '"' + hashlib.sha256(canonical.encode("utf-8")).hexdigest() + '"'
    return body
...
def _manifest(
    ...
    return {
        ...
        "endpoints": {
            ...
            "recommender": {
                "sourcetypeIndex": (f"/api/{API_VERSION}/recommender/sourcetype-index.json"),
                ...
                "splunkbaseIndex": (f"/api/{API_VERSION}/recommender/splunkbase-index.json"),
            },
            "equipment": {
...
def _render(out_root: pathlib.Path) -> None:
    ...
    for fw in regs.get("frameworks", []):
        ...
        for version in detail.get("versions", []):
            ...
            _write_json(
                out_root / "compliance" / "regulations" / f"{detail['id']}@{slug}.json",
                version_payload,
            )
    ...
    for uc in ucs:
        ...
        if isinstance(sb_field, list):
            sidecar_sb_map[uc_id] = [dict(e) for e in sb_field if isinstance(e, Mapping)]
...
def _render_story_surfaces(out_root: pathlib.Path) -> None:
    ...
    def _load_legacy_module(name: str) -> Any:
        # ``augment_regulation_api`` still lives under ``scripts/`` (Tier-2
        # generator migration is incremental). Resolve it against the
        # legacy directory rather than this file's package directory.
        path = _LEGACY_SCRIPTS_DIR / f"{name}.py"
    ...
    # ``clause_index`` and ``story_payload`` migrated to the package in
    # Tier 2 batch 4 (2026-05-09). Import them as proper sibling modules.
    # Imported lazily inside the function (matches the legacy lazy-import
    # contract) so importing ``api_surface`` does not eagerly drag in
    # those generators when only the ``main()`` entry-point is needed.
    from splunk_uc.generators import clause_index as clause_mod
    from splunk_uc.generators import story_payload as story_mod

    augment_mod = _load_legacy_module("augment_regulation_api")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.check:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if not args.out.exists():
                print(
                    f"ERROR: {args.out} does not exist — run without --check first.",
                    file=sys.stderr,
                )
                return 1
            ...
            if diffs:
                print(
                    "API surface is stale. Run `python -m splunk_uc generate-api-surface`.",
                    file=sys.stderr,
                )
                for line in diffs:
    ...
    if args.out.exists():
        ...
        # (e.g. api/v1/evidence-packs/ from
        # ``python -m splunk_uc generate-evidence-packs``) is left intact;
        # the corresponding generator is responsible for its own drift
        # check.
