...
def load_regulations() -> dict[str, Any]:
    payload = _load_json(REGULATIONS_PATH)
    if not isinstance(payload, dict):
        raise SystemExit(f"{REGULATIONS_PATH}: top-level payload is not a JSON object")
    return payload
...
def _compare_assurance(a: str | None, b: str | None) -> str:
    ...
    for uc in ucs:
        ...
        for entry in compliance:
            ...
            if not (
                ...
                continue
            key = (_slugify_regulation(reg), ver, clause)
            buckets[key].append(
                {
                    "ucId": uc["id"],
...
def _top_assurance(entries: Iterable[Mapping[str, Any]]) -> str | None:
    best: str | None = None
    for e in entries:
...
def _coverage_state(entries: Sequence[Mapping[str, Any]]) -> str:
    ...
    if not entries:
        return "uncovered"
    if any(e.get("assurance") == "full" and e.get("mode") == "satisfies" for e in entries):
        return "covered-full"
    if any(e.get("assurance") == "partial" and e.get("mode") == "satisfies" for e in entries):
...
def build_clause_detail(
    ...
    version: str,
    clause: str,
    bucket_entries: Sequence[Mapping[str, Any]],
    reg_entry: Mapping[str, Any] | None,
    generated_at: str,
    api_version: str,
) -> dict[str, Any]:
    ...
    """
    reg_id_raw = reg_entry.get("regulationId") if reg_entry else reg_slug
    reg_id = reg_id_raw if isinstance(reg_id_raw, str) else reg_slug
...
def build_index(
    ...
    return {
        ...
        "tierCoverageStateCounts": {str(k): dict(v) for k, v in sorted(tier_counts.items())},
...
def generate(
    ...
    *,
    ...
    regulations: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
