...
def build_digest(
    *,
    ...
    generated_at = (
        _dt.datetime.combine(reference_date, _dt.time(0, 0, 0), tzinfo=_dt.UTC)
        .isoformat()
        .replace("+00:00", "Z")
    )
...
def render_markdown(digest: dict[str, Any]) -> str:
    ...
    for tier in ("gold", "silver", "bronze", "none"):
        prev_val = quality["previous"][tier] if quality.get("previous") is not None else "—"
    ...
    for axis in LEADER_AXES:
        ...
        for e in entries:
            lines.append(
                f"| {e['name']} | {e['currentCount']} | {e['previousCount']} | {e['delta']:+d} |"
            )
        lines.append("")
    ...
    if stale["topStale"]:
        ...
        for s in stale["topStale"]:
            ...
            lines.append(
                f"| UC-{s['id']} | {s['title']} | {s['status']} | {last} | {s['ageDays']} |"
            )
        lines.append("")
    else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"wrote {json_path.relative_to(PROJECT_ROOT) if json_path.is_relative_to(PROJECT_ROOT) else json_path}"
    )
    print(
        f"wrote {md_path.relative_to(PROJECT_ROOT) if md_path.is_relative_to(PROJECT_ROOT) else md_path}"
    )
    return 0
