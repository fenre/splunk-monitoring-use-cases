...
def load_cat22_areas() -> dict[str, dict[str, Any]]:
    ...
    for line in body.splitlines():
        ...
        # Closes over ``line`` from the enclosing ``for`` loop. The inner
        # function is invoked exclusively in the same iteration, so the
        # late-binding semantics ruff B023 warns about are intentional and
        # safe here.
        def _field(key: str, _line: str = line) -> str | None:
            match = re.search(rf'{key}:\s*"((?:[^"\\]|\\.)*)"', _line)
            if not match:
                return None
...
def _load_detailed_clause(
    ...
    # importing it to keep the two scripts decoupled. The canonical
    # ``f"{reg_id}@{version}#{clause}"`` form is encoded into the
...
def build_buyer_block(
    ...
    * ``topThreeGaps[]`` — highest-priority uncovered / contributing-only
    ...
    if pw_values:
        totals["priorityWeightedCoveragePercent"] = round(sum(pw_values) / len(pw_values), 2)
...
def discover_evidence_pack(reg_id: str) -> str | None:
...
def discover_primer_anchor(reg_short: str) -> str | None:
    ...
    # regulation name.
    starts_match: tuple[int, str] | None = None
    token_match: tuple[int, str] | None = None
    contains_match: tuple[int, str] | None = None
    ...
    token_rx = re.compile(rf"(?:^|[^a-z0-9]){re.escape(slug_target)}(?:$|[^a-z0-9])")
...
def build_nis2_deep_coverage(
    ...
    domain_counts: dict[str, dict[str, int]] = defaultdict(
        lambda: dict.fromkeys(coverage_levels, 0)
    )
    for row in rows:
    ...
    monitorable_rows = sum(1 for row in rows if row.get("splunkCoverageType") != "not-monitorable")
...
def build_story(
    ...
    narrative_ref = None
    if narrative:
        narrative_ref = f"non-technical-view.js#cat-22/area/{_narrative_slug(narrative['name'])}"
...
def generate(
    ...
    *,
    regs_dir: pathlib.Path | None = None,
) -> dict[str, Any]:
