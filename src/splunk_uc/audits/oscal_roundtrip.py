...
def _load_schema() -> dict[str, Any]:
    ...
    schema: dict[str, Any] = json.loads(patched)
    return schema
...
def _audit_file(validator: Draft7Validator, path: Path) -> dict[str, Any]:
    ...
    if uc_id is None:
        ...
        record["issues"].append("filename does not match the expected <uc-id>.json grammar")
        return record
    ...
    if errors:
        ...
        for schema_err in errors[:100]:
            record["schema_errors"].append(
                {
                    "path": "/".join(str(p) for p in schema_err.absolute_path),
                    "message": schema_err.message,
                    "validator": schema_err.validator,
                }
            )
        record["issues"].append(
            f"{len(errors)} schema violation(s) (first 100 recorded in schema_errors)"
            if len(errors) > 100
    ...
    if canonical != original_text:
        ...
        record["issues"].append(
            "byte-equality round-trip failed — regenerate with scripts/generate_api_surface.py"
        )
    ...
    if isinstance(cdef, dict):
        ...
        if oscal_version != EXPECTED_OSCAL_VERSION:
            record["issues"].append(
                f"metadata.oscal-version is {oscal_version!r}, expected '{EXPECTED_OSCAL_VERSION}'"
            )
            if record["status"] == STATUS_OK:
        ...
        for comp in cdef.get("components", []) or []:
            if isinstance(comp, dict):
                ...
                if uuid and not _UUID_RE.match(uuid):
                    record["warnings"].append(f"component uuid not RFC-4122: {uuid}")
        ...
        record["issues"].append("payload has no top-level 'component-definition' object")
        if record["status"] == STATUS_OK:
            record["status"] = STATUS_SCHEMA_VIOLATION
    ...
    if source is None:
        ...
        record["issues"].append(
            f"no crosswalk source under {CROSSWALK_DIR.relative_to(REPO)} for UC-{uc_id}"
        )
        if record["status"] == STATUS_OK:
    ...
    if hard_recs:
        ...
        for rec in hard_recs:
            ...
            if len(rec["schema_errors"]) > 5:
                print(f"    - ... {len(rec['schema_errors']) - 5} more schema error(s)")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Phase 4.5e OSCAL round-trip gate.")
    parser.add_argument(
        "--check",
        action="store_true",
        help=(
    ...
    schema_hash_mismatch = (
        schema_meta["expected_sha256"] is not None and not schema_meta["expected_matches_observed"]
    )
    ...
    if args.check:
        ...
        if existing != payload_str:
            sys.stderr.write(
                ...
                "Run `python -m splunk_uc audit-oscal-roundtrip` "
                "(or `make audit-oscal`) and commit the updated report.\n"
            )
            return 1
    else:
    ...
    if summary["hard_failures"]:
        if schema_hash_mismatch:
            print("ERROR: committed schema SHA-256 does not match the ingest-manifest record.")
