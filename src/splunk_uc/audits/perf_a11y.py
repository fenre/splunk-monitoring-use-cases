...
_PERF_BUDGETS: list[PerfBudget] = [
    {
        ...
        "note": (
            ...
            "Raised to 760 KiB after equipment-app-map Recommended Apps "
            "panel and inventory modal grouping landed in index.html."
        ),
    },
    {
        "file": "scorecard.html",
...
def _run_axe(pages: list[str]) -> dict[str, Any]:
    ...
    if not RUN_AXE_SCRIPT.is_file():
        raise RuntimeError(f"axe runner missing at {RUN_AXE_SCRIPT.relative_to(REPO)}")
...
def _evaluate_a11y(
    ...
    a11y_block = {
        ...
        "summary": {
            ...
            "pages_with_errors": sum(1 for r in annotated_results if r["status"] == "error"),
    ...
    payload = {
        ...
        "perf": {
            ...
            "summary": {
                ...
                "missing_count": sum(1 for r in perf_records if r["status"] == "missing"),
                "over_budget_count": sum(1 for r in perf_records if r["status"] == "over-budget"),
...
def _print_human_summary(
    ...
    for rec in perf_records:
        ...
        if rec["actual_bytes"] is None:
            ...
            headroom_pct = (
                rec["headroom_bytes"] / rec["budget_bytes"] * 100.0 if rec["budget_bytes"] else 0.0
            )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Phase 4.5f performance + accessibility gate.")
    parser.add_argument(
        "--check",
        action="store_true",
        help=(
    ...
    if args.check:
        ...
        if existing_norm != regenerated_norm:
            sys.stderr.write(
                ...
                "Run `python -m splunk_uc audit-perf-a11y` and commit "
                "the updated report.\n"
            )
            import difflib
    ...
    total_hard_failures = perf_hard_failures + a11y_hard_failures + (1 if axe_runner_error else 0)
