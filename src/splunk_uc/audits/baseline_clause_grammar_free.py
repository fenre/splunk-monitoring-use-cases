...
``src/splunk_uc/audits/compliance_mappings.py`` (run via
``python -m splunk_uc audit-compliance-mappings``) so it can never again
be recorded in ``tests/golden/audit-baseline.json`` via ``--update-baseline``.
...
``src/splunk_uc/audits/compliance_mappings.py::Finding.fingerprint`` are
tab-separated:
