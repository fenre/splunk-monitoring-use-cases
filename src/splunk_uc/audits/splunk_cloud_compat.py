...
def render_report(spl_findings: list[Finding], pack_findings: list[Finding], total_ucs: int) -> str:
    ...
    for pack_rule in PACK_RULES:
        lines.append(
            f"| {pack_rule.id} | {pack_rule.severity} "
            f"| `{pack_rule.filename_glob}` | {pack_rule.description} |"
        )
    ...
    if pack_findings:
        ...
        for f in pack_findings:
            lines.append(
                f"| `{f.location}` | {f.rule_id} | {f.severity} | `{_fmt_ctx(f.context)}` |"
            )
    else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--strict", action="store_true", help="Exit non-zero on warnings (fail-level always fails)."
    )
    parser.add_argument(
        "--no-write",
        action="store_true",
