...
# P6 (scripts taxonomy, 2026-05-10) relocated this audit from
# scripts/audit_meraki_spl.py to src/splunk_uc/audits/meraki_spl.py.
# parents[3] resolves: meraki_spl.py -> audits/ -> splunk_uc/ -> src/ ->
# repo root. The legacy ``parent.parent`` chain assumed a one-level depth
# and is now wrong by three. The legacy shim at scripts/audit_meraki_spl.py
# re-exports ``main`` so any direct CLI invocation in maintainer notes
# still works.
...
RE_SOURCETYPE = re.compile(r'sourcetype\s*=\s*(?:"([^"\\]+)"|([A-Za-z_][A-Za-z0-9_:.\-]*))')
RE_INDEX = re.compile(r'index\s*=\s*(?:"([^"\\]+)"|([A-Za-z_][A-Za-z0-9_:.\-]*))')
...
def _is_meraki_uc(uc: dict[str, Any]) -> bool:
        ...
        for field in HALLUCINATED_FIELDS:
            ...
            findings.append(
                Finding(
                    ...
                    snippet=blob[max(0, field_m.start() - 40) : field_m.end() + 40],
                )
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        for path in files:
            try:
                spl = json.loads(path.read_text(encoding="utf-8")).get("spl", "")
            except Exception:
                continue
            if RE_MERAKI_SOURCETYPE.search(spl):
                meraki_files += 1
