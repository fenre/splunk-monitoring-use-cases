...
def parse_changelog() -> tuple[list[ChangelogEntry], list[str]]:
    issues: list[str] = []
    ...
    for i, line in enumerate(text.splitlines(), start=1):
        ...
        date_parsed: object | None = None
        date_raw = rest
        iso_m = re.match(
            r"^(\d{4}-\d{2}-\d{2})(?:\s*[–-]\s*\d{4}-\d{2}-\d{2})?$",  # noqa: RUF001 EN DASH separator is part of the CHANGELOG date-range grammar.
            rest,
        )
        if iso_m:
            try:
                ...
                issues.append(f"CHANGELOG.md:{i}: Invalid calendar date in header: {line!r}")
        else:
            fallback = re.search(r"(\d{4}-\d{2}-\d{2})", rest)
            if fallback:
                try:
                    date_parsed = datetime.strptime(fallback.group(1), "%Y-%m-%d").date()
                except ValueError:
                    issues.append(f"CHANGELOG.md:{i}: Could not parse date from: {line!r}")
            else:
                issues.append(f"CHANGELOG.md:{i}: No YYYY-MM-DD date found in header: {line!r}")
...
def validate_changelog(entries: list[ChangelogEntry]) -> list[str]:
    ...
    for j in range(len(parsed) - 1):
        a, da = parsed[j]
        b, db = parsed[j + 1]
        if da is not None and db is not None and db > da:  # type: ignore[operator]
            issues.append(
