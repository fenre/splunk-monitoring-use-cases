...
# scripts/audit_legacy_orphans.py to src/splunk_uc/audits/legacy_orphans.py.
# parents[3] resolves: legacy_orphans.py -> audits/ -> splunk_uc/ ->
...
_HEADING_TITLE_RE = re.compile(r"#+\s*UC-(\d+\.\d+\.\d+)\s*[·•\-]\s*([^\n]+)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        if orphans:
            print(
                f"::error::{len(orphans)} legacy-only UC IDs remain. See docs/use-cases-burndown.md."
            )
