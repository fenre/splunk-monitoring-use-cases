...
Usage:
    python3 -m splunk_uc audit-quality-metadata           # warn-only
    python3 -m splunk_uc audit-quality-metadata --strict  # exit 1 if any coverage below thresholds
...
# parents[3] resolves: quality_metadata.py -> audits/ -> splunk_uc/ ->
# src/ -> repo root. The legacy ``os.path.dirname(SCRIPT_DIR)`` chain
# assumed a one-level depth and is now wrong by three.
...
def iter_ucs(catalog: dict[str, Any]) -> Iterator[tuple[int, dict[str, Any]]]:
...
def compute_coverage(
    ...
    total = 0
    sec_total = 0
    counts: dict[str, dict[str, int]] = {k: {"present": 0} for k in FIELD_LABEL}
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Exit non-zero if any coverage target is below threshold.",
    )
    ...
    for field, label in FIELD_LABEL.items():
        ...
        print(
            f"  {label:<40} [{bar}] {pct:5.1f}%  ({present}/{denom})  target {threshold:5.1f}%  {status}"
        )
        if pct < threshold:
