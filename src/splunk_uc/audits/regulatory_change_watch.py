...
def _days_since(ts: str, now: datetime | None = None) -> int | None:
...
def _validate_schema(manifest: dict[str, Any]) -> list[str]:
    try:
        ...
        raise SystemExit(f"[change-watch] missing dev dependency 'jsonschema': {exc}") from exc
...
def _fetch_sha256_vendor(
    entry: dict[str, Any], ingest: dict[str, dict[str, Any]]
) -> dict[str, Any] | None:
...
def _fetch_github_release(entry: dict[str, Any]) -> dict[str, Any] | None:
...
def _fetch_http_head(entry: dict[str, Any]) -> dict[str, Any] | None:
...
def _fetch_rss_atom(entry: dict[str, Any]) -> dict[str, Any] | None:
    url = entry["strategy"]["url"]
    _validate_https_url(url)
    status, _, body = _http_get(url, method="GET")
    if status != 200:
    ...
    titles = re.findall(
        r"<title[^>]*>(.*?)</title>", body.decode("utf-8", "replace"), re.DOTALL | re.IGNORECASE
    )
    matches = []
