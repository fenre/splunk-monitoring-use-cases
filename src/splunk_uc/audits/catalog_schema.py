...
# parents[3] resolves: catalog_schema.py -> audits/ -> splunk_uc/ ->
...
# Per ADR-0009 + P1 step 5c, dist/catalog.json is SSOT-authoritative.
# Prefer dist/, fall back to project-root for the one-release transition
# window during which the legacy artefact still ships in the tree.
...
def main(argv: list[str] | None = None) -> int:
                        ...
                        for ui, uc in enumerate(ucs):
                            ...
                            if "pre" in uc:
                                ...
                                if not isinstance(pre, list):
                                    err(
                                        issues,
                                        f"{up}: 'pre' must be a list, got {type(pre).__name__}",
                                    )
                                else:
                                    self_full = f"UC-{uc['i']}" if is_str(uc.get("i")) else None
                                    seen: set[str] = set()
                                    for pi, dep in enumerate(pre):
                                        ...
                                        if dep == self_full:
                                            err(
                                                issues,
                                                f"{up}: 'pre' self-references {dep!r}",
                                            )
                                        if dep in seen:
                                            err(
                                                issues,
                                                f"{up}: 'pre' contains duplicate entry {dep!r}",
                                            )
                                        seen.add(dep)
    ...
    if isinstance(cat_meta, dict) and data_category_ids:
        ...
        if extra_meta:
            err(
                issues,
                "CAT_META has keys not present in DATA category id(s): " + ", ".join(extra_meta),
            )
    ...
    if roadmap is not None:
        if not isinstance(roadmap, dict):
            err(
                issues,
                f"implementationRoadmap must be a dict, got {type(roadmap).__name__}",
            )
        else:
            for cat_id, wave_map in roadmap.items():
                ...
                if not isinstance(wave_map, dict):
                    err(
                        issues,
                        f"{rp}: value must be a dict, got {type(wave_map).__name__}",
                    )
                    continue
                ...
                for wave_key in ROADMAP_WAVE_KEYS:
                    ...
                    if not isinstance(bucket, list):
                        err(
                            issues,
                            f"{bp}: wave bucket must be a list, got {type(bucket).__name__}",
                        )
                        continue
                    for ui, uc_full in enumerate(bucket):
                        if not is_str(uc_full):
                            err(
                                issues,
                                f"{bp}[{ui}]: must be str, got {type(uc_full).__name__}",
                            )
                            continue
