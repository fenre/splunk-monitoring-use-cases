...
AUTOGEN_BEGIN_MARKER = "<!-- BEGIN-AUTOGENERATED-SOURCES -->"
AUTOGEN_END_MARKER = "<!-- END-AUTOGENERATED-SOURCES -->"


def _extract_autogen_footer(path: Path) -> str:
    """Return the autogenerated APA-references footer block from ``path``.

    ``scripts/generate_doc_references.py`` appends a fenced
    ``<!-- BEGIN-AUTOGENERATED-SOURCES -->`` ... ``<!-- END-AUTOGENERATED-SOURCES -->``
    block to every documentation file that has explicit source mappings.
    The matrix doc is one of them, so our overwrite step would otherwise
    strip the footer on every CI run and trip the ``Splunk version-matrix
    audit \u2014 generated artefacts are committed`` drift gate.

    We re-attach the existing footer verbatim. ``generate_doc_references.py``
    runs later in ``make sync-generated-check`` and will refresh the
    contents if upstream source-mappings drift; this helper just makes
    sure we don't *lose* the footer between the audit's write step and
    that refresh.
    """
    ...
    begin = text.find(AUTOGEN_BEGIN_MARKER)
    if begin == -1:
        return ""
    end = text.find(AUTOGEN_END_MARKER, begin)
    if end == -1:
        return ""
    end += len(AUTOGEN_END_MARKER)
    # generate_doc_references.py emits ``\n---\n\n`` immediately before
    # the BEGIN marker; keep that separator with the footer so we don't
    # strip the visual rule when rewriting.
    sep_marker = "\n---\n\n"
    sep_idx = text.rfind(sep_marker, 0, begin)
    if sep_idx != -1 and (begin - sep_idx) < 8:
        begin = sep_idx + 1  # keep the "---\n\n" prefix
...
def main(argv: list[str] | None = None) -> int:
    ...
    if write_outputs:
        ...
        # Preserve the autogenerated APA-references footer that
        # ``scripts/generate_doc_references.py`` maintains on this doc;
        # see _extract_autogen_footer for the full rationale.
        existing_footer = _extract_autogen_footer(MATRIX_MD_PATH)
        final_md = rendered_md
        if existing_footer:
            final_md = rendered_md.rstrip() + "\n\n" + existing_footer + "\n"
        MATRIX_MD_PATH.write_text(final_md, encoding="utf-8")
