...
def _load_uc_sidecar(path: Path) -> dict[str, Any] | None:
    try:
...
def _classify_fixture(
    ...
    if "events_positive" in payload and "events_negative" in payload:
        ...
        if issues or not isinstance(pos_list, list) or not isinstance(neg_list, list):
            return (STATUS_MALFORMED, shape, 0, 0, issues)
...
def _collect_records() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for sidecar_path in sorted(CONTENT.glob("cat-*/UC-*.json")):
        ...
        full_entries = [
            e for e in compliance if isinstance(e, dict) and e.get("assurance") == "full"
        ]
        ...
        records.append(
            {
                ...
                "full_assurance_clauses": sorted(
                    {f"{e.get('regulation', '?')}:{e.get('clause', '?')}" for e in full_entries}
                ),
            }
        )
...
def _render_report(records: list[dict[str, Any]], summary: dict[str, Any]) -> str:
    ...
    payload = {
        "$comment": (
            ...
            "`python -m splunk_uc audit-sandbox-validation`. Hard failures "
            "(malformed or unparseable fixtures) block CI; missing/empty "
            "fixtures are tracked gaps for Phase 5.2 SME review. See "
...
def _print_human_summary(records: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    ...
    if summary["hard_failures"]:
        ...
        for rec in records:
            if rec["status"] in HARD_FAIL_STATUSES:
                print(f"  {rec['uc_id']:<12} {rec['status']:<10} {rec['fixtureRef']}")
                for issue in rec["issues"]:
                    print(f"    - {issue}")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Phase 4.5c sandbox validation gate.")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    if args.check:
        ...
        if existing != payload_str:
            sys.stderr.write(
                ...
                "Run `python -m splunk_uc audit-sandbox-validation` and "
