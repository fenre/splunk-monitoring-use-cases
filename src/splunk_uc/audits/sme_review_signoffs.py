...
-----
    ...
    python -m splunk_uc audit-sme-review-signoffs
"""
...
def _validate_semantics(data: dict[str, Any]) -> tuple[list[str], list[str]]:
    ...
    for idx, record in enumerate(signoffs):
        ...
        if isinstance(commit, str):
            if not _SHA_RE.match(commit):
                errors.append(f"{prefix}: commit SHA {commit!r} is not a valid short/long hex SHA.")
        ...
        for uc_id in ucs:
            ...
            path = _uc_sidecar_path(uc_id)
            if path is None:
                errors.append(f"{prefix}: scope.ucs entry {uc_id!r} is not a valid UC id.")
        ...
        if caveats and ucs:
            sidecar_caveats = _collect_uc_sme_caveats([u for u in ucs if isinstance(u, str)])
    ...
    if isinstance(baseline, str) and not _SHA_RE.match(baseline):
        errors.append(f"baseline_commit: {baseline!r} is not a valid short/long hex SHA.")

    return errors, warnings


def _print_summary(data: dict[str, Any], errors: list[str], warnings: list[str]) -> None:
