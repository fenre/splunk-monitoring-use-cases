...
SILVER_MIN_LENGTHS = {
    ...
    "value": 60,
    "dataSources": 30,
    "detailedImplementation": 200,
    "grandmaExplanation": 20,
}
GOLD_MIN_LENGTHS = {
    "description": 80,
    "value": 80,
    "dataSources": 40,
    "detailedImplementation": 500,
    "grandmaExplanation": 20,
}
...
SECTION_PATTERNS = [
    ...
    re.compile(
        r"(?:step\s*1|configure\s+data|data\s+collection|collection\s+setup)", re.IGNORECASE
    ),
    re.compile(
...
def _has_vendor_ui_reference(text: str) -> bool:
    ...
    patterns = [
        re.compile(
            r"(?:compare|verify|check|confirm)\s+(?:against|with|in|to)\s+(?:the\s+)?(?:\w+\s+){0,3}(?:UI|dashboard|console|portal|interface|center|assurance|inventory)",
            re.IGNORECASE,
        ),
        re.compile(
            r"(?:Catalyst|Meraki|ISE|vCenter|Azure|AWS|GCP|NetApp|Palo\s*Alto)\s+(?:Center|Dashboard|Console|Portal|UI|Manager)",
            re.IGNORECASE,
        ),
    ]
...
def _has_specific_troubleshooting(text: str) -> bool:
    ...
    troubleshoot_match = re.search(
        r"(?:troubleshoot|step\s*5|failure|common\s+issue)", text, re.IGNORECASE
    )
    ...
    specific_patterns = [
        ...
        re.compile(
            r"(?:fewer|less|more)\s+(?:devices?|events?|records?|hosts?)\s+than", re.IGNORECASE
        ),
        re.compile(
            r"(?:API|endpoint|input|modular)\s+(?:error|timeout|throttl|429|5\d\d|fail)",
            re.IGNORECASE,
        ),
        re.compile(
            r"(?:role|permission|access|auth|credential)\s+(?:denied|error|fail|incorrect)",
            re.IGNORECASE,
        ),
    ]
...
def audit_uc(uc: dict[str, Any], filepath: Path) -> dict[str, Any]:
    ...
    if similarity > 0.7:
        warnings.append(
            f"description and value are {similarity:.0%} similar — they should be distinct"
        )
        depth -= 10
    ...
    if isinstance(detailed, str) and detailed:
        ...
        if specificity >= 5:
            ...
            warnings.append(
                "detailedImplementation lacks product-specific terms (sourcetypes, API paths, field names)"
            )
            depth -= 5
...
def find_consolidation_candidates(
    ...
    by_subcat: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for r in results:
        parts = r["id"].split(".")
        if len(parts) >= 2:
...
def write_report(
    ...
    report = {
        ...
        "avg_depth_score": round(sum(r["depth_score"] for r in results) / len(results), 1)
        if results
        else 0,
        "ucs": results,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Audit UC JSON against Gold Standard quality profile."
    )
    parser.add_argument(
        "--check",
        action="store_true",
        help="CI mode: exit 1 if any UC has shallow content or falls below Bronze.",
    )
    parser.add_argument(
        "--summary", action="store_true", help="Print human-readable summary table."
    )
    parser.add_argument(
        "--report", action="store_true", help="Write JSON report to reports/quality-audit.json."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="consolidation",
        help="List consolidation candidates.",
    )
    ...
    consolidation = (
        find_consolidation_candidates(results) if (args.consolidation or args.report) else []
    )
    ...
    if args.consolidation and not args.summary:
        if consolidation:
            for c in consolidation:
                print(
                    f"{c['uc_a']} <-> {c['uc_b']}: {c['title_similarity']:.0%} similar ({c['reason']})"
                )
        else:
    ...
    if args.check:
        ...
        high_boilerplate = [
            r for r in results if any("boilerplate" in w for w in r.get("warnings", []))
        ]
