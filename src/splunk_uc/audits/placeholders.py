...
# scripts/audit_placeholders.py to src/splunk_uc/audits/placeholders.py.
...
_MARKERS: list[tuple[str, re.Pattern[str], str, str]] = [
    ...
    (
        ...
        re.compile(
            r"<\s*(?:YOUR|REPLACE|PUT|ENTER|INSERT|FILL|TODO|TBD)[A-Z0-9_\- ]*\s*>", re.IGNORECASE
        ),
        "HIGH",
...
def _check_known_fp_blank(uc_id: str, file: str, body: str) -> list[Finding]:
    ...
    # The literal set deliberately includes EN DASH and EM DASH because
    # the audit's whole point is to flag those Unicode dash characters
    # leaking into UC content. RUF001 (ambiguous-unicode-character)
    # therefore surfaces a false positive here.
    stripped = text.rstrip(".").strip()
    if stripped in {"|", "—", "-", "–"} or stripped.lower() in {  # noqa: RUF001
        "none",
        "n/a",
        "na",
        "tbd",
...
def _check_editorial_headers(file: str, text: str) -> list[Finding]:
    ...
    for m in RE_EDITORIAL_HEADER.finditer(text):
        ...
        pos = m.start()
        uc_id = "—"
        for uc, _body, s, e in _iter_uc_blocks(text):
            if s <= pos < e:
                uc_id = uc
                break
        findings.append(
