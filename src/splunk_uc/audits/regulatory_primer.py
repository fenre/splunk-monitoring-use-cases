...
# scripts/audit_regulatory_primer.py to src/splunk_uc/audits/regulatory_primer.py.
# Four nested dirnames reach the repo root from the new four-level-deep home.
...
CATEGORY_JSON = os.path.join(REPO_ROOT, "content", "cat-22-regulatory-compliance", "_category.json")
...
def audit(primer_text: str) -> list[Finding]:
    ...
    t2_badge = re.search(r"\*\*Tier 2\*\*[^;]*;\s*(\d+)\s+frameworks", primer_text)
    if t2_badge:
    ...
    # T3 badge: "N today"
    t3_badge = re.search(r"\*\*Tier 3\*\*[^;]*;\s*(\d+)\s+today", primer_text)
    if t3_badge:
        ...
        if claimed != actual_t3:
            findings.append(
                Finding(
                    ...
                    message=(
                        f"T3 badge claims {claimed} today but regulations.json has {actual_t3}."
                    ),
    ...
    for m in phase_pat.finditer(primer_text):
        findings.append(
            Finding(
                ...
                message=(
                    f'Internal jargon "{m.group(0)}" found — reword to a functional description.'
                ),
                line=_find_line(primer_text, m.start()),
            )
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    with open(PRIMER, encoding="utf-8") as fh:
        primer_text = fh.read()
    ...
    if args.json:
        ...
        print(
            "Findings: " + ", ".join(f"{k}={v}" for k, v in sorted(counts.items()))
            if counts
            else "Findings: 0"
        )
        print()
