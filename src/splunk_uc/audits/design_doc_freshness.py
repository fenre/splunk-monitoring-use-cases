...
Usage::
    ...
    python -m splunk_uc audit-design-doc-freshness [--strict]
"""
...
# parents[3] resolves: design_doc_freshness.py -> audits/ -> splunk_uc/ ->
# src/ -> repo root. The legacy ``parent.parent`` chain assumed a one-level
...
def read_design() -> str:
    if not os.path.exists(DESIGN_MD):
        ...
        sys.exit(2)
    with open(DESIGN_MD, encoding="utf-8") as fh:
        return fh.read()
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Exit non-zero when any freshness issue is found.",
    )
    ...
    print(
        f"OK: DESIGN.md has all {len(CANONICAL_SECTIONS)} canonical sections "
