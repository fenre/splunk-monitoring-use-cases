...
def _detect_cycle(adjacency: dict[str, list[str]]) -> list[str] | None:
    ...
    parent: dict[str, str | None] = dict.fromkeys(adjacency)

    def walk_back(stop: str, start: str) -> list[str]:
        path = [start]
        cursor: str | None = parent[start]
    ...
    for uid in sorted(uc_index.keys()):
        ...
        for pi, dep in enumerate(pre):
            if not isinstance(dep, str):
                errors.append(f"shape: {uid} pre[{pi}] is not a string ({type(dep).__name__})")
                continue
            if not PREREQ_UC_PATTERN.match(dep):
                errors.append(f"shape: {uid} pre[{pi}]={dep!r} does not match UC-X.Y.Z pattern")
                continue
            if dep == uid:
...
def _wave_summary(uc_index: dict[str, dict[str, Any]]) -> dict[str, int]:
    counts = dict.fromkeys(VALID_WAVES, 0)
    counts["unassigned"] = 0
...
def _read_existing_report() -> str | None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "  Waves: crawl={crawl}, walk={walk}, run={run}, unassigned={unassigned}".format(**summary)
    )
    print(
    ...
    if args.check:
        ...
        if existing is None:
            print(
                ...
                "Run `python -m splunk_uc audit-prerequisites --write-report` "
                "and commit it.",
                file=sys.stderr,
            )
            return 1
        if existing != serialised:
            print(
                ...
                "`python -m splunk_uc audit-prerequisites --write-report` "
