    ...
    for elem in root.iter():
        ...
        panels.append(
            Panel(
                ...
                spl=spl.strip(),
                earliest=(
                    earliest_elem.text.strip()
                    if earliest_elem is not None and earliest_elem.text
                    else "-15m"
                ),
                latest=(
                    latest_elem.text.strip()
...
def _resolve_token(token_var: str) -> str:
    ...
    # src/splunk_uc/audits/dashboard_spl.py instead of
    # scripts/audit_dashboard_spl.py. parents[3] resolves: dashboard_spl.py ->
    # audits/ -> splunk_uc/ -> src/ -> repo root. The legacy
    # ``parent.parent`` chain assumed a one-level-deep home and is now
    # wrong by three.
    secrets = Path(__file__).resolve().parents[3] / "secrets.env"
    if secrets.exists():
...
def main(argv: list[str] | None = None) -> int:
    ...
    # the dispatcher (``python -m splunk_uc audit-dashboard-spl ...``) can
    # forward sub-command flags. ``argv=None`` preserves the legacy
    ...
    parser.add_argument(
        "--app-root",
        default=str(
            # P6 (2026-05-09) post-relocation depth: parents[3] reaches repo root
            # from src/splunk_uc/audits/dashboard_spl.py.
            Path(__file__).resolve().parents[3] / "splunk-apps/splunk-uc-recommender",
        ),
    ...
    if not args.quiet:
        print(
            f">> dashboard SPL audit ({len(panels)} panels across {len({p.view for p in panels})} dashboards)"
        )
    ...
    for panel in panels:
        ...
        if result.ok:
            if not args.quiet:
                ...
                print(
                    f"   [PASS] {panel.view} :: {panel.panel} ({result.result_count} rows)  {spl_preview}"
                )
        else:
