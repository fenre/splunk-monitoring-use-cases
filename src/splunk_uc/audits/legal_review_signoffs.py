...
# scripts/audit_legal_review_signoffs.py to
# src/splunk_uc/audits/legal_review_signoffs.py. parents[3] resolves:
# legal_review_signoffs.py -> audits/ -> splunk_uc/ -> src/ -> repo root.
...
def _load_json(path: Path, label: str) -> dict[str, Any]:
    if not path.is_file():
        ...
        raise SystemExit(1)
    try:
        # ``json.loads`` returns ``Any``; the data file is contractually
        # required to be a JSON object, hence the cast.
...
def _validate_semantics(data: dict[str, Any]) -> list[str]:
    ...
    for idx, record in enumerate(signoffs):
        ...
        if isinstance(commit, str):
            if not _SHA_RE.match(commit):
                issues.append(f"{prefix}: commit SHA {commit!r} is not a valid short/long hex SHA.")
        ...
        for uc_id in ucs:
            ...
            path = _uc_sidecar_path(uc_id)
            if path is None:
                issues.append(f"{prefix}: scope.ucs entry {uc_id!r} is not a valid UC id.")
        ...
        if caveats and ucs:
            sidecar_caveats = _collect_uc_legal_caveats([u for u in ucs if isinstance(u, str)])
    ...
    if isinstance(baseline, str) and not _SHA_RE.match(baseline):
        issues.append(f"baseline_commit: {baseline!r} is not a valid short/long hex SHA.")

    return issues


def _print_summary(data: dict[str, Any], issues: list[str]) -> None:
    ...
    if signoffs:
        ...
        for record in signoffs[-5:]:
            ...
            role = record.get("reviewerRole", "?")
            outcome = record.get("outcome", "?")
            regs = ", ".join((record.get("scope") or {}).get("regulations") or [])
            print(
...
def main(argv: list[str] | None = None) -> int:
    # P6 (scripts taxonomy, 2026-05-09) added the ``argv`` parameter so the
    # dispatcher (``python -m splunk_uc audit-legal-review-signoffs ...``)
    # can forward sub-command flags. The audit currently accepts no flags;
    # the parameter is wired through and ignored to keep the dispatcher
    # contract uniform with the other audits.
    del argv
