...
# scripts/audit_uc_structure.py to src/splunk_uc/audits/uc_structure.py.
# The file moved from one level deep (scripts/) to four levels deep
# (src/splunk_uc/audits/<name>.py); the dirname chain therefore needs
# four nested calls instead of two to reach the repo root.
...
VALID_CRITICALITY = frozenset({"🔴 Critical", "🟠 High", "🟡 Medium", "🟢 Low"})
VALID_DIFFICULTY = frozenset({"🟢 Beginner", "🔵 Intermediate", "🟠 Advanced", "🔴 Expert"})
...
RE_FIELD_LINE = re.compile(r"^\s*-\s*\*\*([^*]+):\*\*\s*(.*)$")
...
def extract_field_lines(body: str) -> dict[str, str]:
    fields: dict[str, str] = {}
...
def extract_spl_fenced(body: str) -> tuple[str | None, str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    # the dispatcher (``python -m splunk_uc audit-uc-structure ...``) can
    # forward sub-command flags. ``argv=None`` preserves the legacy
    # behaviour where ``argparse`` falls back to ``sys.argv[1:]``.
    ap = argparse.ArgumentParser(
        description=("Audit UC structure across the markdown corpus AND the SSOT JSON sidecars.")
    )
    ap.add_argument(
        "--full",
        action="store_true",
