...
def audit_entry_hashes(ledger: dict[str, Any]) -> list[str]:
    ...
    for idx, entry in enumerate(ledger.get("entries", [])):
        ...
        try:
            canonical = {field: entry[field] for field in CANONICAL_FIELD_ORDER if field in entry}
...
def audit_merkle_root(ledger: dict[str, Any]) -> list[str]:
    ...
    if expected != got:
        return [f"merkle: merkleRoot mismatch: stored={got[:16]}… expected={expected[:16]}…"]
    return []
...
def _run_gh_attestation_verify(
    ...
    if result.returncode != 0:
        return [f"signature: `gh attestation verify` rejected the bundle: {result.stderr.strip()}"]
    return []
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Shorthand for the default behaviour; present for CI consistency.",
    )
    parser.add_argument(
        "--verify-signature",
        action="store_true",
        help="Run `gh attestation verify` when the ledger is 'attested'.",
    )
    parser.add_argument(
        "--require-signature",
        action="store_true",
        help="Treat 'unsigned' state or missing/invalid signature as fatal.",
    )
    ...
    if not LEDGER_PATH.exists():
        print(
            ...
            "Run `python -m splunk_uc generate-mapping-ledger` first.",
            file=sys.stderr,
        )
        return 1
    ...
    if errors:
        ...
        if len(errors) > 40:
            print(
                f"  … and {len(errors) - 40} more errors (see reports/mapping-ledger-audit.json).",
                file=sys.stderr,
            )
        return 1
