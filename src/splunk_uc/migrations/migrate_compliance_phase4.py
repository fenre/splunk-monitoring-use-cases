...
scripts/migrate_compliance_phase4.py to
src/splunk_uc/migrations/migrate_compliance_phase4.py. parents[3]
resolves: migrate_compliance_phase4.py -> migrations/ ->
splunk_uc/ -> src/ -> repo root. The legacy ``parent.parent`` chain
...
the soak period. Module body is otherwise unchanged -- the synthesised
...
class RegulationsIndex:
    ...
    def __init__(self, doc: dict[str, Any]) -> None:
        self.doc = doc
        self.aliases = {k.lower(): v.lower() for k, v in doc.get("aliasIndex", {}).items()}
    ...
    def resolve_id(self, name: str) -> str | None:
        ...
        if key in self.by_short:
            short_id: str = self.by_short[key].get("id", "")
            return short_id.lower() or None
        if key in self.aliases:
            alias_value: str = self.aliases[key]
            return alias_value
    ...
    def clause_meta(self, reg_id: str, version: str, clause: str) -> dict[str, Any] | None:
            ...
            if str(v.get("version")) == str(version):
                for cc in v.get("commonClauses", []):
                    if str(cc.get("clause")) == str(clause):
                        meta: dict[str, Any] = cc
                        return meta
        return None

    def framework(self, reg_id: str) -> dict[str, Any] | None:
...
def _topic_phrase(clause_meta: dict[str, Any] | None, clause: str) -> str:
...
def derive_obligation_ref(entry: dict[str, Any], index: RegulationsIndex) -> str | None:
...
def process_sidecar(
    ...
    for e in sidecar["compliance"]:
        ...
        if key in seen:
            raise SystemExit(f"{path}: duplicate compliance triple {key} produced by migration")
        seen.add(key)
...
def build_obligation_manifest(index: RegulationsIndex) -> str:
    ...
    for tier in (1, 2, 3):
        tier_rows: list[tuple[str, str, str, float | None, str]] = []
        ...
        for framework in index.doc.get("frameworks", []):
            ...
            for version in framework.get("versions", []):
                ...
                for cc in common_sorted:
                    ...
                    tier_rows.append(
                        (
                            framework.get("shortName") or framework.get("id") or "?",
        ...
        if tier_rows:
            ...
            for reg, ver, clause, pw, topic in tier_rows:
                ...
                lines.append(f"| {reg} | `{ver}` | `{clause}` | {pw_str} | {topic} |")
            lines.append("")
    ...
    for tier, c in sorted(per_tier_totals.items()):
        lines.append(f"* Tier {tier}: `{c['done']}` done, `{c['missing']}` pending.")
    lines.append("")
    ...
    *,
    ...
    for path in sidecars:
        ...
        if check_only or dry_run:
            continue
        # process_sidecar above only validated; perform the actual write here.
        # The legacy script also recomputed _canonical_sidecar(_read_json(path))
        # but discarded the result, so that dead computation was dropped during
        # the P6 migration into splunk_uc.migrations.
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Restrict to specific UC id(s). Space/comma-separated. May be repeated.",
    )
    parser.add_argument(
        "--stats-only",
        action="store_true",
