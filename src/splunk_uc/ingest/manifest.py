...
P6 (scripts taxonomy, 2026-05-10) relocated this helper from
scripts/ingest/manifest.py to src/splunk_uc/ingest/manifest.py. It is
NOT a verb -- it is a shared library imported by the ingest drivers
(oscal, attack, d3fend, atomic, olir) for FetchRecord / fetch /
write_manifest / merge_into_manifest. The legacy shim at
scripts/ingest/manifest.py re-exports the public names so any direct
import (``from manifest import fetch``) and the legacy ``ingest_*.py``
sys.path-injecting drivers keep working unchanged during the soak
period. Type hints have been modernised from ``typing.Dict / List /
Optional`` to PEP 585 / 604 (``dict / list / X | None``) for
...
def write_manifest(
    ...
    body: dict[str, Any] = {
        ...
        "schema_version": 1,
        "provenance": [r.to_dict() for r in sorted(records, key=lambda r: (r.source_id, r.url))],
    }
    if extra:
        body.update(extra)
...
def merge_into_manifest(manifest_path: pathlib.Path, new_records: list[FetchRecord]) -> None:
    ...
    existing: dict[tuple[str, str], FetchRecord] = {}
