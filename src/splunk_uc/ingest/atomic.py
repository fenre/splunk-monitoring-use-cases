...
scripts/ingest/ingest_atomic.py to src/splunk_uc/ingest/atomic.py.
parents[3] resolves: atomic.py -> ingest/ -> splunk_uc/ -> src/ ->
repo root. The legacy ``parents[2]`` chain assumed depth two and is
now wrong by one level. The legacy shim at
scripts/ingest/ingest_atomic.py re-exports ``main`` and ``run`` so
...
Run:
  python -m splunk_uc ingest-atomic
...
def _walk_technique_block(block: Any) -> tuple[dict[str, Any], list[Any]]:
    ...
    the technique key, or under a ``technique`` subdocument. ``atomic_tests``
    is intentionally typed as ``list[Any]`` because individual entries are
    only validated at the call site (some upstream YAML versions interleave
    non-dict commentary nodes that we filter out via ``isinstance``).
    """

    if not isinstance(block, dict):
        return {}, []
    raw_meta = block.get("technique")
    technique_meta: dict[str, Any] = raw_meta if isinstance(raw_meta, dict) else {}
    atomic_tests: list[Any] = _as_list(block.get("atomic_tests"))
    return technique_meta, atomic_tests


def _normalise(doc: Any) -> dict[str, Any]:
        ...
        for technique_id, block in techniques.items():
            ...
            for test in tests:
                if not isinstance(test, dict):
                    continue
                platforms = [
                    str(p).strip().lower() for p in _as_list(test.get("supported_platforms")) if p
                ]
                technique_entry["platforms"].update(platforms)
                raw_executor = test.get("executor")
                executor: dict[str, Any] = raw_executor if isinstance(raw_executor, dict) else {}
                ...
                atomics.append(
                    {
                        ...
                        "has_cleanup": bool(
                            isinstance(executor, dict) and executor.get("cleanup_command")
                        ),
                        "url": _attack_url(tid),
                    }
                )
...
def main(argv: list[str] | None = None) -> int:
    """Dispatcher entry-point. ``argv`` accepted for the registry contract; the Atomic Red Team ingest takes no flags."""
    del argv
    return run()
