...
scripts/ingest/ingest_olir.py to src/splunk_uc/ingest/olir.py.
parents[3] resolves: olir.py -> ingest/ -> splunk_uc/ -> src/ ->
repo root. The legacy ``parents[2]`` chain assumed depth two and is
now wrong by one level. The legacy shim at
scripts/ingest/ingest_olir.py re-exports ``main`` and ``run`` so
...
Run:
  python -m splunk_uc ingest-olir
"""
...
def _normalise(src: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    ...
    capability_idx: dict[str, dict[str, Any]] = {}
    attack_idx: dict[str, dict[str, Any]] = {}
...
def main(argv: list[str] | None = None) -> int:
    """Dispatcher entry-point. ``argv`` accepted for the registry contract; the OLIR ingest takes no flags."""
    del argv
    return run()
