...
def write_data_js(data, cat_meta, output_path, recently_added=None, roadmap=None):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"const RECENTLY_ADDED = new Set({recent_json});\n")
        f.write(f"const ROADMAP = {roadmap_json};\n")
...
def validate_prerequisites(data):
    ...
    visited = 0
    # Deterministic processing order for reproducible error messages.
    queue.sort()
    while queue:
        u = queue.pop(0)
        visited += 1
...
def _extract_cycle(index, residual):
    ...
    We do a DFS from the first residual node and report the first back-edge.
    """
    start = residual[0]
    stack = [start]
    in_stack = {start}
    ...
    def dfs(node):
        for nxt in sorted(
            dep for dep in (index[node].get("pre") or []) if dep in index
        ):
            edge = (node, nxt)
    ...
    cycle = dfs(start) or [start, start]
    return cycle
