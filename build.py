...
# authors can use the same visual style as CRITICALITY_MAP / DIFFICULTY_MAP
# without needing a separate docs read. The three values correspond to
# the "crawl → walk → run" roadmap documented in docs/implementation-ordering.md.
# Value "crawl"  = ship first (TA + base feed + a panel).
...
# Canonical UC-id regex for "UC-X.Y.Z" prerequisite references in markdown.
# Accepts the bracketed form "- **Prerequisite UCs:** UC-1.1.1, UC-13.1.1".
# Bare "1.1.1" (without the "UC-" prefix) is rejected to force authors to
# use the full public id and avoid accidental free-form text leaking in.
...
def assign_regulations(uc, cat_id, sub_id):
    ...
    if sub_str.startswith("21.11"):
        ...
        if "nis2" in title:
            auto.add("NIS2")
        # Category 12 (Application Performance) has "dora" in unrelated UC titles
...
def parse_category_file(filepath):
    ...
    while i < len(lines):
        ...
        if current_uc is not None:
            ...
            if m:
                ...
                elif field_name == "wave":
                    # Implementation wave: crawl|walk|run (see WAVE_MAP).
                    # Unknown values are stored verbatim so the schema
                    # validator and validate_prerequisites() can flag
                    # them with a useful error that names the UC.
                    canonical_wave = WAVE_MAP.get(field_value.lower(), field_value.lower())
                    if canonical_wave:
                        current_uc["wv"] = canonical_wave
                elif field_name in ("prerequisite ucs", "prerequisite uc", "prerequisites"):
                    # Comma-separated list of "UC-X.Y.Z" ids. Other tokens
                    # on the line (narrative prose, bullets, etc.) are
                    # stripped by the regex. Deduplicated + sorted for
                    # reproducible output.
                    pre = sorted(set(_PREREQ_UC_RE.findall(field_value)))
                    if pre:
                        current_uc["pre"] = pre
    ...
    if orphan_ucs:
        for msg in orphan_ucs:
            print(f"  ERROR {msg}", file=sys.stderr)
        print(
            f"Parse failed: {len(orphan_ucs)} UC(s) found before any subcategory heading in {basename}",
            file=sys.stderr,
        )
        sys.exit(1)
...
def write_data_js(data, cat_meta, output_path, recently_added=None, roadmap=None):
    """Write data.js with DATA, CAT_META, CAT_GROUPS, EQUIPMENT, FILTER_FACETS, RECENTLY_ADDED, and ROADMAP.

    ROADMAP is the per-category implementation-order bucket map produced
    by compute_implementation_roadmap(); keys are stringified category ids
    and values are {crawl, walk, run, unassigned} arrays of UC-X.Y.Z ids.
    Consumed by index.html to render the "Crawl → Walk → Run" roadmap
    panel on each category page.
    """
    ...
    recent_json = json.dumps(recently_added or [], separators=(",", ":"))
    roadmap_json = json.dumps(roadmap or {}, separators=(",", ":"))

    js_content = (
        "// Auto-generated by build.py — do not edit manually\n"
        f"const DATA = {data_json};\n"
        f"const CAT_META = {meta_json};\n"
        f"const CAT_GROUPS = {groups_json};\n"
        f"const EQUIPMENT = {equipment_json};\n"
        f"const FILTER_FACETS = {facets_json};\n"
        f"const RECENTLY_ADDED = new Set({recent_json});\n"
        f"const ROADMAP = {roadmap_json};\n"
    )
    _atomic_write(output_path, js_content)
...
_WAVE_RANK = {"crawl": 0, "walk": 1, "run": 2}


def validate_prerequisites(data):
    """Validate the uc["pre"] / uc["wv"] dependency graph.

    Fails the build (SystemExit 1) on:
      * Unknown prerequisite ids (referenced UC missing from the catalogue).
      * Self-references (a UC listing its own id as a prerequisite).
      * Cycles (Kahn's topological sort surfaces the first cycle path).

    Emits warnings (does not fail the build) for:
      * Wave monotonicity violations — a UC at an earlier wave
        (e.g. crawl) whose prerequisite list includes a later-wave UC
        (e.g. run). These usually mean the author inverted the tier.

    Also prints a deterministic "Waves:" summary so CI output is easy
    to spot-check.
    """
    id_counts = Counter()
    for cat in data:
    ...
    for uid, count in sorted(id_counts.items()):
        if count > 1:
            errors.append(f"duplicate UC id: {uid} appears {count} times")
            ...
            for uc in sub.get("u", []):
                ...
                for dep in pre:
                    ...
                    if dep not in index:
                        ...
                        continue
                    src_wave = uc.get("wv")
                    dep_wave = index[dep].get("wv")
                    if src_wave and dep_wave:
                        src_rank = _WAVE_RANK.get(src_wave)
    ...
    if visited < len(index):
        # At least one cycle exists. Walk the residual indegree>0 set
        # to surface a concrete cycle path for the error message.
        residual = sorted(u for u, d in indeg.items() if d > 0)
        cycle_path = _extract_cycle(index, residual)
        errors.append("cycle detected: " + " -> ".join(cycle_path))

    # Deterministic wave summary.
    wave_counts = {"crawl": 0, "walk": 0, "run": 0, "unassigned": 0}
    for uc in index.values():
        w = uc.get("wv")
        if w in wave_counts:
            wave_counts[w] += 1
        else:
            wave_counts["unassigned"] += 1
...
def _extract_cycle(index, residual):
    """Return a concrete cycle path from the residual graph for error output.

    ``residual`` is the sorted list of UC ids that still have indegree>0
    after Kahn's algorithm; every one of them is on or feeds into a cycle.
    We try a DFS from each residual node until we find a real back-edge.
    """
    ...
    if len(residual) > 10:
        preview += "..."
    return [f"(cycle among: {preview})"]


def compute_implementation_roadmap(data):
    """Bucket every UC by category and wave for the catalog/UI roadmap.

    Returns a dict keyed by stringified category id, each value a dict
    with ``crawl`` / ``walk`` / ``run`` / ``unassigned`` arrays holding
    ``UC-X.Y.Z`` ids in deterministic (subcategory, index) order.
    """
    roadmap = {}
    for cat in data:
        cat_id = str(cat.get("i", ""))
        if not cat_id:
            continue
        buckets = {"crawl": [], "walk": [], "run": [], "unassigned": []}
        items = []
        for sub in cat.get("s", []):
            sub_id = sub.get("i", "")
            for uc in sub.get("u", []):
                items.append((sub_id, uc.get("i", ""), uc.get("wv")))
        items.sort(key=lambda t: (_uc_sort_key(t[0]), _uc_sort_key(t[1])))
        for _sub, uc_id, wv in items:
            bucket = wv if wv in buckets else "unassigned"
            if uc_id:
                buckets[bucket].append("UC-" + uc_id)
        roadmap[cat_id] = buckets
    return roadmap


def _uc_sort_key(dotted):
    """Numeric tuple sort key for '1.2.3'-style ids; tolerates junk."""
    parts = []
...
def _parse_changelog():
    ...
    if not os.path.isfile(path):
        print("  WARN  CHANGELOG.md not found — skipping release-note sync")
        return []
...
def _md_inline_to_html(text):
    ...
    text = text.replace("\x00EM\x00", "&mdash;")
    text = text.replace("\x00EN\x00", "&ndash;")
    text = text.replace("\x00EL\x00", "&hellip;")
    return text
...
def main():
    ...
    # Early duplicate-ID check before any output is written
    all_uc_ids = Counter()
    for cat in data:
        for sub in cat.get("s", []):
            for uc in sub.get("u", []):
                if "i" in uc:
                    all_uc_ids[uc["i"]] += 1
    dup_ids = {uid: cnt for uid, cnt in all_uc_ids.items() if cnt > 1}
    if dup_ids:
        for uid, cnt in sorted(dup_ids.items()):
            print(f"  ERROR duplicate UC id: UC-{uid} appears {cnt} times", file=sys.stderr)
        print(f"Build aborted: {len(dup_ids)} duplicate UC id(s) found.", file=sys.stderr)
        sys.exit(1)
    ...
    validate_prerequisites(data)

    # Parse INDEX.md for metadata
    cat_meta, cat_starters = parse_index_metadata()
    print(f"CAT_META: {len(cat_meta)} categories")

    # Detect recently added UCs by comparing against existing catalog.json
    recently_added = []
    if os.path.exists(OUTPUT_CATALOG_JSON):
        try:
            with open(OUTPUT_CATALOG_JSON, "r", encoding="utf-8") as f:
                prev = json.load(f)
            prev_ids = set()
            for cat in prev.get("DATA", []):
                for sub in cat.get("s", []):
                    for uc in sub.get("u", []):
                        prev_ids.add(uc.get("i", ""))
            for cat in data:
                for sub in cat.get("s", []):
                    for uc in sub.get("u", []):
                        if uc.get("i", "") and uc["i"] not in prev_ids:
                            recently_added.append(uc["i"])
            print(f"Recently added: {len(recently_added)} new UCs detected via catalog diff")
        except (json.JSONDecodeError, KeyError):
            print("  Could not parse existing catalog.json for diff")

    # Fallback: read from recently-added.json if catalog diff found nothing
    recent_file = os.path.join(SCRIPT_DIR, "recently-added.json")
    if not recently_added and os.path.exists(recent_file):
        try:
            with open(recent_file, "r", encoding="utf-8") as f:
                recently_added = json.load(f)
            print(f"Recently added: {len(recently_added)} UCs loaded from recently-added.json")
    ...
    _atomic_write(OUTPUT_CATALOG_JSON, json.dumps(catalog, ensure_ascii=False, indent=2))
