...
class TestConstants:
    ...
    def test_expected_cats_1_through_25(self) -> None:
        assert rc.EXPECTED_CATS == set(range(1, 26))
...
def isolated_repo(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    content.mkdir()
    # Create all 25 expected category folders.
    for n in range(1, 26):
...
class TestMainFailureModes:
    ...
    def test_cat_groups_missing_category_flagged(
        ...
        def fake_extract() -> tuple[dict[str, list[int]], list[Any]]:
            ...
            return {"core": [n for n in range(1, 26) if n != 5]}, []
    ...
    def test_cat_groups_duplicate_category_flagged(
        ...
        def fake_extract() -> tuple[dict[str, list[int]], list[Any]]:
            return {"core": list(range(1, 26)), "extra": [5, 5]}, []
    ...
    def test_cat_groups_unexpected_id_flagged(
        ...
        def fake_extract() -> tuple[dict[str, list[int]], list[Any]]:
            return {"core": [*list(range(1, 26)), 99]}, []
    ...
    def test_uc_file_with_invalid_category_id(
        ...
        rc_code = rc.main([])
        out = capsys.readouterr().out
        assert "UC-99.1.1 category 99 not in 1-25" in out
        assert rc_code == 2
    ...
    def test_splunk_app_not_a_dict(
        ...
        def fake_extract() -> tuple[dict[str, list[int]], list[Any]]:
            return {"core": list(range(1, 26))}, ["not a dict"]
