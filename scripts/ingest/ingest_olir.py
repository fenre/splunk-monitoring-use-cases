#!/usr/bin/env python3
"""Compatibility shim — delegates to ``splunk_uc.ingest.olir``.
...
``scripts/ingest/ingest_olir.py`` invocation alive while the new
dispatcher (``python -m splunk_uc ingest-olir``) becomes the
...
from splunk_uc.ingest.olir import main, run
