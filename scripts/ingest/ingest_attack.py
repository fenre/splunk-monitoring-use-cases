#!/usr/bin/env python3
"""Compatibility shim — delegates to ``splunk_uc.ingest.attack``.
...
``scripts/ingest/ingest_attack.py`` invocation alive while the new
dispatcher (``python -m splunk_uc ingest-attack``) becomes the
...
from splunk_uc.ingest.attack import main, run
