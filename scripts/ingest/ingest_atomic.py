#!/usr/bin/env python3
"""Compatibility shim — delegates to ``splunk_uc.ingest.atomic``.
...
``scripts/ingest/ingest_atomic.py`` invocation alive while the new
dispatcher (``python -m splunk_uc ingest-atomic``) becomes the
...
from splunk_uc.ingest.atomic import main, run
