...
The v1 audit (`scripts/audit_gold_profile.py`) measures the baseline Gold
tier defined in `schemas/uc-profile-gold.json`. This v2 audit lifts the bar
to the depth demonstrated by the catalogue exemplar UC-1.1.1 and required
by `docs/gold-standard-authoring-playbook.md`.
...
Usage:
    python3 scripts/audit_gold_profile_v2.py
    python3 scripts/audit_gold_profile_v2.py --check
    python3 scripts/audit_gold_profile_v2.py --files <paths>
    python3 scripts/audit_gold_profile_v2.py --regulation NIS2
    python3 scripts/audit_gold_profile_v2.py --pack data/nis2-domain-packs.json --regulation NIS2
...
SPLUNKBASE_ID_RE = re.compile(r"Splunkbase\s+\d{2,5}|splunkbase\.splunk\.com/app/\d+", re.IGNORECASE)
...
API_PATH_RE = re.compile(r"/(?:api|dna|services|servicesNS|now/table|graph)/[a-zA-Z0-9_\-/.]*", re.IGNORECASE)
RBAC_RE = re.compile(r"\b(?:RBAC|role|admin-role|reader-role|writer-role|service[- ]account|API\s+token|OAuth)", re.IGNORECASE)
TIMEBOUND_RE = re.compile(r"\b\d+\s*(?:second|minute|hour|day|week|month)s?\b", re.IGNORECASE)
MODULAR_INPUT_RE = re.compile(r"(?:modular\s+input|inputs\.conf|HEC|forwarder|scripted\s+input)", re.IGNORECASE)
VENDOR_UI_RE = re.compile(r"(?:Settings|Configuration|Configure|Console|Portal|Dashboard|Inventory|Admin|Activity|System)\s*>\s*\w+", re.IGNORECASE)
SUPPRESSION_RE = re.compile(r"(?:exception\s+register|nis2_\w+\.csv|time-bound\s+exception|where\s+\w+|lookup\s+\w+|allow[- ]list|block[- ]list|filter\s+the\s+spl)", re.IGNORECASE)
NAMED_PRODUCT_RE = re.compile(r"\b(?:Veeam|Commvault|Rubrik|Cohesity|CyberArk|Vault|Okta|Entra|Azure|Microsoft|Cisco|Tenable|Qualys|Rapid7|KnowBe4|Cornerstone|ServiceNow|Phantom|SOAR|Stream|Cyber\s+Vision|Nozomi|Claroty|Defender|Sentinel|Splunk|ES|ITSI|GRC|CMDB|Workday)\b")
KFP_SEPARATOR_RE = re.compile(r"(?:^\s*[•\-\*\d+\.\)]\s+)|(?:^\s*[A-Z][^\.]+:\s)|(?:\.\s+[A-Z][a-z]+\s+(?:scenario|window|case|exception|maintenance|rotation|holiday|cycle|onboarding))", re.MULTILINE)
...
def _count_unique_specifics(text: str) -> int:
    ...
    if not text:
        return 0
    found: set[str] = set()
    for pat in (SOURCETYPE_RE, INDEX_RE, API_PATH_RE, MODULAR_INPUT_RE, VENDOR_UI_RE, RBAC_RE, TIMEBOUND_RE, SPLUNKBASE_ID_RE):
...
def _has_field_list(text: str) -> bool:
    ...
    return bool(re.search(r"[a-z][a-zA-Z_]+\s*,\s*[a-z][a-zA-Z_]+\s*,\s*[a-z][a-zA-Z_]+", text or ""))
...
def audit_uc_v2(uc: dict[str, Any], filepath: Path) -> dict[str, Any]:
    ...
    if di_chars < V2_THRESHOLDS["detailedImplementation_min_chars"]:
        gaps.append(f"detailedImplementation {di_chars} chars (need >= {V2_THRESHOLDS['detailedImplementation_min_chars']})")
        out["score"] -= 30
    unique_specifics = _count_unique_specifics(di)
    if unique_specifics < V2_THRESHOLDS["di_unique_specifics_min"]:
        gaps.append(f"detailedImplementation only has {unique_specifics} unique product-specific signals (need >= {V2_THRESHOLDS['di_unique_specifics_min']})")
        out["score"] -= 15
    ...
    if kfp_count < V2_THRESHOLDS["kfp_min_scenarios"]:
        gaps.append(f"knownFalsePositives lists {kfp_count} named scenarios (need >= {V2_THRESHOLDS['kfp_min_scenarios']})")
        out["score"] -= 15
    if not _has_suppression_mechanism(kfp):
        gaps.append("knownFalsePositives does not name a suppression mechanism (lookup, exception register, time-bound exception, filter)")
        out["score"] -= 5

    # dataSources depth -------------------------------------------------
    ds = uc.get("dataSources", "") or ""
    if len(ds) < V2_THRESHOLDS["datasources_min_chars"]:
        gaps.append(f"dataSources {len(ds)} chars (need >= {V2_THRESHOLDS['datasources_min_chars']})")
        out["score"] -= 5
    ...
    if sim > 0.6:
        gaps.append(f"description and value share {sim:.0%} word stems (should be < 60% — they describe different things)")
        out["score"] -= 10

    # references ---------------------------------------------------------
    refs = uc.get("references", []) or []
    if len(refs) < V2_THRESHOLDS["references_min"]:
        gaps.append(f"references has {len(refs)} entries (need >= {V2_THRESHOLDS['references_min']})")
        out["score"] -= 5
    ...
    if pos and neg:
        ...
        if diff < V2_THRESHOLDS["control_test_diff_min_chars"] and sim_ct > 0.7:
            gaps.append("controlTest positive/negative scenarios are too similar — likely placeholder text")
            out["score"] -= 5
    else:
            ...
            for token in re.split(r"\s+", ta_lower):
                if len(token) >= 6 and token in text:
                    ...
                    if not any(canon in text for canon in (ta_lower,)):
                        drift.append(f"Pack `{pname}`: text mentions `{token}` but not the canonical TA name (`{ta_lower}`)")
                        break
    return drift
...
def print_report(results: list[dict]) -> None:
    ...
    print(f"  PASS (score >= {V2_THRESHOLDS['passing_score']}):  {pass_n} ({pass_n/total*100:.1f}%)")
...
def main() -> int:
    ...
    ap.add_argument("--json", action="store_true", help="Emit JSON report instead of human-readable table")
    args = ap.parse_args()
