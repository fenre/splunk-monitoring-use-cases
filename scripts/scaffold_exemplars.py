#!/usr/bin/env python3
"""scripts/scaffold_exemplars.py — Phase 1.6 authoring driver.
...
Running the script is idempotent: sidecars are overwritten and the markdown
file is rewritten between the ``<!-- PHASE-1.6 BEGIN -->`` and
``<!-- PHASE-1.6 END -->`` sentinels.
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    for uc in UCS:
        if uc.get("markdown_only"):
            # Authoritative sidecar already exists and was vetted separately.
            continue
        _write_sidecar(uc)
        _write_fixture(uc)
        sidecar_count += 1
    ...
    print(
        ...
        f"({len([u for u in UCS if u.get('markdown_only')])} markdown-only) "
        f"and regenerated Phase 1.6 markdown block."
    )
    return 0
