    ...
    for path in sorted(CONTENT_ROOT.glob("cat-*/UC-*.json")):
        ...
        for entry in doc.get("compliance", []) or []:
            if str(entry.get("regulation", "")).strip().lower() == "nis2":
                entries.append((str(doc.get("id", path.stem)), str(path.relative_to(REPO_ROOT)), entry))
    return entries
...
def _validate_matrix(matrix: dict[str, Any], source_map: dict[str, Any]) -> list[str]:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if not isinstance(refs, list) or not refs:
            ...
            first = refs[0]
            if not isinstance(first, dict) or first.get("type") not in {"binding-law", "official-guidance", "national-guidance"}:
    ...
    required_clauses = {"Art.20(1)", "Art.20(2)", "Art.21(2)(a)", "Art.21(2)(j)", "Art.23(4)(a)", "Annex 3.2", "Annex I", "Annex II"}
