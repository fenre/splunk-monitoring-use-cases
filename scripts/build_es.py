...
def _fits_correlation_search(spl: str) -> bool:
    ...
    low = spl.strip().lower()
    if not (
        low.startswith("index=") or low.startswith("| tstats") or low.startswith("search index=")
    ):
        return False
...
def _uc_value_one_line(uc: dict) -> str:
    ...
    return (uc.get("v") or "").strip().replace("\n", " ").replace("\r", "")
...
CONF_HEADER = (
    ...
    "# Source of truth: catalog.json + content/cat-*/UC-*.json.\n"
...
def render_savedsearches(ucs: list[dict]) -> str:
    ...
    for uc in ucs:
        ...
        out.append(f"[{title}]")
        out.append(f"description = UC-{uid}: {uc.get('n', '')}. {_uc_value_one_line(uc)}")
        ...
        if uc.get("mitre"):
            out.append(f"action.notable.param.mitre_attack_id = {','.join(uc['mitre'])}")
        if uc.get("kfp"):
            out.append(
...
def render_governance(ucs: list[dict]) -> str:
    ...
    for uc in ucs:
        ...
        out.append("governance = mitre_attack")
        out.append(f"mitre_attack = {','.join(mitre)}")
        # Derive tactics from technique prefixes.
        tactics = sorted(
            {
                TACTIC_BY_PREFIX[t.split(".", 1)[0]]
                for t in mitre
                if t.split(".", 1)[0] in TACTIC_BY_PREFIX
            }
        )
        if tactics:
...
def render_analytic_stories(ucs: list[dict]) -> str:
    ...
    for tactic, names in sorted(by_tactic.items()):
        ...
        out.append(
            f"description = Correlation searches that map to the MITRE ATT&CK ``{tactic}`` tactic."
        )
        out.append(
            "narrative = Curated detections derived from the splunk-monitoring-use-cases catalog."
        )
...
EVENTTYPES_ES: list[tuple[str, str, str]] = [
    ...
    (
        "ess_privilege_escalation",
        "tag=change tag=account tag=modified",
        "CIM: account privilege changes",
    ),
...
def render_tags() -> str:
    ...
    pairs = [
        ...
        (
            "eventtype=ess_privilege_escalation",
            [("account", "enabled"), ("change", "enabled"), ("modified", "enabled")],
        ),
        ...
        (
            "eventtype=ess_dns",
            [("network", "enabled"), ("resolution", "enabled"), ("dns", "enabled")],
        ),
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if result.returncode != 0:
                ...
                if not out and result.returncode != 0:
                    sys.stderr.write(
                        f"(diff exited non-zero; compare {tmp_default!r} vs {DEFAULT_DIR!r})\n"
                    )
                return 1
