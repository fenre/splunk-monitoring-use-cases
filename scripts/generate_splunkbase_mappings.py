...
def _equipment_match(
    uc: Dict[str, Any], catalog: Dict[str, Dict[str, Any]]
) -> List[int]:
...
def _discover_ucs(filter_uc_id: Optional[str], filter_cat: Optional[str]) -> List[pathlib.Path]:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        "--cat", help="Limit to a single category number (e.g. '5' or '05').",
    )
    parser.add_argument(
        "--quiet", action="store_true", help="Suppress per-UC stdout logging.",
    )
