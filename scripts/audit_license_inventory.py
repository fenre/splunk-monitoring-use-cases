...
Repo-overhaul plan §P11 (2026-05-09): every Python dependency declared
...
def render_markdown(inventory: dict[str, Any]) -> str:
    ...
    lines.append("")
    lines.append(f"- **Repo licence**: `{inventory['repo_license']['spdx']}` "
    ...
    if not inventory["vendored_licenses"]:
        ...
        for v in inventory["vendored_licenses"]:
            lines.append(
                f"| `{v['path']}` | `{v['spdx']}` | {_md_cell(v['subject'])} |"
            )
    lines.append("")
    return "\n".join(lines)
