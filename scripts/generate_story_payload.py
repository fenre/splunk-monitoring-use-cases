#!/usr/bin/env python3
"""Compatibility shim — delegates to ``splunk_uc.generators.story_payload``.
...
``scripts/generate_story_payload.py`` invocation alive while the new
dispatcher (``python -m splunk_uc generate-story-payload``) becomes the
...
from splunk_uc.generators.story_payload import generate, main
